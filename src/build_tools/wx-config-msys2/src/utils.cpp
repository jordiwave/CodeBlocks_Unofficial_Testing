        ...
        if (arg.starts_with("--prefix"))
        {
            ...
            if (arg.starts_with("--wxcfg"))
            {
                ...
                if (arg.starts_with("--libs"))
                {
                    ...
                    if (arg.starts_with("--cflags") || arg.starts_with("--cxxflags"))
                    {
                        set_is_cxxflags();
                    }
                    else
                        if (arg.starts_with("--rcflags"))
                        {
                            set_is_rcflags();
                        }
                        else
...
void CommandLineParser::parse_libs(const string & libs)
{
    ...
    auto has_base = find_if(
                        vlibs.begin(), vlibs.end(), [](const string & lib)
    {
        return lib == "base" || lib == "all" || lib == "std";
    });
