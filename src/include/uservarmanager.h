...
class UserVarManagerUI
{
    public:
        ...
        /** \brief Ask the user for a variable with the name, description and default value
         *
         * \param name const wxString& Variable name
         * \param desc const wxString& description for the variable
         * \param def const wxString& default value of the variable
         *
         */
        virtual bool AskUserForVariable(const wxString & name, const wxString & desc, const wxString & def, wxString & value) = 0;
};
...
static const std::vector<wxString> cBuiltinMembers = {"base", "include", "lib", "obj", "bin", "cflags", "lflags"};
static const wxString cSets(_T("/sets/"));
static const wxString cDir(_T("dir"));

static const wxString defaultSetName(_T("default"));

bool IsBuildIn(const wxString & member);
}
...
class DLLIMPORT UserVariableManager : public Mgr<UserVariableManager>
{
        struct PreemtEntry
        {
            PreemtEntry(const wxString & var, const ProjectBuildTarget * target) : m_var(var), m_target(target)
            {
                UserVariableManager::ParseVariableName(var, m_varName, m_memberName);
            }

            PreemtEntry(const wxString & var, const wxString & varName, const wxString & memberName, const ProjectBuildTarget * target) :
                m_var(var),
                m_varName(varName),
            ...
            const ProjectBuildTarget * m_target;

            bool operator==(const PreemtEntry & lh)
            {
                return m_var == lh.m_var && m_target == lh.m_target;
            }
        };
        ...
        std::vector<PreemtEntry>   m_Preempted;     //!< A list with all found but not existing variables that occurred during project loading.
         ...
         */
        static bool ParseVariableName(const wxString & variable, wxString & varName, wxString & memberName);
    ...
    public:
         ...
         * \param[in] build target from what the replace is asked, or nullptr
         * \param[out] errorMessages
         * \return The result string or an empty string
         */
        wxString Replace(const wxString & variable, const ProjectBuildTarget * target = nullptr, std::vector<wxString> * errorMessages = nullptr);
         ...
         */
        void Preempt(const wxString & variable, const ProjectBuildTarget * target);

        /** \brief Ask user for all non existent variables collected with \ref Preempt(const wxString& variable)
         */
        void Arrogate();

        /** \brief Search for global variables in a given string and put them in the output set
         *
         * \param[in] searchIn String to search variables in
         * \param[out] out set that stores unique variables found in searchIn string
         */
        void CollectVariableNames(wxString searchIn, std::set<wxString> & out) const;

        /** \brief Ask the user for a Uservariable with the defaults and description defined in the target (project)
         *
         * \param var const wxString& Variable
         * \param target const ProjectBuildTarget* target in what the defaults and descriptions should be searched
         * \return virtual bool true if user entered the variable
         *
         */
        virtual bool AskForVariable(const wxString & var, const ProjectBuildTarget * target);
