    ...
    for (size_t i = 0; i < indices.size(); i++)
    {
        ...
        else
        {
            ...
            if (menu.at(index).kind == MenuEntry::mekNormal)
            {
                ...
                if (menu.at(index).kind == MenuEntry::mekRadio)
                {
                    out_menu.AppendRadioItem(id, menu.at(index).text, menu.at(index).help);
                }
                else
                    if (menu.at(index).kind == MenuEntry::mekCheck)
                    {
    ...
    for (size_t i = 0; i < menu.size(); i++)
    {
        ...
        if (required)
        {
            if (id == -1)
            {
                ...
                if (menu.at(i).kind == MenuEntry::mekNormal)
                {
                    out_menu.Append(id, menu.at(i).text, menu.at(i).help);
                }
                else
                    if (menu.at(i).kind == MenuEntry::mekRadio)
                    {
                        out_menu.AppendRadioItem(id, menu.at(i).text, menu.at(i).help);
                    }
                    else
                        if (menu.at(i).kind == MenuEntry::mekCheck)
                        {
