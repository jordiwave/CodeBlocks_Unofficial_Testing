...
{
    if(curr_line == 0) return -1;

    if(linesWithDifferences.empty()) return -1;
    ...
    else
    {
        ...
        if(itr == linesWithDifferences.begin()) return -1;
        return *(--itr);
    }
}
...
{
    if(linesWithDifferences.empty()) return false;
