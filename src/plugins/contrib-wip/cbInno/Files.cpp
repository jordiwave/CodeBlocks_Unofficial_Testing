...
void CFiles::Analize(const wxString & content, const wxString & line)
{
    ...
    while (!cont.empty())
    {
        ...
        if (part.CmpNoCase(_T("source")) == 0)
        {
            ...
            if (part.CmpNoCase(_T("DestDir")) == 0)
            {
                SetDestDir(settings);
            }
            else
                if (part.CmpNoCase(_T("destname")) == 0)
                {
                    SetDestName(settings);
                }
                else
                    if (part.CmpNoCase(_T("excludes")) == 0)
                    {
                        SetExcludes(settings);
                    }
                    else
                        if (part.CmpNoCase(_T("ExternalSize")) == 0)
                        {
                            SetExternalSize(settings);
                        }
                        else
                            if (part.CmpNoCase(_T("CopyMode")) == 0)
                            {
                                ...
                                if (part.CmpNoCase(_T("Attribs")) == 0)
                                {
                                    ...
                                    if (part.CmpNoCase(_T("Permissions")) == 0)
                                    {
                                        ...
                                        if (part.CmpNoCase(_T("FontInstall")) == 0)
                                        {
                                            SetFontInstall(settings);
                                        }
                                        else
                                            if (part.CmpNoCase(_T("StrongAssemblyName")) == 0)
                                            {
