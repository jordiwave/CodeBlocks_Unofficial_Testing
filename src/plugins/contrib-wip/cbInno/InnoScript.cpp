...
void InnoScript::AnalizePreprocessor(const wxString & content)
{
    ...
    if (content.StartsWith(_T("include"), &con))
    {
        ...
        if (content.StartsWith(_T("define"), &con))
        {
        }
        else
            if (content.StartsWith(_T("undef"), &con))
            {
            }
            else
                if (content.StartsWith(_T("file"), &con))
                {
                }
                else
                    if (content.StartsWith(_T("emit"), &con))
                    {
                    }
                    else
                        if (content.StartsWith(_T("expr"), &con))
                        {
                        }
                        else
                            if (content.StartsWith(_T("insert"), &con))
                            {
                            }
                            else
                                if (content.StartsWith(_T("append"), &con))
                                {
                                }
                                else
                                    if (content.StartsWith(_T("for"), &con))
                                    {
                                    }
                                    else
                                        if (content.StartsWith(_T("pragma"), &con))
                                        {
                                        }
                                        else
                                            if (content.StartsWith(_T("error"), &con))
                                            {
                                            }
}
...
void InnoScript::AnalizeSection(const wxString & content, const wxString & line)
{
    if (content.CmpNoCase(_T("setup")) == 0)
    {
        ...
        if (content.CmpNoCase(_T("types")) == 0)
        {
            m_part = TYPES;
        }
        else
            if (content.CmpNoCase(_T("components")) == 0)
            {
                m_part = COMPONENTS;
            }
            else
                if (content.CmpNoCase(_T("tasks")) == 0)
                {
                    m_part = TASKS;
                }
                else
                    if (content.CmpNoCase(_T("dirs")) == 0)
                    {
                        m_part = FOLDERS;
                    }
                    else
                        if (content.CmpNoCase(_T("files")) == 0)
                        {
                            m_part = FILES;
                        }
                        else
                            if (content.CmpNoCase(_T("icons")) == 0)
                            {
                                m_part = ICONS;
                            }
                            else
                                if (content.CmpNoCase(_T("ini")) == 0)
                                {
                                    m_part = INI;
                                }
                                else
                                    if (content.CmpNoCase(_T("installdelete")) == 0)
                                    {
                                        m_part = INSTALLDEL;
                                    }
                                    else
                                        if (content.CmpNoCase(_T("languages")) == 0)
                                        {
                                            m_part = LANGUAGES;
                                        }
                                        else
                                            if (content.CmpNoCase(_T("messages")) == 0)
                                            {
                                                m_part = MESSAGES;
                                            }
                                            else
                                                if (content.CmpNoCase(_T("custommessages")) == 0)
                                                {
                                                    m_part = CMESSAGES;
                                                }
                                                else
                                                    if (content.CmpNoCase(_T("langoptions")) == 0)
                                                    {
                                                        m_part = LANGOPT;
                                                    }
                                                    else
                                                        if (content.CmpNoCase(_T("registry")) == 0)
                                                        {
                                                            m_part = REGISTRY;
                                                        }
                                                        else
                                                            if (content.CmpNoCase(_T("run")) == 0)
                                                            {
                                                                m_part = RUN;
                                                            }
                                                            else
                                                                if (content.CmpNoCase(_T("uninstalldelete")) == 0)
                                                                {
                                                                    m_part = UNINSTALLDEL;
                                                                }
                                                                else
                                                                    if (content.CmpNoCase(_T("uninstallrun")) == 0)
                                                                    {
                                                                        m_part = UNINSTALL;
                                                                    }
                                                                    else
                                                                        if (content.CmpNoCase(_T("code")) == 0)
                                                                        {
                                                                            m_part = PASCALCODE;
                                                                        }
                                                                        else
