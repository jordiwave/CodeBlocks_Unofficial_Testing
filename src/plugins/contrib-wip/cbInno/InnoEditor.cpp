...
//---- Icons ---------------------------------------------------------------------------------------
#include "inno.xpm"
#include "resources/Images/components.xpm"
#include "resources/Images/custom_msg.xpm"
#include "resources/Images/files.xpm"
#include "resources/Images/folder.xpm"
#include "resources/Images/icon.xpm"
#include "resources/Images/ini.xpm"
#include "resources/Images/install.xpm"
#include "resources/Images/install_del.xpm"
#include "resources/Images/language.xpm"
#include "resources/Images/msg.xpm"
#include "resources/Images/registry.xpm"
#include "resources/Images/pas.xpm"
#include "resources/Images/script.xpm"
#include "resources/Images/steps.xpm"
#include "resources/Images/tasks.xpm"
#include "resources/Images/types.xpm"
#include "resources/Images/uninstall.xpm"
#include "resources/Images/uninstall_del.xpm"
#include "resources/Images/run.xpm"
//----  --------------------------------------------------------------------------------------------
...
void InnoEditor::OnTreeCtrl1ItemActivated(wxTreeEvent & event)
{
    ...
    else
    {
        ...
        if (activated.compare(_T("Files")) == 0)
        {
            ...
            if (activated.compare(_T("Folders")) == 0)
            {
                inno_part = FOLDERS;
            }
            else
                if (activated.compare(_T("Icons")) == 0)
                {
                    inno_part = ICONS;
                }
                else
                    if (activated.compare(_T("Ini")) == 0)
                    {
                        inno_part = INI;
                    }
                    else
                        if (activated.compare(_T("Tasks")) == 0)
                        {
                            inno_part = TASKS;
                        }
                        else
                            if (activated.compare(_T("Registry")) == 0)
                            {
                                inno_part = REGISTRY;
                            }
                            else
                                if (activated.compare(_T("Language")) == 0)
                                {
                                    inno_part = LANGUAGES;
                                }
                                else
                                    if (activated.compare(_T("Types")) == 0)
                                    {
                                        inno_part = TYPES;
                                    }
                                    else
                                        if (activated.compare(_T("Components")) == 0)
                                        {
                                            inno_part = COMPONENTS;
                                        }
                                        else
                                            if (activated.compare(_T("Messages")) == 0)
                                            {
                                                inno_part = MESSAGES;
                                            }
                                            else
                                                if (activated.compare(_T("Custom Messages")) == 0)
                                                {
                                                    inno_part = CMESSAGES;
                                                }
                                                else
                                                    if (activated.compare(_T("Pascal Code")) == 0)
                                                    {
                                                        inno_part = PASCALCODE;
                                                    }
                                                    else
                                                        if (activated.compare(_T("Install Run")) == 0)
                                                        {
                                                            inno_part = RUN;
                                                        }
                                                        else
                                                            if (activated.compare(_T("Install Delete")) == 0)
                                                            {
                                                                inno_part = INSTALLDEL;
                                                            }
                                                            else
                                                                if (activated.compare(_T("Uninstall Run")) == 0)
                                                                {
                                                                    inno_part = UNINSTALL;
                                                                }
                                                                else
                                                                    if (activated.compare(_T("Uninstall Delete")) == 0)
                                                                    {
                                                                        inno_part = UNINSTALLDEL;
                                                                    }
                                                                    else
                                                                        if (activated.compare(_T("Pre Compilation Steps")) == 0)
                                                                        {
                                                                            inno_part = PRE;
                                                                        }
                                                                        else
                                                                            if (activated.compare(_T("Post Compilation Steps")) == 0)
                                                                            {
                                                                                inno_part = POST;
                                                                            }
