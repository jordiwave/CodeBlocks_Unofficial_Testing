...
wxString & StringUtils::Rtrim(wxString & str)
{
    return str.Trim();
}

wxString & StringUtils::Ltrim(wxString & str)
{
    return str.Trim(false);
}

wxString & StringUtils::Trim(wxString & str)
{
    return str.Trim().Trim(false);
}
        ...
        switch (state)
        {
            ...
            case ARGV_STATE_ESCAPE:
            {
                if (prev_state == ARGV_STATE_DQUOTE)
                {
                    ...
                    if (prev_state == ARGV_STATE_BACKTICK)
                    {
                        switch (ch)
                        {
                            case '`':
...
const wxString & std::to_string(const wxString & str)
{
    return str;
}

wxString & StringUtils::ToNativePath(wxString & path)
{
    return ConvertSlashes(path, SOURCE_SEP, PATH_SEP);
}

wxString & StringUtils::ToUnixPath(wxString & path)
{
