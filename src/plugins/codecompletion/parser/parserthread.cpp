        ...
        switch (token.Length())
        {
                ...
                switch (static_cast<wxChar>(token[0]))
                {
                    ...
                    case ParserConsts::colon_chr:
                    {
                        if (m_LastToken == ParserConsts::kw_public)
                        {
                            ...
                            if (m_LastToken == ParserConsts::kw_protected)
                            {
                                m_LastScope = tsProtected;
                            }
                            else
                                if (m_LastToken == ParserConsts::kw_private)
                                {
            ...
            case 4:
                if (token == ParserConsts::kw_else)
                {
                    ...
                    if (token == ParserConsts::kw_enum)
                    {
                        ...
                        if (token == ParserConsts::kw_case)
                        {
                            m_Str.Clear();
                            SkipToOneOfChars(ParserConsts::colon, true, true);
                        }
                        else
                            if (token == ParserConsts::kw___at)
                            {
            ...
            case 6:
                if (token == ParserConsts::kw_delete)
                {
                    ...
                    if (token == ParserConsts::kw_switch)
                    {
                        if (!m_Options.useBuffer || m_Options.bufferSkipBlocks)
                        {
                            SkipToOneOfChars(ParserConsts::semicolonclbrace, true, true);
                        ...
                        if (token == ParserConsts::kw_return)
                        {
                            SkipToOneOfChars(ParserConsts::semicolonclbrace, true, true);
                            m_Str.Clear();
                        }
                        else
                            if (token == ParserConsts::kw_extern)
                            {
                                // check for "C", "C++"
                                m_Str = m_Tokenizer.GetToken();
                                        ...
                                        || token == ParserConsts::kw_inline)
                                {
                                    ...
                                    if (token == ParserConsts::kw_friend)
                                    {
                                        ...
                                        if (token == ParserConsts::kw_struct)
                                        {
                                            m_Str.Clear();
            ...
            case 7:
                if (token == ParserConsts::kw_typedef)
                {
                    ...
                    if (token == ParserConsts::kw_virtual)
                    {
            ...
            case 8:
                if (token == ParserConsts::kw_template)
                {
                    ...
                    if (token == ParserConsts::kw_noexcept)
                    {
                        m_Str << token << _T(" ");
                    }
                    else
                        if (token == ParserConsts::kw_operator)
                        {
                            wxString func = token;
                                    ...
                                    // NOTE: opbracket_chr already handled above
                                                ...
                                                || peek == ParserConsts::equals_chr)
                                        {
                                            ...
                                            if (peek == ParserConsts::oparray_chr)
                                            {
                                                ...
                                                if (peek == ParserConsts::equals_chr)
                                                {
                                                    SkipToOneOfChars(ParserConsts::commasemicolonopbrace, true);
                                                    m_Tokenizer.UngetToken();
                                                }
                                            ...
                                            if (!m_EncounteredNamespaces.empty())
                                            {
...
void ParserThread::HandleIncludes()
{
    ...
    if (!token.IsEmpty())
    {
        if (token.GetChar(0) == '"')
        {
            ...
            if (token.GetChar(0) == ParserConsts::lt_chr)
            {
                isGlobal = true;
