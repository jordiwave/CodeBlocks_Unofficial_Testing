...
// ----------------------------------------------------------------------------
{
    ...
    private:
        ...
        bool GetLSP_Initialized()
        {
            return m_LSP_initialized;
        }
        void SetLSP_Initialized(bool trueOrFalse)
        {
            m_LSP_initialized = trueOrFalse;
        }
        // Return ptr to server process for this client instance
#if defined(_WIN32)
        IProcess * GetLSP_Server()
        {
            return m_pServerProcess;
        }
#else
        UnixProcess * GetLSP_Server()
        {
            return m_pServerProcess;
        }
#endif
    ...
    public:
        ...
        size_t LSP_GetServerFilesParsingCount()
        {
            return m_LSP_CurrBackgroundFilesParsing.size();
        }
    ...
    public:
        ...
        long       GetLSP_Server_PID()
        {
            return processServerPID;
        }
        ...
        bool IsAlive()
        {
            return (kill(GetLSP_Server_PID(), 0)) == 0;
        }
#endif
        ...
        LSPDiagnosticsResultsLog * LSP_GetLog()
        {
            return m_pDiagnosticsLog;
        }

        // ID to use when defining clinet/server events
        void       SetLSP_UserEventID(int id)
        {
            m_LSP_UserEventID = id;
        }
        int        GetLSP_UserEventID()
        {
            return m_LSP_UserEventID;
        }

        // The project respresented by this client/server
        void       SetCBProject(cbProject * pProject)
        {
            m_pCBProject = pProject;
        }
        void       SetParser(Parser * pParser)
        {
            m_pParser = pParser;   //(ph 2022/03/30)
        }
        cbProject * GetClientsCBProject()
        {
            return m_pCBProject;
        }
...
#define EDITOR_STATUS_MODIFIED 3        //Is modified
        ...
        void  SetCompileCommandsChangedTime(bool trueOrFalse)
        {
            ...
            {
                m_LSP_CompileCommandsChangedTime = 0;
            }
        }
    ...
    public:
        ...
        bool validUtf8(std::vector<int> & data, vector<int> & badLocs)
        {
            for (int i = 0; i < int(data.size()); ++ i)
            {
                ...
                {
                    /*return false;*/badLocs.push_back(i);
                    continue;
                }
            }
