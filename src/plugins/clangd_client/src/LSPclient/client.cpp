...
// ----------------------------------------------------------------------------
{
    ...
    {
        clangd_Dir = clangLocator.Locate_ClangdDir();
    }
    ...
    {
        wxString msg;
        msg << "clangd_client plugin could not auto detect a clangd installation.";
        ...
        msg.Empty();
        msg << "Error: " << __FUNCTION__ << "() Could not find clangd installation.";

        if (clangd_Dir.empty())
        {
            msg << " clangd dir not found.";
        }

        if (clangdResourceDir.empty())
        {
    ...
    processServerPID =  m_pServerProcess->GetPid();
    // -------------------------------------------------------
...
// ----------------------------------------------------------------------------
{
    ...
    if (m_pServerProcess)
    {
        m_pServerProcess->Detach();    //ignore any further messages //(ph 2021/07/8)
    }
...
// ----------------------------------------------------------------------------
void ProcessLanguageClient::OnLSP_Terminated(wxThreadEvent & event_pipedprocess_terminated)
// ----------------------------------------------------------------------------
{
    ...
    {
        processExitCode = -1;    //terminated while initialized and running.
    }
        ...
        if (pJson->size())
        {
            if (pJson->count("id"))
            {
                ...
                if (pJson->contains("method"))
                {
                    ...
                    if (pJson->contains("Exit!")) //(ph 2020/09/26)
                    {
                        //This never occurs
                    }
        }
        ...
        if (idValue == "initialize")
        {
            ...
            if (idValue == "shutdown")
            {
                        ...
                        or idValue.StartsWith("textDocument/definition"))
                {
                    ...
                    if (idValue.StartsWith("textDocument/references"))
                    {
                        ...
                        if (idValue.StartsWith("textDocument/documentSymbol"))
                        {
                            ...
                            if (idValue.StartsWith("textDocument/completion"))
                            {
                                lspevt.SetString(idValue + STX + "result");
                                SetCompletionTimeBusy(0);
                            }
                            else
                                if (idValue.StartsWith("textDocument/hover"))
                                {
                                    lspevt.SetString(idValue + STX + "result");
                                }
                                else
                                    if (idValue.StartsWith("textDocument/signatureHelp"))
                                    {
                                        lspevt.SetString(idValue + STX + "result");
                                    }
                                    else
                                        if (idValue.StartsWith("textDocument/rename"))
                                        {
                                            lspevt.SetString(idValue + STX + "result");
                                        }
    }//endif "id"
...
// ----------------------------------------------------------------------------
{
    ...
    if (Has_LSPServerProcess())
    {
        ...
        if (m_pServerProcess)
        {
            m_pServerProcess->Detach();    //throw away any further responses
        }
...
// ----------------------------------------------------------------------------
{
    ...
    if (pcbEd)
    {
        ...
        if ((not pProjectFile) or (not pEdProject))
        {
            ...
            if (pEdProject == GetClientsCBProject()) //GetCBProject returns client project
            {
                owned = true;
            }
            else
                if (pEdProject->GetTitle() == "~ProxyProject~") //for non-owned project files
                {
...
// ----------------------------------------------------------------------------
{
    ...
    if ((not pEd) or (not ClientProjectOwnsFile(pEd)))
    {
        return;
    }
...
// ----------------------------------------------------------------------------
{
    ...
    if (not platform::windows)
    {
        compileCommandsFullPath.Replace("\\", "/");
    }

    if (wxFileExists(compileCommandsFullPath))
        switch (1)
        {
            default:
...
// ----------------------------------------------------------------------------
{
    ...
    if (logRequest == _("Build log"))
    {
        ...
        if (logRequest == _("Build messages"))
        {
            return lm_BuildMsgLogIndex;
        }
        else
            if (logRequest == _("Debugger"))
            {
                return lm_DebuggerLogIndex;
            }
            else
                if (logRequest == _("Debugger (debug)"))
                {
                    return lm_DebuggersDebugLogIndex;
                }
                else
                    if (logRequest == _("Code::Blocks Debug"))
                    {
                        return lm_CodeBlocksDebugLogIndex;
                    }
                    else
                        if (logRequest == _("Search results"))
                        {
                            return lm_CodeBlocksSearchResultsLogIndex;
                        }
                        else
                            if (logRequest == _("LSP messages"))
                            {
