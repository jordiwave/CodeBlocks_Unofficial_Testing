...
#include "stringutils.h"    //(ph 2020/09/26)
...
#include "manager.h" //(ph 2020/12/15)
...
template <typename T> bool WriteStdin(const T& buffer, HANDLE hStdin, HANDLE hProcess)
{
    ...
    // ?? dwMode = PIPE_READMODE_BYTE | PIPE_NOWAIT;  //removing this made the IO almost instantaneous //(ph 2020/12/15)
    ...
    while(bytesLeft > 0 && (retryCount < 100)) {
        ...
        if(!WriteFile(hStdin, buffer.c_str() + offset, bytesLeft, &dwWritten, NULL)) {
            int errorCode = GetLastError(); wxUnusedVar(errorCode);
        ...
        // The following not necessary after removeing dwMode set above //(ph 2020/12/15)
        //writing large files overruns the pipe causing unreported file truncation.
        // if (retryCount & 16)    //delay every 64k (16 buffs * 4k per buff)    //(ph 2020/12/15)
            ;//std::this_thread::sleep_for(std::chrono::milliseconds(250));      //(ph 2020/12/15)
        //-Manager::Yield();
    }
    return true;
}
...
{
    // Sanity
    return Write(ToStdString(buff));
}

bool WinProcessImpl::Write(const std::string& buff) { return WriteRaw(buff + "\r\n"); }

bool WinProcessImpl::WriteRaw(const wxString& buff) { return WriteRaw(ToStdString(buff)); }
