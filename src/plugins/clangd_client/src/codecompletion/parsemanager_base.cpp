 ...
 *
 * $Revision: 12287 $
 * $Id: ParseManager_base.cpp 12287 2021-01-23 05:46:23Z mortenmacfly $
 * $HeadURL: svn://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/codecompletion/ParseManager_base.cpp $
 */
...
////    // ----------------------------------------------
////    // loop on the input parameter procResult, and only records some container tokens, such as
                                           ...
                                           std::queue<ParserComponent> & components)
{
    ...
    if (s_DebugSmartSense)
    {
        CCLogger::Get()->DebugLog(wxString::Format(_T("BreakUpComponents() Breaking up '%s'"), statement.wx_str()));
    }
    ...
    while (true)
    {
        ...
        if (s_DebugSmartSense)
        {
            ...
            CCLogger::Get()->DebugLog(wxString::Format(_T("BreakUpComponents() Found component: '%s' (%s)"),
        ...
        if (!tok.IsEmpty() || (tokenType == pttSearchText && !components.empty()))
        {
            if (s_DebugSmartSense)
            {
                CCLogger::Get()->DebugLog(wxString::Format(_T("BreakUpComponents() Adding component: '%s'."), tok.wx_str()));
            }
...
////            else
////            {
////                CCLogger::Get()->DebugLog(wxString::Format(_T("ResolveExpression() Error to find initial search scope.")));
...
////        if (s_DebugSmartSense)
////        {
////            CCLogger::Get()->DebugLog(wxString::Format(_T("ResolveExpression() Search scope with %lu result:"),
////                                        static_cast<unsigned long>(initialScope.size())));
////            for (TokenIdxSet::const_iterator tt = initialScope.begin(); tt != initialScope.end(); ++tt)
////                CCLogger::Get()->DebugLog(wxString::Format(_T("- Search scope: %d"), (*tt)));
////        }
////
////        CC_LOCKER_TRACK_TT_MTX_LOCK(s_TokenTreeMutex)
////
...
////
////        if (s_DebugSmartSense)
////            CCLogger::Get()->DebugLog(wxString::Format(_T("ResolveExpression() Looping %lu result."),
...
////                {
////                    if (s_DebugSmartSense)
////                        CCLogger::Get()->DebugLog(wxString::Format(_T("ResolveExpression() token is NULL?!")));
////                    continue;
////                }
////
...
////
////                if (s_DebugSmartSense)
////                    CCLogger::Get()->DebugLog(wxString::Format(_T("ResolvExpression() Match:'%s(ID=%lu) : type='%s'"),
////                                                token->m_Name.wx_str(),
////                                                static_cast<unsigned long>(id),
////                                                token->m_BaseType.wx_str()));
////
////                // record the template map message here. hope it will work.
        ...
        for (TokenIdxSet::const_iterator it = parent->m_Children.begin(); it != parent->m_Children.end(); ++it)
        {
            ...
            if (token && MatchType(token->m_TokenKind, kindMask))
            {
                if (MatchText(token->m_Name, target, caseSens, isPrefix))
                {
                    ...
                    if (token && token->m_TokenKind == tkNamespace && token->m_Aliases.size()) // handle namespace aliases
                    {
                        ...
                        if (token && token->m_TokenKind == tkEnum) // check enumerators for match too
                            // All functions that call the recursive GenerateResultSet should already entered a critical section.
                        {
