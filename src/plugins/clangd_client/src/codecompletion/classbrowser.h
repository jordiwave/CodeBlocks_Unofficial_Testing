...
public:
    ...
    enum ETreeOperator
    {
        ...
        OpExpandAll,     /// Expand all items (needed if root is hidden)
     ...
     * @param np the ParseManager instance, which contains all the images for the wxTreeCtrl
     */
     ...
     *  @param parsedFilename A filename that may not be the active editor/filename but whos symbols should be updated.
    ...
    {
        return m_ParseManager;   //(ph 2022/02/14)
    }
...
private:
    ...
    bool GetTokenTreeLock(void (T::*method)(T1 x1), P1 x1);
