 ...
 *
 * $Revision: 63 $
 * $Id: searchtree.cpp 63 2022-05-20 17:21:14Z pecanh $
 * $HeadURL: http://svn.code.sf.net/p/cb-clangd-client/code/trunk/clangd_client/src/codecompletion/parser/searchtree.cpp $
 */
...
size_t SearchTreeNode::AddItemNo(size_t depth, size_t itemno)
{
    ...
    if (found == m_Items.end()) // the specified depth not found, so we add one
    {
        ...
        if (found->second == 0) // find an item of specified depth, but its value is not correct, so fix it.
        {
            m_Items[depth] = itemno;
        }
        else
...
size_t BasicSearchTree::insert(const wxString & s)
{
    ...
    if (m_Points.size() < result)   // FIXME (ollydbg#1#): How does this happen?
    {
        ...
        if (m_Points.size() == result) //record the new added item point in this case
        {
                                ...
                                if (entity == _T("apos"))
                                {
                                    ...
                                    if (entity == _T("lt"))
                                    {
                                        ...
                                        if (entity == _T("gt"))
                                        {
                                            ...
                                            if (entity[0] == _T('#') && S2U(entity.substr(1), u))
                                            {
                                                ch = u;
                                            }
                                            else
