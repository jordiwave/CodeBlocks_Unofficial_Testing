...
// ----------------------------------------------------------------------------
{
    ...
    for (int ii = 0; ii < edCount; ++ii)
    {
        ...
        if (pPrjFile->GetParentProject() == pProject)
            ...
            {
                pClient->LSP_DidOpen(pcbEd);    //send editors file to clangd
            }
    }
...
// ----------------------------------------------------------------------------
{
    ...
    {
        assert((uri.Length() > 0) && "Missing JSON URI filename");
        return;
    }
    ...
    if (evtString.EndsWith(wxString(STX) + "result"))
        try
        {
            ...
            for (size_t itemNdx = 0; itemNdx < valueItemsCount && itemNdx < CCMaxMatches; ++itemNdx)
            {
                ...
                if (filterText.size())
                {
                    ccctoken.name = filterText;    //(ph 2022/06/20)
                }
