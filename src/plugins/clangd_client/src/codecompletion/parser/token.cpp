 ...
 *
 * $Revision: 63 $
 * $Id: token.cpp 63 2022-05-20 17:21:14Z pecanh $
 * $HeadURL: http://svn.code.sf.net/p/cb-clangd-client/code/trunk/clangd_client/src/codecompletion/parser/token.cpp $
 */
...
wxString Token::DisplayName() const
{
    ...
    if (m_TokenKind == tkClass)
    {
        ...
        if (m_TokenKind == tkNamespace)
        {
            return result << _T("namespace ") << m_Name << _T(" {...}");
        }
        else
            if (m_TokenKind == tkEnum)
            {
                return result << _T("enum ")      << m_Name << _T(" {...}");
            }
            else
                if (m_TokenKind == tkTypedef)
                {
                    result << _T("typedef");
