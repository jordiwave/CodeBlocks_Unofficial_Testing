 ...
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/projectsimporter/msvcloader.cpp $
 */
    ...
    while (!file.Eof())
    {
        ...
        if (line.StartsWith(_T("# TARGTYPE")))
        {
            ...
            if (idx != -1)
            {
                ...
                if (projcode.Matches(_T("0101")))
                {
                    type = ttExecutable;
                }
                else
                    if (projcode.Matches(_T("0102")))
                    {
                        type = ttDynamicLib;
                    }
                    else
                        if (projcode.Matches(_T("0103")))
                        {
                            type = ttConsoleOnly;
                        }
                        else
                            if (projcode.Matches(_T("0104")))
                            {
                                type = ttStaticLib;
                            }
                            else
                                if (projcode.Matches(_T("010a")))
                                {
                                    type = ttCommandsOnly;
                                }
                                else
            ...
            if (line.StartsWith(_T("!MESSAGE \"")))
            {
                ...
                if (line.StartsWith(_T("!IF  \"$(CFG)\" ==")))
                {
                    ...
                    if (line.StartsWith(_T("!ELSEIF  \"$(CFG)\" ==")))
                    {
                        size = 20;
                    }
                    else
                        if (line == _T("# Begin Target"))
                        {
                            // done
...
bool MSVCLoader::ParseConfiguration(int index)
{
    ...
    if (it != m_TargetBasedOn.end())
    {
        m_Type = it->second;
    }
    else
    {
        Manager::Get()->GetLogManager()->DebugLog(_T("ERROR: could not find the target type of ") + m_Configurations[index]);
    }
    ...
    while (!file.Eof())
    {
        ...
        //        else if (line.StartsWith("# PROP BASE Intermediate_Dir "))
            ...
            if (line.StartsWith(_T("# PROP Intermediate_Dir ")))
            {
                line = line.Mid(24);
                line.Trim(true).Trim(false);
                wxString tmp = RemoveQuotes(line);
                ...
                if (line.StartsWith(_T("# ADD BASE CPP ")))
                {
                    line = line.Mid(15);
                    line.Trim(true).Trim(false);
                    ...
                    if (line.StartsWith(_T("# ADD CPP ")))
                    {
                        ...
                        if (line.StartsWith(_T("# ADD BASE LINK32 ")))
                        {
                            line = line.Mid(18);
                            line.Trim(true).Trim(false);
                            ...
                            if (line.StartsWith(_T("# ADD LINK32 ")))
                            {
                                line = line.Mid(13);
                                line.Trim(true).Trim(false);
                                ProcessLinkerOptions(bt, line);
                                ...
                                if (line.StartsWith(_T("# ADD BASE RSC "))) // To import resource compiler options
                                {
                                    line = line.Mid(16);
                                    line.Trim(true).Trim(false);
        ...
        if (m_ConvertSwitches)
        {
            if (opt.Matches(_T("/D")))
            {
                ...
                if (opt.Matches(_T("/U")))
                {
                    ...
                    if (opt.Matches(_T("/Zi")) || opt.Matches(_T("/ZI")))
                    {
                        ...
                        if (opt.Matches(_T("/I")))
                        {
                            ...
                            if (opt.Matches(_T("/W0")))
                            {
                                target->AddCompilerOption(_T("-w"));
                            }
                            else
                                if (opt.Matches(_T("/O1")) ||
                                        opt.Matches(_T("/O2")) ||
                                        opt.Matches(_T("/O3")))
                                {
    ...
    for (unsigned int i = 0; i < array.GetCount(); ++i)
    {
                                ...
                                if (opt.Matches(_T("/dll")))
                                {
                                    ...
                                    if (opt.StartsWith(_T("/out:")))
                                    {
                                        ...
                                        if (opt.StartsWith(_T("@")))
                                        {
                                            ...
                                            if (opt.Find(_T(".lib")) == -1) // don't add linking lib (added below, in common options)
                                            {
        ...
        else // !m_ConvertSwitches
        {
            if (opt.StartsWith(_T("/libpath:")))
            {
                ...
                if (opt.Matches(_T("/nologo"))) {} // ignore silently
                else
                    if (opt.StartsWith(_T("@")))
                    {
