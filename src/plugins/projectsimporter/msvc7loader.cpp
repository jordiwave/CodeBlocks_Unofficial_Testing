 ...
 *
 * $Revision: 10874 $
 * $Id: msvc7loader.cpp 10874 2016-07-16 20:00:28Z jenslody $
...
bool MSVC7Loader::Open(const wxString & filename)
{
    ...
    if (ver.IsSameAs(_T("7.0")) || ver.IsSameAs(_T("7.00")))
    {
        m_Version = 70;
    }

    if (ver.IsSameAs(_T("7.1")) || ver.IsSameAs(_T("7.10")))
    {
        m_Version = 71;
    }

    if (ver.IsSameAs(_T("8.0")) || ver.IsSameAs(_T("8.00")))
    {
        m_Version = 80;
    }
...
bool MSVC7Loader::DoImport(TiXmlElement * conf)
{
    ...
    if (conftype.IsSameAs(_T("1"))) // typeApplication 1, no difference between console or gui here, we must check the subsystem property of the linker
    {
        ...
        if (conftype.IsSameAs(_T("2"))) // typeDynamicLibrary 2
        {
            bt->SetTargetType(ttDynamicLib);
        }
        else
            if (conftype.IsSameAs(_T("4"))) // typeStaticLibrary 4
            {
                bt->SetTargetType(ttStaticLib);
            }
            else
                if (conftype.IsSameAs(_T("-1"))) // typeNative -1, check subsystem property of the linker to make sure
                {
                    bt->SetTargetType(ttNative);
                }
                else
                    if (conftype.IsSameAs(_T("10"))) // typeGeneric 10
                    {
            ...
            if (strcmp(tool->Attribute("Name"), "VCCLCompilerTool") == 0)
            {
                ...
                if (m_ConvertSwitches)
                {
                    if (tmp.IsSameAs(_T("0")))
                    {
                        ...
                        if (tmp.IsSameAs(_T("1")) || tmp.IsSameAs(_T("2")) || tmp.IsSameAs(_T("3")))
                        {
                            bt->AddCompilerOption(_T("-W"));
                        }
                        else
                            if (tmp.IsSameAs(_T("4")))
                            {
                ...
                if (!m_ConvertSwitches && tmp.IsSameAs(_T("1")))
                {
                    bt->AddCompilerOption(_T("/Ob1"));
                }
                ...
                */
                ...
                if (!m_ConvertSwitches)
                {
                        ...
                        if (tmp.IsSameAs(_T("1")))
                        {
                            ...
                            if (tmp.IsSameAs(_T("2")))
                            {
                                ...
                                if (tmp.IsSameAs(_T("3")))
                                {
                                    ...
                                    if (tmp.IsSameAs(_T("4")))
                                    {
                                        bt->AddCompilerOption(_T("/ML"));
                                    }
                                    else
                                        if (tmp.IsSameAs(_T("5")))
                                        {
                                            bt->AddCompilerOption(_T("/MLd"));
                                        }
                    ...
                    */
                    ...
                    if (tmp.IsSameAs(_T("TRUE")))
                    {
                        bt->AddCompilerOption(_T("EHsc"));    // add C++ exception handling
                    }
                }
                ...
                */
                ...
                for (i = 0; i < arr.GetCount(); ++i)
                {
                    if (arr[i].IsSameAs(_T("/D")) || arr[i].IsSameAs(_T("-D")))
                    {
                        ...
                        if (arr[i].StartsWith(_T("/D")) || arr[i].StartsWith(_T("-D")))
                        {
                            bt->AddCompilerOption((m_ConvertSwitches ? _T("-D") : _T("/D")) + arr[i].Mid(2));
                        }
                        else
                            if (arr[i].IsSameAs(_T("/Zi")))
                            {
                                bt->AddCompilerOption(m_ConvertSwitches ? _T("-g") : _T("/Zi"));
                            }
                            else
                ...
                if (strcmp(tool->Attribute("Name"), "VCPreBuildEventTool") == 0)
                {
                    // pre-build step
                    ...
                    if (strcmp(tool->Attribute("Name"), "VCPostBuildEventTool") == 0)
                    {
                        // post-build step
                        wxString cmd = ReplaceMSVCMacros(cbC2U(tool->Attribute("CommandLine")));
