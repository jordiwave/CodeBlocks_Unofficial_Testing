...
bool MSVC10Loader::GetProjectGlobals(const TiXmlElement * root)
{
    ...
    if (title)
    {
        m_pProject->SetTitle(cbC2U(title));
    }
    ...
    for (; prop; prop = prop->NextSiblingElement("PropertyGroup"))
    {
        ...
        if (label.IsSameAs(_T("Globals"), false))
        {
            ...
            if (!pProjectName)
            {
                pProjectName = prop->FirstChildElement("RootNamespace");    // old
            }
...
bool MSVC10Loader::GetProjectConfigurations(const TiXmlElement * root)
{
    ...
    for (HashProjectsConfs::iterator it = m_pc.begin(); it != m_pc.end(); ++it)
    {
        ...
        if (pc.sOutDir.IsEmpty())
        {
            pc.sOutDir = m_OutDir.IsEmpty() ? wxString(_T("$(SolutionDir)$(Configuration)")) + wxFILE_SEP_PATH : m_OutDir;
        }

        if (pc.sIntDir.IsEmpty())
        {
            pc.sIntDir = m_IntDir.IsEmpty() ? wxString(_T("$(Configuration)")) + wxFILE_SEP_PATH : m_IntDir;
        }

        if (pc.sTargetName.IsEmpty())
        {
            pc.sTargetName = _T("$(ProjectName)");
        }
        ...
        if (pc.sTargetExt.IsEmpty())
        {
            if (pc.TargetType.IsSameAs(_T("DynamicLibrary"), false))
            {
                ...
                if (pc.TargetType.IsSameAs(_T("StaticLibrary"),  false))
                {
...
bool MSVC10Loader::GetConfiguration(const TiXmlElement * root)
{
    ...
    for (; prop; prop = prop->NextSiblingElement("PropertyGroup"))
    {
        ...
        if (!config.IsEmpty())
        {
            ...
            if (e)
            {
                m_pc[config].TargetType = GetText(e);
            }
            ...
            if (sConf.IsSameAs(sName))
            {
                ...
                if (!m_ConvertSwitches && !m_pc[sName].Charset.IsEmpty())
                {
                    if (m_pc[sName].Charset.IsSameAs(_T("NotSet"), false))
                        ...
                        if (m_pc[sName].Charset.IsSameAs(_T("Unicode"), false))
                        {
                            bt->AddCompilerOption(_T("/D_UNICODE /DUNICODE"));
                        }
                        else
                            if (m_pc[sName].Charset.IsSameAs(_T("MultiByte"), false))
                            {
                ...
                if (!m_pc[sName].sConf.IsEmpty())
                {
                    if (m_pc[sName].sConf.IsSameAs(_T("Release"), false))
                    {
                        ...
                        if (m_pc[sName].sConf.IsSameAs(_T("Debug"), false))
                        {
                ...
                if (comp)
                {
                    ...
                    if ((copt = comp->FirstChildElement("Optimization")))
                    {
                        ...
                        if (val.IsSameAs(_T("Disabled"), false))
                        {
                            ...
                            if (val.IsSameAs(_T("MinSpace"), false))
                            {
                                if (!m_ConvertSwitches)
                                {
                                ...
                                if (val.IsSameAs(_T("MaxSpeed"), false))
                                {
                                    if (!m_ConvertSwitches)
                                    {
                                    ...
                                    if (val.IsSameAs(_T("Full"), false))
                                    {
                                        if (!m_ConvertSwitches)
                                        {
                                            bt->AddCompilerOption(_T("/Ox"));
                                        }
                                        else
                                        {
                                            bt->AddLinkerOption(_T("-s"));
                    ...
                    if (!m_ConvertSwitches && (copt = comp->FirstChildElement("RuntimeLibrary")))
                    {
                        ...
                        if (val.IsSameAs(_T("MultiThreaded"), false))
                        {
                            ...
                            if (val.IsSameAs(_T("MultiThreadedDebug"), false))
                            {
                                bt->AddCompilerOption(_T("/MTd"));
                            }
                            else
                                if (val.IsSameAs(_T("MultiThreadedDll"), false))
                                {
                                    bt->AddCompilerOption(_T("/MD"));
                                }
                                else
                                    if (val.IsSameAs(_T("MultiThreadedDebugDll"), false))
                                    {
                    ...
                    if ((copt = comp->FirstChildElement("WarningLevel")))
                    {
                        ...
                        if (val.IsSameAs(_T("Level1"), false))
                        {
                            if (!m_ConvertSwitches)
                            {
                                bt->AddCompilerOption(_T("/W1"));
                            }
                        }
                        else
                            if (val.IsSameAs(_T("Level2"), false))
                            {
                                bt->AddCompilerOption(!m_ConvertSwitches ? _T("/W2") : _T("-Wall"));
                            }
                            else
                                if (val.IsSameAs(_T("Level3"), false))
                                {
                                    bt->AddCompilerOption(!m_ConvertSwitches ? _T("/W3") : _T("-Wall"));
                                }
                                else
                                    if (val.IsSameAs(_T("Level4"), false))
                                    {
                                        if (!m_ConvertSwitches)
                                        {
                                            bt->AddCompilerOption(_T("/W4"));
                                        }
                                        else
                                        {
...
bool MSVC10Loader::DoCreateConfigurations()
{
    ...
    for (HashProjectsConfs::iterator it = m_pc.begin(); it != m_pc.end(); ++it)
    {
        ...
        if (bt)
        {
            ...
            if (it->second.TargetType == _T("Application"))
            {
                tt = ttExecutable;
            }
            else
                if (it->second.TargetType == _T("Console"))
                {
                    tt = ttConsoleOnly;
                }
                else
                    if (it->second.TargetType == _T("StaticLibrary"))
                    {
                        tt = ttStaticLib;
                    }
                    else
                        if (it->second.TargetType == _T("DynamicLibrary"))
                        {
                            tt = ttDynamicLib;
                        }
                        else
