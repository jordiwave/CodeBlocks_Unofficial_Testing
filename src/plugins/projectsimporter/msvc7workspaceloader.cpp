 ...
 *
 * $Revision: 12607 $
 * $Id: msvc7workspaceloader.cpp 12607 2021-12-23 08:50:04Z wh11204 $
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/projectsimporter/msvc7workspaceloader.cpp $
 */
...
bool MSVC7WorkspaceLoader::Open(const wxString & filename, wxString & Title)
{
    ...
    while (!file.Eof())
    {
        ...
        if (line.StartsWith(_T("Project(")))
        {
            ...
            if (project)
            {
                registerProject(uuid, project);
            }
        }
        else
            if (line.StartsWith(_T("GlobalSection(ProjectDependencies)")))
            {
                depSection = true;
                global = true;
            }
            else
                if (line.StartsWith(_T("ProjectSection(ProjectDependencies)")))
                {
                    depSection = true;
                    global = false;
                }
                else
                    if (line.StartsWith(_T("GlobalSection(ProjectConfiguration)")))
                    {
                        projConfSection = true;
                    }
                    else
                        if (line.StartsWith(_T("GlobalSection(SolutionConfiguration)")))
                        {
                            slnConfSection = true;
                        }
                        else
                            if (line.StartsWith(_T("EndGlobalSection")) || line.StartsWith(_T("EndProjectSection")))
                            {
                                depSection = false;
                                ...
                                if (depSection)
                                {
                                    ...
                                    if (slnConfSection)
                                    {
                                        ...
                                        if (projConfSection && line.StartsWith(_T("{")))
                                        {
                                            ...
                                            if (key[2] == _T("Build"))
                                            {
                                                addConfigurationMatching(key[0], key[1], value[0]);
                                            }
                                        }
    }
