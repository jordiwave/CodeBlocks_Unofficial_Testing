...
*
* $Revision: 10688 $
* $Id: wxscorrector.cpp 10688 2016-01-22 12:24:56Z mortenmacfly $
...
bool wxsCorrector::FillEmpty(wxsItem * Item)
{
    ...
    if (Parent)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            if (FillEmpty(Parent->GetChild(i)))
            {
                Ret = true;
            }
        }
...
void wxsCorrector::SetNewVarName(wxsItem * Item)
{
    ...
    for (int i = 1;; i++)
    {
        ...
        if (m_Vars.find(NewName) == m_Vars.end())
        {
            break;
        }
    }
...
void wxsCorrector::SetNewIdName(wxsItem * Item)
{
    ...
    for (int i = 1;; i++)
    {
        ...
        if (m_Ids.find(NewName) == m_Ids.end())
        {
            break;
        }
    }
