        ...
        case wxsCPP:
        {
            ...
            if (GetPropertiesFlags() & flSource)
            {
                if (m_ColsCount >= 0 && m_RowsCount >= 0 && (m_ColsCount > 0 || m_RowsCount > 0))
                {
                    ...
                    if (m_LabelRowHeight > 0)
                    {
                        Codef(_T("%ASetColLabelSize(%d);\n"), m_LabelRowHeight);
                    }

                    if (m_LabelColWidth  > 0)
                    {
                        Codef(_T("%ASetRowLabelSize(%d);\n"), m_LabelColWidth);
                    }

                    if (m_DefaultRowSize  > 0)
                    {
                        Codef(_T("%ASetDefaultRowSize(%d, %b);\n"), m_DefaultRowSize, true);
                    }

                    if (m_DefaultColSize  > 0)
                    {
                        Codef(_T("%ASetDefaultColSize(%d, %b);\n"), m_DefaultColSize, true);
                    }
                    ...
                    if (ss.Len() > 0)
                    {
                        Codef(_T("%ASetLabelTextColour(%s);\n"), ss.wx_str());
                    }
...
wxObject * wxsGrid::OnBuildPreview(wxWindow * parent, long flags)
{
    ...
    if (GetPropertiesFlags() & flSource)
    {
        if (m_ColsCount >= 0 && m_RowsCount >= 0 && (m_ColsCount > 0 || m_RowsCount > 0))
        {
            ...
            if (m_LabelRowHeight > 0)
            {
                preview->SetColLabelSize(m_LabelRowHeight);
            }

            if (m_LabelColWidth  > 0)
            {
                preview->SetRowLabelSize(m_LabelColWidth);
            }

            if (m_DefaultRowSize > 0)
            {
                preview->SetDefaultRowSize(m_DefaultRowSize, true);
            }

            if (m_DefaultColSize > 0)
            {
                preview->SetDefaultColSize(m_DefaultColSize, true);
            }
