...
void wxsCoderContext::AddHeader(const wxString & Header, const wxString & DeclaredClass, long HeaderFlags)
{
    ...
    if (!(m_Flags & flSource))
    {
        if (IsLocal)
        {
            return;    // Do not need any local includes because they're used only inside manually building code
        }

        if (m_Flags & flLocal)
        {
            return;    // Skip any headers for items which are not members (they're not used at all)
        }
    }
