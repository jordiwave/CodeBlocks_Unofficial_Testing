...
class PLUGIN_EXPORT wxsItemResData
{
    public:
         ...
         */
        inline wxsItem * GetRootItem()
        {
            return m_RootItem;
        }

        /** \brief Getting main item of selection */
        inline wxsItem * GetRootSelection()
        {
            return m_RootSelection;
        }

        /** \brief Getting properties filter based on current edit  mode */
        inline int GetPropertiesFilter()
        {
        ...
        inline bool GetTreeId(wxsResourceItemId & Id, wxsItem * Item)
        {
            return FindId(Id, Item);
        }

        /** \brief Showing popup menu from given item */
        inline wxsItemEditor * GetEditor()
        {
            return m_Editor;
        }
        ...
        inline void Undo()
        {
            SetXmlData(m_Undo.Undo());
        }

        /** \brief Redoing */
        inline void Redo()
        {
            SetXmlData(m_Undo.Redo());
        }
        ...
        inline wxsItem * GetLastSelection()
        {
            return m_RootSelection;
        }
        ...
        inline int GetToolsCount()
        {
            return (int)m_Tools.Count();
        }

        /** \brief Getting tool at given index */
        inline wxsTool * GetTool(int Index)
        {
            return ((Index >= 0) && (Index < GetToolsCount())) ? m_Tools[Index] : 0;
        }
        ...
        inline void NotifyPreviewClosed()
        {
            m_Preview = 0;
        }
    ...
    private:
        ...
        inline wxWindow * BuildExactPreview(wxWindow * Parent)
        {
            return m_Functions->OnBuildExactPreview(Parent, this);
        }
