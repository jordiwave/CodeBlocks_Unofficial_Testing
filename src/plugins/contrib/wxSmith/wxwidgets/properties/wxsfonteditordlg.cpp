...
void wxsFontEditorDlg::StoreData(wxsFontData & _Data)
{
    ...
    {
        if (StyleNorm->GetValue())
        {
            Data.Style = wxFONTSTYLE_NORMAL;
        }

        if (StyleItal->GetValue())
        {
            Data.Style = wxFONTSTYLE_ITALIC;
        }

        if (StyleSlant->GetValue())
        {
            Data.Style = wxFONTSTYLE_SLANT;
        }
    }

    if ((Data.HasWeight = WeightUse->GetValue()))
    {
        if (WeightLight->GetValue())
        {
            Data.Weight = wxFONTWEIGHT_LIGHT;
        }

        if (WeightNorm->GetValue())
        {
            Data.Weight = wxFONTWEIGHT_NORMAL;
        }

        if (WeightBold->GetValue())
        {
            Data.Weight = wxFONTWEIGHT_BOLD;
        }
    }

    if ((Data.HasUnderlined = UnderUse->GetValue()))
    {
        if (UnderYes->GetValue())
        {
