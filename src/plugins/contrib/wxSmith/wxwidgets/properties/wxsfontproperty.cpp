...
wxFont wxsFontData::BuildFont()
{
    ...
    if (!SysFont.empty() && HasSysFont)
    {
        ...
        if (SysFont == _T("wxSYS_OEM_FIXED_FONT"))
        {
            Base = wxSystemSettings::GetFont(wxSYS_OEM_FIXED_FONT);
        }
        else
            if (SysFont == _T("wxSYS_ANSI_FIXED_FONT"))
            {
                Base = wxSystemSettings::GetFont(wxSYS_ANSI_FIXED_FONT);
            }
            else
                if (SysFont == _T("wxSYS_ANSI_VAR_FONT"))
                {
                    Base = wxSystemSettings::GetFont(wxSYS_ANSI_VAR_FONT);
                }
                else
                    if (SysFont == _T("wxSYS_SYSTEM_FONT"))
                    {
                        Base = wxSystemSettings::GetFont(wxSYS_SYSTEM_FONT);
                    }
                    else
                        if (SysFont == _T("wxSYS_DEVICE_DEFAULT_FONT"))
                        {
                            Base = wxSystemSettings::GetFont(wxSYS_DEVICE_DEFAULT_FONT);
                        }
                        else
                            if (SysFont == _T("wxSYS_DEFAULT_GUI_FONT"))
                            {
                                Base = wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
                            }
        ...
        if (HasSize)
        {
            ...
            if (HasRelativeSize)
            {
                Base.SetPointSize((int)(Base.GetPointSize() * RelativeSize));
            }
        ...
        if (!Faces.empty())
        {
            Base.SetFaceName(Face);
        }

        if (HasEncoding)
        {
            Base.SetEncoding(Enc);
        }
...
wxString wxsFontData::BuildFontCode(const wxString & FontName, wxsCoderContext * Context)
{
    ...
    switch (Context->m_Language)
    {
        case wxsCPP:
        {
            ...
            if (HasFamily)
            {
                if (Family == wxFONTFAMILY_DECORATIVE)
                {
                    FamilyStr = _T("wxFONTFAMILY_DECORATIVE");
                }
                else
                    if (Family == wxFONTFAMILY_ROMAN)
                    {
                        FamilyStr = _T("wxFONTFAMILY_ROMAN");
                    }
                    else
                        if (Family == wxFONTFAMILY_SCRIPT)
                        {
                            FamilyStr = _T("wxFONTFAMILY_SCRIPT");
                        }
                        else
                            if (Family == wxFONTFAMILY_SWISS)
                            {
                                FamilyStr = _T("wxFONTFAMILY_SWISS");
                            }
                            else
                                if (Family == wxFONTFAMILY_MODERN)
                                {
                                    FamilyStr = _T("wxFONTFAMILY_MODERN");
                                }
                                else
                                    if (Family == wxFONTFAMILY_TELETYPE)
                                    {
                                        FamilyStr = _T("wxFONTFAMILY_TELETYPE");
                                    }
            }
            ...
            if (HasStyle)
            {
                ...
                {
                    StyleStr = _T("wxFONTSTYLE_ITALIC");
                }
                else
                    if (Style == wxFONTSTYLE_SLANT)
                    {
                        StyleStr = _T("wxFONTSTYLE_SLANT");
                    }
            }
            ...
            if (HasWeight)
            {
                ...
                {
                    WeightStr = _T("wxFONTWEIGHT_BOLD");
                }
                else
                    if (Weight == wxFONTWEIGHT_LIGHT)
                    {
                        WeightStr = _T("wxFONTWEIGHT_LIGHT");
                    }
            }
...
bool wxsFontProperty::XmlRead(wxsPropertyContainer * Object, TiXmlElement * Element)
{
    ...
    if ((VALUE.HasStyle = XmlGetString(Element, Val, _T("style"))))
    {
        if (Val == _T("italic"))
        {
            VALUE.Style = wxFONTSTYLE_ITALIC;
        }
        else
            if (Val == _T("slant"))
            {
                VALUE.Style = wxFONTSTYLE_SLANT;
            }
            else
            {
    ...
    if ((VALUE.HasWeight = XmlGetString(Element, Val, _T("weight"))))
    {
        if (Val == _T("bold"))
        {
            VALUE.Weight = wxFONTWEIGHT_BOLD;
        }
        else
            if (Val == _T("light"))
            {
                VALUE.Weight = wxFONTWEIGHT_LIGHT;
            }
            else
            {
    ...
    {
        if (Val == _T("decorative"))
        {
            VALUE.Family = wxFONTFAMILY_DECORATIVE;
        }
        else
            if (Val == _T("roman"))
            {
                VALUE.Family = wxFONTFAMILY_ROMAN;
            }
            else
                if (Val == _T("script"))
                {
                    VALUE.Family = wxFONTFAMILY_SCRIPT;
                }
                else
                    if (Val == _T("swiss"))
                    {
                        VALUE.Family = wxFONTFAMILY_SWISS;
                    }
                    else
                        if (Val == _T("modern"))
                        {
                            VALUE.Family = wxFONTFAMILY_MODERN;
                        }
                        else
                            if (Val == _T("teletype"))
                            {
                                VALUE.Family = wxFONTFAMILY_TELETYPE;
                            }
                            else
                            {
...
bool wxsFontProperty::XmlWrite(wxsPropertyContainer * Object, TiXmlElement * Element)
{
    ...
    if (VALUE.HasStyle)
    {
        if (VALUE.Style == wxFONTSTYLE_ITALIC)
        {
            XmlSetString(Element, _T("italic"), _T("style"));
        }
        else
            if (VALUE.Style == wxFONTSTYLE_SLANT)
            {
                XmlSetString(Element, _T("slant"), _T("style"));
            }
            else
            {
    ...
    if (VALUE.HasWeight)
    {
        if (VALUE.Weight == wxFONTWEIGHT_BOLD)
        {
            XmlSetString(Element, _T("bold"),  _T("weight"));
        }
        else
            if (VALUE.Weight == wxFONTWEIGHT_LIGHT)
            {
                XmlSetString(Element, _T("light"), _T("weight"));
            }
            else
            {
    ...
    if (VALUE.HasUnderlined)
    {
        if (VALUE.Underlined)
        {
            XmlSetString(Element, _T("1"), _T("underlined"));
        }
        else
        {
            XmlSetString(Element, _T("0"), _T("underlined"));
        }
    }

    if (VALUE.HasFamily)
    {
        if (VALUE.Family == wxFONTFAMILY_DECORATIVE)
        {
            XmlSetString(Element, _T("decorative"), _T("family"));
        }
        else
            if (VALUE.Family == wxFONTFAMILY_ROMAN)
            {
                XmlSetString(Element, _T("roman"),     _T("family"));
            }
            else
                if (VALUE.Family == wxFONTFAMILY_SCRIPT)
                {
                    XmlSetString(Element, _T("script"),    _T("family"));
                }
                else
                    if (VALUE.Family == wxFONTFAMILY_SWISS)
                    {
                        XmlSetString(Element, _T("swiss"),     _T("family"));
                    }
                    else
                        if (VALUE.Family == wxFONTFAMILY_MODERN)
                        {
                            XmlSetString(Element, _T("modern"),    _T("family"));
                        }
                        else
                            if (VALUE.Family == wxFONTFAMILY_TELETYPE)
                            {
                                XmlSetString(Element, _T("teletype"),  _T("family"));
                            }
                            else
                                if (VALUE.Family == wxFONTFAMILY_DEFAULT)
                                {
                                    XmlSetString(Element, _T("default"),   _T("family"));
                                }
    }
...
bool wxsFontProperty::PropStreamRead(wxsPropertyContainer * Object, wxsPropertyStream * Stream)
{
    ...
    Stream->GetBool(_T("has_underlined"), Tmp, false);
    VALUE.HasUnderlined = Tmp;
    Stream->GetBool(_T("underlined"), VALUE.Underlined, false);
    Stream->GetBool(_T("has_family"), Tmp, false);
    VALUE.HasFamily = Tmp;
    ...
    Stream->PopCategory();
    Stream->GetBool(_T("has_encoding"), Tmp, false);
    VALUE.HasEncoding = Tmp;
    Stream->GetString(_T("encoding"), VALUE.Encoding, _T(""));
    Stream->GetBool(_T("has_relativesize"), Tmp, false);
    VALUE.HasRelativeSize = Tmp;
...
bool wxsFontProperty::PropStreamWrite(wxsPropertyContainer * Object, wxsPropertyStream * Stream)
{
    ...
    Stream->PutBool(_T("has_encoding"), Tmp, false);
    VALUE.HasEncoding = Tmp;
    Stream->PutString(_T("encoding"), VALUE.Encoding, _T(""));
    Tmp = VALUE.HasRelativeSize;
    Stream->PutBool(_T("has_relativesize"), Tmp, false);
    VALUE.HasRelativeSize = Tmp;
