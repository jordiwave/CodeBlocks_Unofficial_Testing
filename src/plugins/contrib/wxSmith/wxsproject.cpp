...
*
* $Revision: 12304 $
* $Id: wxsproject.cpp 12304 2021-03-16 23:28:31Z fuscated $
    ...
    if (m_GUI)
    {
        if (!m_GUI->CheckIfApplicationManaged())
        {
            ...
            if (wxMessageBox(_("wxSmith does not manage this application's source.\n"
                               "Should I create proper bindings?"), _("wxSmith"), wxYES_NO) == wxNO)
            {
                return;
            }

            if (!m_GUI->CreateApplicationBinding())
            {
                return;
            }
        }
