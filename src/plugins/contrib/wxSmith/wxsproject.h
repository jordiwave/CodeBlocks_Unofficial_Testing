...
class wxsProject
{
    public:
        ...
        inline int GetResourcesCount()
        {
            return (int)m_Resources.Count();
        }

        /** \brief Getting resource from index */
        inline wxsResource * GetResource(int Index)
        {
            return ((Index >= 0) & (Index < GetResourcesCount())) ? m_Resources[Index] : 0;
        }
        ...
        inline bool GetWasModifiedDuringLoad()
        {
            return m_WasModifiedDuringLoad;
        }
