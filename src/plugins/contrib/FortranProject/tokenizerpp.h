...
class TokenizerPP
{
    ...
    public:
        ...
        {
            return m_TokensFiles[m_ActiveFileIdx]->GetLineStartIndex(indexInLine);
        }
        unsigned int GetLineEndIndex(unsigned int indexInLine)
        {
            return m_TokensFiles[m_ActiveFileIdx]->GetLineEndIndex(indexInLine);
        }
        void SetDetailedParsing(bool detPars)
        {
            return m_TokensFiles[m_ActiveFileIdx]->SetDetailedParsing(detPars);
        }
        void SetFilename(const wxString & filename)
        {
            return m_TokensFiles[m_ActiveFileIdx]->SetFilename(filename);
        }
        void SetParent(ParserThreadF * parent);
        void UngetToken()
        {
            return m_TokensFiles[m_ActiveFileIdx]->UngetToken();
        }
        bool SkipToEOL()
        {
            return m_TokensFiles[m_ActiveFileIdx]->SkipToEOL();
        }
        std::vector<wxString> GetParsedFileNames();
        std::set<wxString> * GetDefinedTokens()
        {
            return &m_AllDefineTokens;
        }
