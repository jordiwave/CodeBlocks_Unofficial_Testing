...
void AutoInsert::MakeAutoInsert(cbEditor * ed)
{
    ...
    if (m_AITMap[key] == aitSeparate)
    {
        ...
        if (m_AITMap[key] == aitTogether)
        {
            ...
            if (m_AITMap[key] == aitTogetherCap)
            {
                if (islower(word.GetChar(0)))
                {
                    addStr << "end" << key;
...
bool AutoInsert::DoEndStatementIsRequired(cbStyledTextCtrl * stc, const wxString & key)
{
        ...
        while (line < lcount)
        {
                    ...
                    || (reFinish2 && reFinish2->Matches(str)))
            {
                        ...
                        || m_RegMap["program"]->Matches(str) || m_RegMap["endinterface"]->Matches(str))
                {
                    ...
                    if (reEndCur1->Matches(str) || (reEndCur2 && reEndCur2->Matches(str)))
                    {
                        return false;
                    }
                    else
    ...
    while (line >= lfin)
    {
                ...
                || (reFinish2 && reFinish2->Matches(str)))
        {
            ...
            if ((reFinBack1 && reFinBack1->Matches(str))
            ...
            if (inLabel)
            {
                ...
                if ((inName || wasInName) && (ch == ':'))
                {
                    ...
                    if (inName && !isalnum(ch) && (ch != '_'))
                    {
                        ...
                        if (!inLabel && !inName && !wasInName && (isalpha(ch) || (ch == '_')))
                        {
                            wasCh = true;
                            inName = true;
                            nsPos = i;
                            ...
                            if (wasInName && !isblank(ch))
                            {
                                break;
                            }
                            else
                                if (!isalnum(ch) && (ch != '_') && (ch != '&') && !isblank(ch))
                                {
                                    break;
                                }
    }
