    ...
    switch (keyCode)
    {
        ...
        default:
            ...
            if (wxIsalnum(keyCode))
            {
                ...
                if ((res = NumpadKeyCodeToString(keyCode)) != wxEmptyString)
                {
                    res << wxT(" (numpad)");        // so it is clear it's different from other keys
                    break;
                }
                else
                    if (wxIsprint(keyCode))   //v+0.5
                    {
                        res << (wxChar)keyCode;
                        break;
                    }
...
// ----------------------------------------------------------------------------
{
    ...
    {
        return WXK_RIGHT;    //+v0.5
    }

    if (keyName == wxT("DOWN"))
    {
        return WXK_DOWN;    //+v0.5
    }

    if (keyName == wxT("HOME"))
    {
        return WXK_HOME;    //+v0.5
    }

    if (keyName == wxT("PAGEUP"))
    {
        return WXK_PAGEUP;    //+v0.5
    }

    if (keyName == wxT("PAGEDOWN"))
    {
        return WXK_PAGEDOWN;    //+v0.5
    }
    ...
    {
        return WXK_INSERT;    //+v0.5
    }

    if (keyName == wxT("DELETE"))
    {
        return WXK_DELETE;    //+v0.5
    }

    if (keyName == wxT("LEFT (numpad)"))
    {
        return WXK_NUMPAD_LEFT ;    //+v0.1-
    }

    if (keyName == wxT("UP (numpad)"))
    {
        return WXK_NUMPAD_UP;    //+v0.1-
    }

    if (keyName == wxT("RIGHT (numpad)"))
    {
        return WXK_NUMPAD_RIGHT;    //+v0.1-
    }

    if (keyName == wxT("DOWN (numpad)"))
    {
        return WXK_NUMPAD_DOWN;    //+v0.1-
    }

    if (keyName == wxT("HOME (numpad)"))
    {
        return WXK_NUMPAD_HOME;    //+v0.1-
    }

    if (keyName == wxT("PAGEUP (numpad)"))
    {
        return WXK_NUMPAD_PAGEUP;    //+v0.1-
    }

    if (keyName == wxT("PAGEDOWN (numpad)"))
    {
        return WXK_NUMPAD_PAGEDOWN;    //+v0.1-
    }

    if (keyName == wxT("END (numpad)"))
    {
        return WXK_NUMPAD_END;    //+v0.1-
    }

    if (keyName == wxT("BEGIN (numpad)"))
    {
        return WXK_NUMPAD_BEGIN;    //+v0.1-
    }

    if (keyName == wxT("INSERT (numpad)"))
    {
        return WXK_NUMPAD_INSERT;    //+v0.1-
    }

    if (keyName == wxT("DELETE (numpad)"))
    {
        return WXK_NUMPAD_DELETE;    //+v0.1-
    }

    if (keyName == wxT("= (numpad)"))
    {
        return WXK_NUMPAD_EQUAL;    //+v0.1-
    }

    if (keyName == wxT("* (numpad)"))
    {
        return WXK_NUMPAD_MULTIPLY;    //+v0.1-
    }

    if (keyName == wxT("+ (numpad)"))
    {
        return WXK_NUMPAD_ADD;    //+v0.1-
    }

    if (keyName == wxT(". (numpad)"))
    {
        return WXK_NUMPAD_DECIMAL;    //+v0.1-
    }

    if (keyName == wxT("/ (numpad)"))
    {
        return WXK_NUMPAD_DIVIDE;    //+v0.1-
    }
...
// ----------------------------------------------------------------------------
{
    ...
    if (!fnc)
    {
        return NULL;    // for release builds
    }
