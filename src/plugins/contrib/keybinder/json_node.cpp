...
wxPoint JSONElement::toPoint() const
{
    ...
    wxString str = wxString::Format(_T("%s"), _json->valuestring);
...
wxColour JSONElement::toColour(const wxColour& defaultColour) const
{
    ...
    //-return wxColour(_json->valuestring);                 //(wx28 2019/04/18)-
    wxString jsonValueStr(_json->valuestring, wxConvUTF8);  //(wx28 2019/04/20)+
    return wxColour(jsonValueStr);                          //(wx28 2019/04/20)+
}
...
JSONElement& JSONElement::addProperty(const wxString& name, const wxStringMap_t& stringMap)
{
    ...
    for(; iter != stringMap.end(); ++iter)
    {
        ...
        obj.addProperty(_T("key"), iter->first);
        obj.addProperty(_T("value"), iter->second);
        arr.arrayAppend(obj);
    }
    append(arr);
    return *this;
...
wxStringMap_t JSONElement::toStringMap() const
{
    ...
    for(int i = 0; i < arraySize(); ++i)
    {
        wxString key = arrayItem(i).namedObject(_T("key")).toString();
        wxString val = arrayItem(i).namedObject(_T("value")).toString();
...
{
    return addProperty(name, ToString(font));
}
...
// ----------------------------------------------------------------------------
wxString JSONElement::ToString(const wxFont& font)              //(2019/04/3)
// ----------------------------------------------------------------------------

{
    ...
    wxString str;
    str << font.GetFaceName() << _T(";") << font.GetPointSize() << _T(";") << (int)font.GetFamily() << _T(";")
        << (int)font.GetWeight() << _T(";") << (int)font.GetStyle();
    return str;
}
// ----------------------------------------------------------------------------
wxFont JSONElement::FromString(const wxString& str) const            //(2019/04/3)
// ----------------------------------------------------------------------------
{
    wxArrayString parts = ::wxStringTokenize(str, _T(";"), wxTOKEN_STRTOK);
    if(parts.size() != 5) return wxNullFont;

    long iPointSize, iFamily, iWeight, iStyle;
    wxString facename = parts.Item(0);
#if wxVERSION_NUMBER >= 3000                //(wx28 2019/04/18)
    parts.Item(1).ToCLong(&iPointSize);
    parts.Item(2).ToCLong(&iFamily);
    parts.Item(3).ToCLong(&iWeight);
    parts.Item(4).ToCLong(&iStyle);
#else
    parts.Item(1).ToLong(&iPointSize);
    parts.Item(2).ToLong(&iFamily);
    parts.Item(3).ToLong(&iWeight);
    parts.Item(4).ToLong(&iStyle);
#endif
    ...
    wxFont font(iPointSize, (wxFontFamily)iFamily, iWeight, false, _T("") ); //(wx28 2019/04/19)
    wxUnusedVar(bold);
    wxUnusedVar(italic);
#endif
    return font;
