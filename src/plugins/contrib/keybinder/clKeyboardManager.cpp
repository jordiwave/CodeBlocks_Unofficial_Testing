...
// ----------------------------------------------------------------------------
{
    ...
    if (not config.Exists()) //does cbKeyBinder__.conf exist? eg. %appdata%\<personality>.cbKeyBinder<version>.conf
    {
        ...
        if (fnFileToLoad.FileExists())
        {
            ...
            if (not ReadFileContent(fnFileToLoad, content))
            {
                return;
            }
...
mnuContinue:
        ...
        else // Have amatching map resoureID and menu structure resourceID (ie., menuItemID)
        {
            ...
            if (mapParent.empty())
            {
                continue;    //skip .conf global accelerators
            }
...
// -----------------------------------------------------------------------------------------------------------------
{
    ...
    {
        if (accelIter->second.accel.empty())
        {
            continue;
        }

        if (accelIter->second.parentMenu.empty())
        {
            continue;    //skip global accelerators
        }
    ...
    if (dupMsgs.GetCount())
    {
        ...
        {
            pMainWin = Manager::Get()->GetAppWindow();
        }
