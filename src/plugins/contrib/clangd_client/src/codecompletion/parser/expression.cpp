 ...
 *
 * $Revision: 63 $
 * $Id: expression.cpp 63 2022-05-20 17:21:14Z pecanh $
 * $HeadURL: http://svn.code.sf.net/p/cb-clangd-client/code/trunk/clangd_client/src/codecompletion/parser/expression.cpp $
 */
...
ExpressionNode::ExpressionNodeType ExpressionNode::ParseNodeType(wxString token)
{
    if (token.IsEmpty())
    {
        ...
        if (token == ExpressionConsts::Plus)
        {
            return ExpressionNode::Plus;
        }
        else
            if (token == ExpressionConsts::Subtract)
            {
                return ExpressionNode::Subtract;
            }
            else
                if (token == ExpressionConsts::Multiply)
                {
                    return ExpressionNode::Multiply;
                }
                else
                    if (token == ExpressionConsts::Divide)
                    {
                        return ExpressionNode::Divide;
                    }
                    else
                        if (token == ExpressionConsts::Mod)
                        {
                            return ExpressionNode::Mod;
                        }
                        else
                            if (token == ExpressionConsts::Power)
                            {
                                return ExpressionNode::Power;
                            }
                            else
                                if (token == ExpressionConsts::LParenthesis)
                                {
                                    return ExpressionNode::LParenthesis;
                                }
                                else
                                    if (token == ExpressionConsts::RParenthesis)
                                    {
                                        return ExpressionNode::RParenthesis;
                                    }
                                    else
                                        if (token == ExpressionConsts::BitwiseAnd)
                                        {
                                            return ExpressionNode::BitwiseAnd;
                                        }
                                        else
                                            if (token == ExpressionConsts::BitwiseOr)
                                            {
                                                return ExpressionNode::BitwiseOr;
                                            }
                                            else
                                                if (token == ExpressionConsts::And)
                                                {
                                                    return ExpressionNode::And;
                                                }
                                                else
                                                    if (token == ExpressionConsts::Or)
                                                    {
                                                        return ExpressionNode::Or;
                                                    }
                                                    else
                                                        if (token == ExpressionConsts::Not)
                                                        {
                                                            return ExpressionNode::Not;
                                                        }
                                                        else
                                                            if (token == ExpressionConsts::Equal)
                                                            {
                                                                return ExpressionNode::Equal;
                                                            }
                                                            else
                                                                if (token == ExpressionConsts::Unequal)
                                                                {
                                                                    return ExpressionNode::Unequal;
                                                                }
                                                                else
                                                                    if (token == ExpressionConsts::GT)
                                                                    {
                                                                        return ExpressionNode::GT;
                                                                    }
                                                                    else
                                                                        if (token == ExpressionConsts::LT)
                                                                        {
                                                                            return ExpressionNode::LT;
                                                                        }
                                                                        else
                                                                            if (token == ExpressionConsts::GTOrEqual)
                                                                            {
                                                                                return ExpressionNode::GTOrEqual;
                                                                            }
                                                                            else
                                                                                if (token == ExpressionConsts::LTOrEqual)
                                                                                {
                                                                                    return ExpressionNode::LTOrEqual;
                                                                                }
                                                                                else
                                                                                    if (token == ExpressionConsts::LShift)
                                                                                    {
                                                                                        return ExpressionNode::LShift;
                                                                                    }
                                                                                    else
                                                                                        if (token == ExpressionConsts::RShift)
                                                                                        {
                                                                                            return ExpressionNode::RShift;
                                                                                        }
                                                                                        else
                                                                                        {
                                                                                            if (wxIsdigit(token[0]))
                                                                                            {
                                                                                                return ExpressionNode::Numeric;
                                                                                            }
                                                                                            else
                                                                                            {
                                                                                                return ExpressionNode::Unknown;
...
void Expression::ConvertInfixToPostfix()
{
    ...
    for (PostfixVector::size_type i = 0; i < m_InfixExpression.size(); ++i)
    {
        ...
        if (type == ExpressionNode::Numeric)
        {
            ...
            if (type == ExpressionNode::LParenthesis)
            {
                // Left Parentheses, add to stack
                stackOperator.push(expNode);
            }
            else
                if (type == ExpressionNode::RParenthesis)
                {
