...
#include "wx/xrc/xmlres.h"      //XRCID() //(2021/04/30)
#include "manager.h"
...
// ----------------------------------------------------------------------------
{
    ...
    // check for BrowseTracker menu accelerators //(2021/05/6)
        ...
        if ((event.GetKeyCode() == m_btBackKeyCode) || (event.GetKeyCode() == m_btFrwdKeyCode)
                || (event.GetKeyCode() == GetExtraNavigationKey()))
        {
            ...
            if (event.GetKeyCode() == WXK_TAB || event.GetKeyCode() == GetExtraNavigationKey())
            {
                if (event.ShiftDown())
                {
                    m_items.SetSelection(m_items.GetSelection() - 1);
                ...
                if (event.GetKeyCode() == WXK_DOWN || event.GetKeyCode() == WXK_NUMPAD_DOWN)
                {
                    ...
                    if (event.GetKeyCode() == WXK_UP || event.GetKeyCode() == WXK_NUMPAD_UP)
                    {
                        ...
                        if (event.GetKeyCode() == WXK_HOME || event.GetKeyCode() == WXK_NUMPAD_HOME)
                        {
                            m_items.SetSelection(0);
                            AdvanceToNextSelectableItem(1);
                            GenerateSelectionEvent();
                            ...
                            if (event.GetKeyCode() == WXK_END || event.GetKeyCode() == WXK_NUMPAD_END)
                            {
                                m_items.SetSelection(m_items.GetItemCount() - 1);
                                AdvanceToNextSelectableItem(-1);
                                GenerateSelectionEvent();
...
// ----------------------------------------------------------------------------
{
    ...
    int idMenuTrackerforward        = XRCID("ViewForwardEd");   //(2021/04/30)
    int idMenuTrackerBackward       = XRCID("ViewBackwardEd");
    // Get the "backward and forward editor" wxKey menu accelerator
    //wxMenuBar: wxMenuItem *  FindItem (int id, wxMenu **menu=NULL) const
    wxMenuBar * pMenuBar = Manager::Get()->GetAppFrame()->GetMenuBar();
    wxMenuItem * pMenuItem = pMenuBar->FindItem(idMenuTrackerBackward, nullptr);
    wxAcceleratorEntry * pAccelEntry = pMenuItem->GetAccel();
    //int btBackFlags = pAccelEntry->GetFlags();
    m_btBackKeyCode = pAccelEntry->GetKeyCode();
    pMenuItem = pMenuBar->FindItem(idMenuTrackerforward, nullptr);
    pAccelEntry = pMenuItem->GetAccel();
    m_btFrwdKeyCode = pAccelEntry->GetKeyCode();

    if (wxGetKeyState(WXK_ALT))
    {
        SetModifierKey(WXK_ALT);
    }

    if (wxGetKeyState(WXK_CONTROL))
    {
        SetModifierKey(WXK_CONTROL);
    }

    if (wxGetKeyState(WXK_SHIFT))
    {
        SetModifierKey(WXK_SHIFT);
    }
}
...
void wxSwitcherDialog::OnPaint(wxPaintEvent & WXUNUSED(event))
{
    ...
    if (m_switcherBorderStyle == wxBORDER_SIMPLE)
    {
        ...
        //(2021/05/15) neuter the descrription box
