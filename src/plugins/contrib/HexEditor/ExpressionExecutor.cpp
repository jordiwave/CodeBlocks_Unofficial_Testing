...
{
    inline T operator()(const T & val1, const T & val2)
    {
        return val1 + val2;
    }
};
...
{
    inline long double operator()(const long double & /*val1*/, const long double & /*val2*/)
    {
...
wxString Executor::ErrorDesc()
{
    ...
    switch (m_Status)
    {
        case executedSuccessfully:
            return _("Executed successfully") + pos;

        case errorArgumentIndex:
            return _("Invalid index of code arguments") + pos;

        case errorOperationIndex:
            return _("Invalid index of operation") + pos;

        case errorStackIndex:
            return _("Invalid index of stack") + pos;

        case errorContentIndex:
            return _("Invalid address inside the content") + pos;

        case errorOperation:
            return _("Invalid operation") + pos;

        case errorDivByZero:
            return _("Divide by zero") + pos;

        case errorType:
            return _("Type mismatch") + pos;

        case errorScript:
            return _("Script error") + pos;

        default:
            return _("Unknown error") + pos;
    }
}
...
bool Executor::GetResult(unsigned long long & val)
{
    ...
    if (!m_Stack.front().IsUnsignedInt())
    {
        return false;
    }
...
bool Executor::Run()
{
    ...
    if (m_Status != executedSuccessfully)
    {
        return false;
    }
...
inline void Executor::PushAddress(const Operation & op, long long address)
{
    switch (op.m_Mod1)
    {
        case Operation::modChar:
            PushStack(Content< char               >(address));
            break;

        case Operation::modByte:
            PushStack(Content< unsigned char      >(address));
            break;

        case Operation::modShort:
            PushStack(Content< short              >(address));
            break;

        case Operation::modWord:
            PushStack(Content< unsigned short     >(address));
            break;

        case Operation::modLong:
            PushStack(Content< long               >(address));
            break;

        case Operation::modDword:
            PushStack(Content< unsigned long      >(address));
            break;

        case Operation::modLongLong:
            PushStack(Content< long long          >(address));
            break;

        case Operation::modQword:
            PushStack(Content< unsigned long long >(address));
            break;

        case Operation::modFloat:
            PushStack(Content< float              >(address));
            break;

        case Operation::modDouble:
            PushStack(Content< double             >(address));
            break;

        case Operation::modLongDouble:
            PushStack(Content< long double        >(address));
            break;
