...
bool Value::operator< (const Value & second) const
{
    ...
    {
        case tSignedInt:
            return m_SignedInt   < second.m_SignedInt;

        case tUnsignedInt:
            return m_UnsignedInt < second.m_UnsignedInt;

        case tFloat:
            return m_Float       < second.m_Float;
    ...
    for (int i = 0; i < (int)m_Code.size(); ++i)
    {
        switch (m_Code[i].m_OpCode)
        {
            case Operation::endScript:
                res += wxString::Format(_T("%d: End\n"), (int)i);
                break;

            case Operation::pushCurrent:
                res += wxString::Format(_T("%d: PushCur\n"), (int)i);
                break;

            case Operation::loadMem:
                res += wxString::Format(_T("%d: LoadMem %s, + %d\n"), (int)i, Type(m_Code[i].m_Mod1), (int)m_Code[i].m_ConstArgument);
                break;

            case Operation::loadArg:
                res += wxString::Format(_T("%d: LoadArg %s, %d\n"), (int)i, Type(m_Code[i].m_Mod1), (int)m_Code[i].m_ConstArgument);
                break;

            case Operation::conv:
                res += wxString::Format(_T("%d: Conv    %s -> %s\n"), (int)i, Type(m_Code[i].m_Mod2), Type(m_Code[i].m_Mod1));
                break;

            case Operation::add:
                res += wxString::Format(_T("%d: Add\n"), (int)i);
                break;

            case Operation::mul:
                res += wxString::Format(_T("%d: Mul\n"), (int)i);
                break;

            case Operation::div:
                res += wxString::Format(_T("%d: Div\n"), (int)i);
                break;

            case Operation::mod:
                res += wxString::Format(_T("%d: Mod\n"), (int)i);
                break;

            case Operation::neg:
                res += wxString::Format(_T("%d: Neg\n"), (int)i);
                break;

            case Operation::fnSin:
                res += wxString::Format(_T("%d: Sin\n"), (int)i);
                break;

            case Operation::fnCos:
                res += wxString::Format(_T("%d: Cos\n"), (int)i);
                break;

            case Operation::fnTan:
                res += wxString::Format(_T("%d: Tan\n"), (int)i);
                break;

            case Operation::fnLn:
                res += wxString::Format(_T("%d: Ln\n"), (int)i);
                break;

            case Operation::fnPow:
                res += wxString::Format(_T("%d: Pow\n"), (int)i);
                break;

            default:
                res += wxString::Format(_T("%d: ???\n"), (int)i);
                break;
        }
    }
