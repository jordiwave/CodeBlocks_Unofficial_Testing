...
* $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/HexEditor/SearchDialog.cpp $
*/
...
void SearchDialog::OnOk(wxCommandEvent & /*event*/)
{
    if (m_SearchTypeString->GetValue())
    {
        ...
        if (m_SearchTypeHex->GetValue())
        {
            SearchHex(m_SearchValue->GetValue().c_str());
        }
        else
            if (m_SearchTypeExpression->GetValue())
            {
