...
*
* $Revision: 12524 $
* $Id: HexEditPanel.cpp 12524 2021-09-04 11:32:54Z fuscated $
* $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/HexEditor/HexEditPanel.cpp $
*/
#include <sdk.h>
...
void HexEditPanel::OnContentScroll(wxScrollEvent & /*event*/)
{
    ...
    if (m_ContentScroll->GetThumbPosition() == 0)
    {
        ...
        if (m_ContentScroll->GetThumbPosition() >= m_ContentScroll->GetRange() - m_ContentScroll->GetThumbSize())
        {
            DetectStartOffset();
...
void HexEditPanel::SetModified(bool modified)
{
    if (m_Content)
    {
        m_Content->SetModified(modified);
    }
}
