...
wxString wxsAuiPaneInfoExtra::AllParamsCode(wxsCoderContext * Ctx, wxsItem * ChildPane)
{
    ...
    switch (Ctx->m_Language)
    {
        case wxsCPP:
            ...
            //Caption and caption bar buttons
            if (m_Caption != wxEmptyString)
            {
                str << _T(".Caption(_(\"") << m_Caption << _T("\"))");
            }

            if (m_CaptionVisible && !(m_StandardPane == DefaultPane))
            {
                str << _T(".CaptionVisible()");
            }
            ...
            if (m_MinimizeButton)
            {
                str << _T(".MinimizeButton()");
            }

            if (m_MaximizeButton)
            {
                str << _T(".MaximizeButton()");
            }
            ...
            if (m_CloseButton && (m_StandardPane == CenterPane))
            {
                str << _T(".CloseButton()");
            }
            else
                if (!m_CloseButton && !(m_StandardPane == CenterPane))
                {
                    str << _T(".CloseButton(false)");
                }

            //Layer, row and position
            if (m_Layer > 0)
            {
                str << wxString::Format(_T(".Layer(%ld)"), m_Layer);
            }

            if (m_Row > 0)
            {
                str << wxString::Format(_T(".Row(%ld)"), m_Row);
            }

            if (m_Position != 0)
            {
                str << wxString::Format(_T(".Position(%ld)"), m_Position);
            }
            ...
            if (m_DockDirection == wxAUI_DOCK_TOP)
            {
                ...
                if (m_DockDirection == wxAUI_DOCK_BOTTOM)
                {
                    str << _T(".Bottom()");
                }
                else
                    if (m_DockDirection == wxAUI_DOCK_LEFT)
                    {
                        str << _T(".Left()");
                    }
                    else
                        if (m_DockDirection == wxAUI_DOCK_RIGHT)
                        {
                            str << _T(".Right()");
                        }
                        else
                            if (m_DockDirection == wxAUI_DOCK_CENTER && !(m_StandardPane == CenterPane))
                            {
                                str << _T(".Center()");
                            }
            ...
            //Floatable
            if (m_Floatable && (m_StandardPane == CenterPane))
            {
                str << _T(".Floatable()");
            }
            else
                if (!m_Floatable && !(m_StandardPane == CenterPane))
                {
                    str << _T(".Floatable(false)");
                }

            //Floating properties
            if (!m_FloatingPosition.IsDefault)
            {
                str << _T(".FloatingPosition(") << m_FloatingPosition.GetPositionCode(Ctx) << _T(")");
            }

            if (!m_FloatingSize.IsDefault)
            {
                str << _T(".FloatingSize(") << m_FloatingSize.GetSizeCode(Ctx) << _T(")");
            }

            //Resizable
            if (m_Resizable && (m_StandardPane == ToolbarPane))
            {
                str << _T(".Resizable()");
            }
            else
                if (!m_Resizable && !(m_StandardPane == ToolbarPane))
                {
                    str << _T(".Resizable(false)");
                }

            //Size properties
            if (!BaseProps->m_Size.IsDefault)
            {
                str << _T(".BestSize(") << BaseProps->m_Size.   GetSizeCode(Ctx) << _T(")");
            }

            if (!BaseProps->m_MinSize.IsDefault)
            {
                str << _T(".MinSize(") << BaseProps->m_MinSize.GetSizeCode(Ctx) << _T(")");
            }

            if (!BaseProps->m_MaxSize.IsDefault)
            {
                str << _T(".MaxSize(") << BaseProps->m_MaxSize.GetSizeCode(Ctx) << _T(")");
            }

            //Movable
            if (m_Movable && (m_StandardPane == CenterPane))
            {
                str << _T(".Movable(false)");
            }
            else
                if (!m_Movable && !(m_StandardPane == CenterPane))
                {
            ...
            //Gripper
            if (m_Gripper == 0 && (m_StandardPane == ToolbarPane))
            {
                str << _T(".Gripper(false)");
            }
            else
                if (m_Gripper == wxLEFT)
                {
                    ...
                    if (m_Gripper == wxTOP)
                    {
                        str << _T(".Gripper().GripperTop()");
                    }

            //Pane border
            if (!m_PaneBorder)
            {
                str << _T(".PaneBorder(false)");
            }
...
wxAuiPaneInfo wxsAuiPaneInfoExtra::GetPaneInfoFlags(wxWindow * Parent, wxsItem * ChildPane, bool Exact)
{
        ...
        if (m_DockDirection == wxAUI_DOCK_BOTTOM)
        {
            ...
            if (m_DockDirection == wxAUI_DOCK_LEFT)
            {
                ...
                if (m_DockDirection == wxAUI_DOCK_RIGHT)
                {
                    ...
                    if (m_DockDirection == wxAUI_DOCK_CENTER)
                    {
                        PaneInfo.Center();
                    }
    ...
    if (!m_FloatingPosition.IsDefault)
    {
        PaneInfo.FloatingPosition(m_FloatingPosition.GetPosition(Parent));
    }

    if (!m_FloatingSize.IsDefault)
    {
        PaneInfo.FloatingSize(m_FloatingSize.GetSize(Parent));
    }
    ...
    if (!BaseProps->m_Size.IsDefault)
    {
        PaneInfo.BestSize(BaseProps->m_Size.GetSize(Parent));
    }

    if (!BaseProps->m_MinSize.IsDefault)
    {
        PaneInfo.MinSize(BaseProps->m_MinSize.GetSize(Parent));
    }

    if (!BaseProps->m_MaxSize.IsDefault)
    {
        PaneInfo.MaxSize(BaseProps->m_MaxSize.GetSize(Parent));
    }
...
wxObject * wxsAuiManager::OnBuildPreview(wxWindow * Parent, long Flags)
{
    ...
    if (Count)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            if (ChildAsToolBar)
            {
                ((wxsAuiToolBar *)Child)->m_GripperSize = AuiManager->GetArtProvider()->GetMetric(wxAUI_DOCKART_GRIPPER_SIZE);
            }
...
void wxsAuiManager::OnAddChildQPP(wxsItem * Child, wxsAdvQPP * QPP)
{
    ...
    if (ChildExtra->m_FirstAdd)
    {
        ...
        if (wxDynamicCast(Child->BuildPreview(shortLiveFrame.get(), 0), wxAuiToolBar))
        {
            ...
            if (ChildExtra->m_Layer == 0)
            {
                ChildExtra->m_Layer = 10;
            }
