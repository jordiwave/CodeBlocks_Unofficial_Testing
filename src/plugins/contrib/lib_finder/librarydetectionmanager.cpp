...
* $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/lib_finder/librarydetectionmanager.cpp $
*/
...
int LibraryDetectionManager::LoadXmlFile(const wxString & Name)
{
    ...
    if (!TinyXML::LoadDocument(Name, &Doc) || Doc.Error())
    {
        return 0;
    }
...
int LibraryDetectionManager::LoadXml(TiXmlElement * Elem, LibraryDetectionConfig & Config, LibraryDetectionConfigSet * ConfigSet, bool Filters, bool Settings)
{
            ...
            Data = Data->NextSiblingElement())
    {
        ...
        if (Filters)
        {
            ...
            if (Node == _T("platform"))
            {
                Type = LibraryDetectionFilter::Platform;
            }
            else
                if (Node == _T("file"))
                {
                    Type = LibraryDetectionFilter::File;
                }
                else
                    if (Node == _T("exec"))
                    {
                        Type = LibraryDetectionFilter::Exec;
                    }
                    else
                        if (Node == _T("compiler"))
                        {
                            Type = LibraryDetectionFilter::Compiler;
                        }
        ...
        if (Settings)
        {
            ...
            if (Node == _T("path"))
            {
                ...
                if (!Lib.empty())
                {
                    Config.LibPaths.Add(Lib);
                }

                if (!Obj.empty())
                {
                    Config.ObjPaths.Add(Obj);
                }
            ...
            if (Node == _T("flags"))
            {
                ...
                if (!cFlags.empty())
                {
                    Config.CFlags.Add(cFlags);
                }

                if (!lFlags.empty())
                {
                    Config.LFlags.Add(lFlags);
                }
...
int LibraryDetectionManager::StoreNewSettingsFile(const wxString & shortcut, const std::vector< char > & content)
{
    ...
    if (!doc.RootElement()->Attribute("short_code"))
    {
        return -1;
    }

    if (strcmp(doc.RootElement()->Attribute("short_code"), cbU2C(shortcut)))
    {
        return -1;
    }
