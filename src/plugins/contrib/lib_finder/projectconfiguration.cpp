...
* $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/lib_finder/projectconfiguration.cpp $
*/
...
void ProjectConfiguration::XmlWrite(TiXmlElement * Node, cbProject * Project)
{
    ...
    if (!LibFinder)
    {
        LibFinder = Node->InsertEndChild(TiXmlElement("lib_finder"))->ToElement();
    }
    ...
    for (wxMultiStringMap::iterator iter = m_TargetsUsedLibs.begin();
            ...
            ++iter)
    {
        if (!Project->GetBuildTarget(iter->first))
        {
            continue;
        }
