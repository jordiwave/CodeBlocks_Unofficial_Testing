...
bool WebResourcesManager::LoadDetectionConfigurations(const wxArrayString & baseUrls, ProgressHandler * handler)
{
    ...
    for (size_t i = 0; i < baseUrls.Count(); i++)
    {
        ...
        if (!DoDownload(listUrl, handler, arr))
        {
            continue;
        }
        ...
        if (handler)
        {
            handler->Error(_("Invalid data in libraries list in: ") + listUrl, handler->idDownloadList);
        }
    }

    if (handler)
    {
        handler->JobFinished(handler->idDownloadList);
    }
...
bool WebResourcesManager::DoDownload(const wxString & urlName, ProgressHandler * handler, std::vector< char > & arr)
{
    ...
    if (url->GetError() != wxURL_NOERR)
    {
        if (handler)
        {
            handler->Error(_("Couldn't open url: ") + urlName, id);
        }
    ...
    if (length != wxInvalidOffset)
    {
        ...
        while (left)
        {
            ...
            if (!nowRead)
            {
                if (handler)
                {
                    handler->Error(_("Read error from url: ") + urlName, id);
                }
            ...
            if (handler)
            {
                handler->SetProgress(100.f * read / length, id);
            }
        }
    }
    else
...
bool WebResourcesManager::LoadDetectionConfig(const wxString & shortcut, std::vector< char > & content, ProgressHandler * handler)
{
    ...
    for (; entry ; entry = entry->m_Next)
    {
        if (DoDownload(entry->m_Url, handler, content))
        {
            if (handler)
            {
                handler->JobFinished(handler->idDownloadConfig);
            }
    ...
    if (handler)
    {
        handler->Error(_("Couldn't download any configuration"), handler->idDownloadConfig);
    }
