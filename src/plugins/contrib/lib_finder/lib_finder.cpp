...
*
* $Revision: 12437 $
* $Id: lib_finder.cpp 12437 2021-05-09 12:51:35Z fuscated $
* $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/lib_finder/lib_finder.cpp $
*/
...
void lib_finder::OnProjectHook(cbProject * project, TiXmlElement * elem, bool loading)
{
    ...
    if (loading)
    {
        Proj->XmlLoad(elem, project);
    }
    else
    {
        Proj->XmlWrite(elem, project);
    }
}
...
bool lib_finder::AddLibraryToProject(const wxString & LibName, cbProject * Project, const wxString & TargetName)
{
    ...
    {
        if (!Project->GetBuildTarget(TargetName))
        {
            return false;
        }
...
bool lib_finder::SetupTargetManually(CompileTargetBase * Target)
{
    ...
    if (m_Singleton->m_Targets.find(Target) == m_Singleton->m_Targets.end())
    {
        return false;
    }
