...
int myFindReplaceDlg::GetFlags()
{
    ...
    {
        flags |= myFR_MATCHCASE;
    }

    if (m_wholeword->GetValue())
    {
        flags |= myFR_WHOLEWORD;
    }

    if (m_findregex->GetValue())
    {
        flags |= myFR_FINDREGEX;
    }

    if (m_subfolder->GetValue())
    {
        flags |= myFR_SUBFOLDER;
    }
...
int myGotoDlg::GetPosition()
{
    ...
    if (m_position->GetValue().ToLong(&position))
    {
        return position;
    }
