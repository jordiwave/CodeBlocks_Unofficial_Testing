...
static wxString findColourIDFromControlID(long controlID)
{
    if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker0))
    {
        ...
        if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker1))
        {
            return "thread_search_text_back";
        }
        else
            if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker2))
            {
                return "thread_search_file_fore";
            }
            else
                if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker3))
                {
                    return "thread_search_file_back";
                }
                else
                    if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker4))
                    {
                        return "thread_search_lineno_fore";
                    }
                    else
                        if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker5))
                        {
                            return "thread_search_lineno_back";
                        }
                        else
                            if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker6))
                            {
                                return "thread_search_match_fore";
                            }
                            else
                                if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker7))
                                {
                                    return "thread_search_match_back";
                                }
                                else
                                    if (controlID == controlIDs.Get(ControlIDs::idConfPanelColorPicker8))
                                    {
