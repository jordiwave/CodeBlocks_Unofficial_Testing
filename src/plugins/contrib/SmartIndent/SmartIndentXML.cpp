        ...
        // close string
            ...
            if (complQuote && (ch == wxT('"') || ch == wxT('\'')))
            {
                if (stc->GetCharAt(pos) == ch)
                {
                    ...
                    if (!stc->IsString(stc->GetStyleAt(pos - 2)))
                    {
                        stc->InsertText(pos, ch);
                    }
            }
            else
                if (ch == wxT('?') && stc->GetCharAt(pos - 2) == wxT('<'))
                {
                    wxString tag;
                ...
                // close CDATA
                    ...
                    if (ch == wxT('[') && pos > 8 && stc->GetTextRange(pos - 9, pos - 1) == wxT("<![CDATA"))
                    {
                        wxString tag;
                    ...
                    // embedded languages
                        ...
                        if (complQuote && curSty >= wxSCI_HJ_START && curSty <= wxSCI_HPHP_OPERATOR && !stc->IsString(curSty))
                        {
            ...
            && ((ch == wxT('\n')) || ((stc->GetEOLMode() == wxSCI_EOL_CR) && (ch == wxT('\r')))))
    {
        ...
        if (SmartIndentEnabled()) // smart indent
        {
            ...
            if (idx != wxNOT_FOUND)
            {
                ...
                if (stc->GetStyleAt(pos) >= wxSCI_HJ_START && stc->GetStyleAt(pos) <= wxSCI_HPHP_OPERATOR)
                {
                    ...
                    if (lineSuffix == wxT('}') || lineSuffix == wxT(']'))
                    {
                        ...
                        if (GetLastNonWhitespaceChar(ed) == wxT('{'))
                        {
                            Indent(stc, indent);
                        }
                }
