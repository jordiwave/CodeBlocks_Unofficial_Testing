    ...
    switch (id)
    {
        default:
            if (id == idDragScrollAddWindow)
            {
                ...
                if (id == idDragScrollRemoveWindow)
                {
                    OnDragScrollEventRemoveWindow(event);
                    break;
                }
                else
                    if (id == idDragScrollRescan)
                    {
                        if (not GetMouseDragScrollEnabled())
                        {
                            return;
                        ...
                        if (id == idDragScrollReadConfig)
                        {
                            OnDragScrollEvent_RereadConfig(event);
                            break;
                        }
                        else
                            if (id == idDragScrollInvokeConfig)
                            {
...
// ----------------------------------------------------------------------------
{
    ...
    if (KeyDown(event))
    {
        ...
        if (KeyUp(event) && (m_DragMode != DRAG_NONE))
        {
            // Finish dragging
            ...
            if (event.Dragging() && (m_DragMode != DRAG_NONE))
            {
...
// ----------------------------------------------------------------------------
{
    ...
    //--------- Key UP -------------------------------------------------------
        ...
        //--------- DRAGGING  -----------------------------------------------------
            ...
            if ((m_DragMode != DRAG_NONE) && event.Dragging()) //v0.12
            {
                //-LOGIT( _T("Dragging") ) ;
                //make sure user didnt leave client area and lift mouse key
                if (not KeyIsDown(event))
