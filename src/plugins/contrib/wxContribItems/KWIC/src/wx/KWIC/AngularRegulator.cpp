...
//
//	2010/4/22 Fixed a bug that caused tag colour not to be respected.
//
/////////////////////////////////////////////////////////////////////////////
...
void kwxAngularRegulator::OnMouse(wxMouseEvent & event)
{
    if (m_nStato == 0 && event.Entering())	//dentro nel controllo
    {
        ...
        if (m_nStato >= 1 && event.Leaving())	//fuori dal controllo
        {
            m_nStato = 0 ;
        }
        else
            if (m_nStato == 1 && event.LeftDown())	//click o inizio trascinamento
            {
                m_nStato = 2 ;
                ...
                if (m_nStato == 2 && event.LeftIsDown())	//trascinamento
                {
                    m_mousePosition = event.GetPosition();
                    SetPosition() ;
                }
                else
                    if (m_nStato == 2 && event.LeftUp())	//fine trascinamento o rilascio click
                    {
                        m_nStato = 1 ;
                    }
}
