...
public:
    ...
    void Init(const MatrixObject& mo)
    {
        Init(mo.GetData(),mo.GetWidth(),mo.GetHeight());
    }
    ...
    bool SetDataAt(const wxPoint& punkt, char data)
    {
        return SetDataAt(punkt.x,punkt.y,data);
    }
    bool SetDatesAt(int x, int y, const MatrixObject &mo);
    bool SetDatesAt(const wxPoint& punkt, const MatrixObject &mo)
    {
        return SetDatesAt(punkt.x,punkt.y,mo);
    }
