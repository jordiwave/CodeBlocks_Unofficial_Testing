...
bool wxBmpComboBox::SetButtonStyle(long style)
{
    ...
    if (style & wxBMPCOMBO_LEFT)
    {
        n_styles++;
    }

    if (style & wxBMPCOMBO_RIGHT)
    {
        n_styles++;
    }
    ...
    if (n_styles < 1)
    {
        style |= (m_label_style & BMPCOMBO_LABEL_MASK);
    }
...
wxSize wxBmpComboBox::DoGetBestSize() const
{
    if (GetCount() == 0)
    {
        return DropDownBase::DoGetBestSize();
    }
    ...
    if (size.y < DROPDOWN_DROP_HEIGHT)
    {
...
void wxBmpComboBox::CalcLayout()
{
    ...
    for (int n = 0; n < count; n++)
    {
        ...
        if (bmp.Ok())
        {
            ...
            if (width  > m_bitmapSize.x)
            {
                m_bitmapSize.x = width;
            }

            if (height > m_bitmapSize.y)
            {
                m_bitmapSize.y = height;
            }
        }
        ...
        if (!m_labels[n].IsEmpty())
        {
            ...
            if (width  > m_labelSize.x)
            {
                m_labelSize.x = width;
            }

            if (height > m_labelSize.y)
            {
                m_labelSize.y = height;
            }
        }
    }
...
void wxBmpComboBox::CalcLabelBitmapPos(int n, const wxSize & area, wxPoint & labelPos, wxPoint & bitmapPos) const
{
    ...
    if (m_bitmapSize.x == 0)      // There aren't any bitmaps, left align label
    {
        ...
        if (m_labelSize.x == 0)  // There aren't any labels, center bitmap
        {
            bitmapPos = wxPoint((area.x - bw) / 2, (area.y - bh) / 2);
        }
        else
            if ((m_label_style & wxBMPCOMBO_RIGHT) != 0)
            {
