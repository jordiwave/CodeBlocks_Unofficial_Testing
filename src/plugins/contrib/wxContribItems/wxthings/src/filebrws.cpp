...
void wxFileBrowser::DoSize()
{
    ...
    if (m_splitterWin && m_splitterWin->IsSplit())
    {
        ...
        if (splitter_pos < min_size)
        {
            ...
            if (splitter_pos > splitter_size - min_size)
            {
...
void wxFileBrowser::SetBrowserStyle(long style)
{
    ...
    if (style & wxFILEBROWSER_LIST)
    {
        n_styles++;
    }

    if (style & wxFILEBROWSER_DETAILS)
    {
        n_styles++;
    }

    if (style & wxFILEBROWSER_LARGE_ICON)
    {
        n_styles++;
    }

    if (style & wxFILEBROWSER_SMALL_ICON)
    {
        n_styles++;
    }

    if (style & wxFILEBROWSER_PREVIEW)
    {
        n_styles++;
    }
...
void wxFileBrowser::AddPathHistory(const wxString & path)
{
    ...
    if (count == 0)
    {
        ...
        if (m_path_history_index == count - 1)
        {
            m_path_history_index++;
            m_pathHistory.Add(path);
        }
...
void wxFileBrowser::OnListMenu(wxCommandEvent & event)
{
    switch (event.GetId())
    {
        ...
        case ID_wxFILEBROWSER_SHOW_HIDDEN    :
            ShowHidden(event.IsChecked());
            break;

        case ID_wxFILEBROWSER_SHOW_FOLDERS   :
            ShowFolders(event.IsChecked());
            break;

        case ID_wxFILEBROWSER_SPLIT_VERTICAL :
            SplitVertical(event.IsChecked());
            break;
...
bool wxFileBrowser::OpenFilePath(const wxString & filePath, bool send_event)
{
    ...
    if (path.Contains(wxT("..")))
    {
        ...
        // path is "../some/dir" - note: no need to check for at least 3 chars
            ...
            if ((path.Mid(0, 3) == wxString(wxT("..")) + wxFILE_SEP_PATH))
            {
                path = GetPath(false).BeforeLast(wxFILE_SEP_PATH) + wxFILE_SEP_PATH + path.AfterFirst(wxFILE_SEP_PATH);
            }
            // path is "/some/where/.."
            else
                if (path.Mid(path.Len() - 3) == wxFILE_SEP_PATH + wxString(wxT("..")))
                {
...
bool wxFileBrowser::InsertComboItem(wxComboBox * combo, const wxString & item, int pos) const
{
    ...
    if (combo_index == wxNOT_FOUND)
    {
        ...
        if ((combo_index == pos) || (combo_index < pos))
        {
            return true;
        }
        else
...
bool wxFileBrowser::SetFilters(const wxString & filter, int select)
{
    ...
    if (!m_dirCtrl)
    {
        return true;    // This is during creation
    }
