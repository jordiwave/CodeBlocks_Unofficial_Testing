...
wxMatrix2D wxMatrix2D::InsertRows(int row, const wxMatrix2D & matrix) const
{
    ...
    if (row < 1)                // prepend the matrix to the beginning
    {
        ...
        if (row >= height)     // append the matrix to the end
        {
...
wxMatrix2D wxMatrix2D::InsertCols(int col, const wxMatrix2D & matrix) const
{
    ...
    for (j = 0; j < height; j++)
    {
        if (col < 1)                // prepend the matrix to the beginning
        {
            ...
            if (col >= width)     // append the matrix to the end
            {
    ...
    for (r = 0; r <= height / 2; ++r)
    {
        ...
        for (s = 0; s < w * 4 - 4; ++s)
        {
            ...
            if ((i == w - 1) && (i_inc == 1))       // upper right going down
            {
                ...
                if ((j == w - 1) && (j_inc == 1))  // lower right going left
                {
                    i_inc = -1;
                    j_inc = 0;
                }
                else
                    if ((i == 0) && (i_inc == -1))     // lower left going up
                    {
                        i_inc = 0;
                        j_inc = -1;
                    }
            ...
            if ((ii == w - 1) && (ii_inc == 1))     // upper right going down
            {
                ...
                if ((jj == w - 1) && (jj_inc == 1)) // lower right going left
                {
                    ii_inc = -1;
                    jj_inc = 0;
                }
                else
                    if ((ii == 0) && (ii_inc == -1))   // lower left going up
                    {
                        ii_inc = 0;
                        jj_inc = -1;
                    }
                    else
                        if ((jj == 0) && (jj_inc == -1))   // upper left going right
                        {
                            ii_inc = 1;
                            jj_inc = 0;
                        }
