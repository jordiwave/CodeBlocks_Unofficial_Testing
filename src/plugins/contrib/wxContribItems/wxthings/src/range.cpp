...
{
    if (int(m_ranges.GetCount()) < 1)
    {
        return wxEmptyRangeInt;
    }
...
int wxRangeIntSelection::Index(int i) const
{
    ...
    if (i < m_ranges[0].m_min)
    {
        return wxNOT_FOUND;
    }

    if (i > m_ranges[count - 1].m_max)
    {
        return wxNOT_FOUND;
    }
...
int wxRangeIntSelection::NearestIndex(int i) const
{
    ...
    while (lo < hi)
    {
        ...
        if (res == 0)
        {
            ...
            if ((i >= m_ranges[tmp].m_max) && (i < m_ranges[wxMin(tmp + 1, count - 1)].m_min))
            {
                return tmp;
            }
            else
        ...
        else
        {
            ...
            for (i = wxMax(nearest - 1, 1); i < count; i++)
            {
                if (range.m_max + 1 < m_ranges[i - 1].m_min)
                {
                    ...
                    if (m_ranges[i - 1].Combine(m_ranges[i], true))
                    {
                        m_ranges.RemoveAt(i);
                        count--;
                        i--;
