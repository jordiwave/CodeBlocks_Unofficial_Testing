...
wxPanel * wxFNBCustomizeDialog::CreateOptionsPage()
{
    ...
    if (pc->HasFlag(wxFNB_FF2))
    {
        ...
        if (pc->HasFlag(wxFNB_VC71))
        {
            ...
            if (pc->HasFlag(wxFNB_VC8))
            {
                ...
                if (pc->HasFlag(wxFNB_FANCY_TABS))
                {
                    selection = 3;
                    enableGrad = false;
                    enableColorTab = false;
    ...
    if (pc->HasFlag(wxFNB_DROPDOWN_TABS_LIST))
    {
        ...
        if (pc->HasFlag(wxFNB_NO_NAV_BUTTONS) && !pc->HasFlag(wxFNB_DROPDOWN_TABS_LIST))
        {
