                           ...
                           const wxString & inName)            // name of button
{
    ...
    if (name.Len() == 0)
    {
        name.Printf(_T("SpeedButton-%d"), sbgCount);
    }
    ...
    if (! wxControl::Create(inParent, inID, pos, size, n, inVal, name))
    {
        return false;
    }
...
wxSize wxSpeedButton::DoGetBestSize(void)
{
    ...
    if (((i & wxBU_LEFT) != 0) || ((i & wxBU_RIGHT) != 0))
    {
        ...
        if (((i & wxBU_TOP) != 0) || ((i & wxBU_BOTTOM) != 0))
        {
            n = (bw > lw) ? bw : lw;
...
void    wxSpeedButton::SetAlign(int inAlign)
{
    ...
    if ((n != wxBU_LEFT) && (n != wxBU_TOP) && (n != wxBU_RIGHT) && (n != wxBU_BOTTOM))
    {
        n = wxBU_LEFT;
    }
...
void wxSpeedButton::SendEvent(bool inLeft)
{
    ...
    if (inLeft)
    {
        event.SetEventType(wxEVT_COMMAND_LEFT_CLICK);
    }
    else
    {
        event.SetEventType(wxEVT_COMMAND_RIGHT_CLICK);
    }
...
void    wxSpeedButton::Paint(wxDC & dc)
{
    ...
    if (mMouseDown || mButtonDown)
    {
        n = n | wxCONTROL_PRESSED;
    }

    if (mButtonFocused)
    {
        n = n | wxCONTROL_ISDEFAULT;
    }
    ...
    if (bmp.IsOk())
    {
        dc.DrawBitmap(bmp, mGlyphPos.x, mGlyphPos.y, true);
    }
