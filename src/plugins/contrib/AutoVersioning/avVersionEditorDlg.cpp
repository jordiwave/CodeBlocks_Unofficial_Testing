...
ForceValidation:; //To force validation when the user clicks the Accept buttom or other control not of type wxTexCtrl
        ...
        if (l_FocusedControl != this->FindFocus() && l_FocusedControl != 0)
        {
            ...
            if (l_FocusedControl->GetName() == _T("ID_MINORMAXIMUM_TEXT"))
            {
                ...
                if (l_FocusedControl->GetName() == _T("ID_BUILDNUMBERMAX_TEXT"))
                {
                    ifBlank = _T("0");
                    ifZero = _T("0");
                }
                else
                    if (l_FocusedControl->GetName() == _T("ID_REVISIONMAX_TEXT"))
                    {
                        ifBlank = _T("0");
                        ifZero = _T("0");
                    }
                    else
                        if (l_FocusedControl->GetName() == _T("ID_BUILDTIMES_TEXT"))
                        {
                            ifBlank = _T("100");
                            ifZero = _T("1");
                        }
                        else
                            if (l_FocusedControl->GetName() == _T("ID_REVISIONRANDOM_TEXT"))
                            {
                                ifBlank = _T("10");
                                ifZero = _T("1");
                            }
            ...
            if (l_FocusedControl->GetValue() == _T("0"))
            {
                ...
                if (l_FocusedControl->GetValue().Trim() == _T(""))
                {
        ...
        if (this->FindFocus() == txtRevisionRandom)
        {
            ...
            if (this->FindFocus() == txtMinorMaximun)
            {
                l_FocusedControl = txtMinorMaximun;
            }
            else
                if (this->FindFocus() == txtBuildNumberMaximun)
                {
                    l_FocusedControl = txtBuildNumberMaximun;
                }
                else
                    if (this->FindFocus() == txtRevisionMax)
                    {
                        l_FocusedControl = txtRevisionMax;
                    }
                    else
                        if (this->FindFocus() == txtBuildTimes)
                        {
