...
class FS_DWebBrowserEvents2 : public IDispatch
{
    ...
    public:
        ...
        bool Process(WXTYPE etype, wxString text = _T(""), long l1 = 0, long l2 = 0)
        {
            if (! m_iewin || ! m_iewin->GetParent())
            {
                return true;
                            ...
                            unsigned int * puArgErr)
        {
            ...
            switch (dispIdMember)
            {
                ...
                case DISPID_PROGRESSCHANGE:
                    Post(wxEVT_COMMAND_MSHTML_PROGRESSCHANGE, _T(""), LONG_ARG(1), LONG_ARG(0));
                    break;
...
void wxIEHtmlWin::LoadUrl(const wxString & url)
{
    ...
    if (m_webBrowser.Ok() && disp.Ok())
    {
        ...
        if (doc.Ok())
        {
            ...
            if (doc->get_location(&p) == S_OK)
            {
                hret = p->replace((BSTR)(const wchar_t *) url.wc_str(/*wxConvUTF8*/wxConvLocal));
...
#endif
    hret = m_webBrowser->Navigate((BSTR)(const wchar_t *) url.wc_str(/*wxConvUTF8*/wxConvLocal),
