...
void NassiView::OnKeyDown(wxKeyEvent & event)
{
    ...
    if (lastSelectedGBrick != 0 || event.ShiftDown())
    {
        return;
    }
...
bool NassiView::ExportCSource(wxTextOutputStream & text_stream, wxUint32 n)
{
    ...
    if (first)
    {
        first->SaveSource(text_stream, n);
    }
    else
    {
        return false;
