...
void NassiPlugin::OnChangeTool(wxCommandEvent & event)
{
    ...
    if (id == NASSI_ID_IF)
    {
        ed->ChangeToolTo(NassiView::NASSI_TOOL_IF);
    }
    else
        if (id == NASSI_ID_SWITCH)
        {
            ed->ChangeToolTo(NassiView::NASSI_TOOL_SWITCH);
        }
        else
            if (id == NASSI_ID_WHILE)
            {
                ed->ChangeToolTo(NassiView::NASSI_TOOL_WHILE);
            }
            else
                if (id == NASSI_ID_DOWHILE)
                {
                    ed->ChangeToolTo(NassiView::NASSI_TOOL_DOWHILE);
                }
                else
                    if (id == NASSI_ID_FOR)
                    {
                        ed->ChangeToolTo(NassiView::NASSI_TOOL_FOR);
                    }
                    else
                        if (id == NASSI_ID_BLOCK)
                        {
                            ed->ChangeToolTo(NassiView::NASSI_TOOL_BLOCK);
                        }
                        else
                            if (id == NASSI_ID_BREAK)
                            {
                                ed->ChangeToolTo(NassiView::NASSI_TOOL_BREAK);
                            }
                            else
                                if (id == NASSI_ID_CONTINUE)
                                {
                                    ed->ChangeToolTo(NassiView::NASSI_TOOL_CONTINUE);
                                }
                                else
                                    if (id == NASSI_ID_RETURN)
                                    {
                                        ed->ChangeToolTo(NassiView::NASSI_TOOL_RETURN);
                                    }
                                    else /*NASSI_ID_INSTRUCTION*/
                                    {
                                        ed->ChangeToolTo(NassiView::NASSI_TOOL_INSTRUCTION);
                                    }
}
...
void NassiPlugin::OnExport(wxCommandEvent & event)
{
    ...
    if (id == NASSI_ID_EXPORT_SOURCE)
    {
        ed->ExportCSource();
    }
    else
        if (id == NASSI_ID_EXPORT_SVG)
        {
            ed->ExportSVG();
        }
        else
            if (id == NASSI_ID_EXPORT_VHDL)
            {
                ed->ExportVHDLSource();
            }
                ...
                if (id == NASSI_ID_EXPORT_PS)
                {
                    ...
                    if (id == NASSI_ID_EXPORT_STRUKTEX)
                    {
                        ed->ExportStrukTeX();
                    }
                    else /*NASSI_ID_EXPORT_BITMAP*/
                    {
                        ed->ExportBitmap();
                    }
}
