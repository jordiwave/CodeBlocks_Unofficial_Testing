...
{
    bool operator()(AlignerMenuEntry i, AlignerMenuEntry j)
    {
        return (i.UsageCount <= j.UsageCount);
    }
...
void EditorTweaks::OnKeyPress(wxKeyEvent & event)
{
    ...
    if (m_laptop_friendly && keyCode == WXK_LEFT && event.AltDown())
    {
        ...
        if (m_laptop_friendly && keyCode == WXK_RIGHT && event.AltDown())
        {
            cbStyledTextCtrl * control = GetSafeControl();
            ...
            if (m_laptop_friendly && keyCode == WXK_UP && event.AltDown())
            {
                cbStyledTextCtrl * control = GetSafeControl();
                ...
                if (m_laptop_friendly && keyCode == WXK_DOWN && event.AltDown())
                {
                    cbStyledTextCtrl * control = GetSafeControl();
                    ...
                    if (m_laptop_friendly && keyCode == WXK_BACK && event.GetModifiers() == wxMOD_SHIFT)
                    {
                        cbStyledTextCtrl * control = GetSafeControl();
