...
bool wxSTEditorPrefs::IsEqualTo(const wxSTEditorPrefs & prefs) const
{
    ...
    if (prefArray.GetCount() != otherPrefArray.GetCount())
    {
        return false;
    }
    ...
    for (n = 0; n < count; n++)
    {
        if (prefArray[n] != otherPrefArray[n])
        {
            return false;
        }
    }
...
void wxSTEditorPrefs::UpdateEditor(wxSTEditor * editor)
{
    ...
    if ((margin0_width < 0) || (margin1_width < 0) || (margin2_width < 0))
    {
        ...
        if (margin0_width < 0)
        {
            margin0_width = M_PREFDATA->m_lineNumberAutoWidth;
        }

        if (margin1_width < 0)
        {
            margin1_width = M_PREFDATA->m_lineNumberAutoWidth;
        }

        if (margin2_width < 0)
        {
            margin2_width = M_PREFDATA->m_lineNumberAutoWidth;
        }
    }
                                 ...
                                 const wxString & configPath)
{
    ...
    for (pref_n = 0; pref_n < pref_count; pref_n++)
    {
        ...
        if (HasPrefFlag(pref_n, STE_PREF_FLAG_INT | STE_PREF_FLAG_BOOL))
        {
            ...
            if (config.Read(key + name, &strVal))
            {
