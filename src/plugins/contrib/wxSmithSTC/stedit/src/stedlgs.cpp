...
void wxSTEditorPrefDialogPagePrefs::GetControlValues()
{
    ...
    for (n = 0; n < count; n++)
    {
        ...
        if ((win_id == ID_STEDLG_FOLD_STYLES_LISTBOX) && wxDynamicCast(win, wxCheckListBox))
        {
            ...
            if (wxDynamicCast(win, wxCheckBox))
            {
                stePrefs.SetPrefBool(n, ((wxCheckBox *)win)->GetValue());
            }
            else
                if (wxDynamicCast(win, wxSpinCtrl))
                {
                    stePrefs.SetPrefInt(n, ((wxSpinCtrl *)win)->GetValue());
                }
                else
                    if (wxDynamicCast(win, wxChoice))
                    {
                        ...
                        if (wxDynamicCast(win, wxComboBox))
                        {
                            ...
                            if (wxDynamicCast(win, wxListBox))
                            {
...
void wxSTEditorPrefDialogPageStyles::OnMarginClick(wxStyledTextEvent & event)
{
    ...
    if (!m_fontBackButton)
    {
        return;    // set after editor is fully created
    }
...
void wxSTEditorPrefDialogPageStyles::GetControlValues()
{
    if (m_styleNotebook->GetSelection() == 0)
    {
        ...
        if (m_styleNotebook->GetSelection() == 1)
        {
...
void wxSTEditorPrefDialogPageStyles::SetControlValues()
{
    ...
    if (use_font)
    {
        m_fontSizeSpin->SetValue(steStyles.GetSize(m_current_style));
    }
...
void wxSTEditorPrefDialogPageLangs::SetControlValues()
{
    ...
    if (m_current_style_n > (int)style_count)
    {
        m_current_style_n = 0;
    }
    ...
    : wxDialog()
{
    ...
    if (wxDynamicCast(win, wxSTEditor) != NULL)
    {
        ...
        if (wxDynamicCast(win, wxSTEditorNotebook) != NULL)
        {
...
{
    if (sm_splitBeforeArray.GetCount() == 0)
    {
        sm_splitBeforeArray.Add(wxT("){}"));
    }

    if (sm_splitAfterArray.GetCount()  == 0)
    {
        sm_splitAfterArray.Add(wxT("(,;"));
    }

    if (sm_preserveArray.GetCount()    == 0)
    {
        sm_preserveArray.Add(wxT("\"\""));
    }
