...
static void wxMenu_SetAccelText(wxMenu * menu, const wxAcceleratorEntry & accel)
{
            ...
            it++)
    {
        ...
        if (item->IsSubMenu())
        {
            ...
            if (item->GetId() == accel.GetCommand())
            {
...
const char * wxConvAuto_GetBOMChars(wxBOM bom, size_t * count)
{
    ...
    {
        case wxBOM_UTF32BE:
            *count = WXSIZEOF(BOM_UTF32BE);
            return BOM_UTF32BE;

        case wxBOM_UTF32LE:
            *count = WXSIZEOF(BOM_UTF32LE);
            return BOM_UTF32LE;

        case wxBOM_UTF16BE:
            *count = WXSIZEOF(BOM_UTF16BE);
            return BOM_UTF16BE;

        case wxBOM_UTF16LE:
            *count = WXSIZEOF(BOM_UTF16LE);
            return BOM_UTF16LE;

        case wxBOM_UTF8   :
            *count = WXSIZEOF(BOM_UTF8);
            return BOM_UTF8;
...
const char * wxTextEncoding::GetBOMChars(TextEncoding_Type encoding, size_t * count)
{
    ...
    {
        case UTF8:
            return wxConvAuto_GetBOMChars(wxBOM_UTF8, count);

        case Unicode_LE:
            return wxConvAuto_GetBOMChars(wxBOM_UTF16LE, count);
#ifdef __WXMSW__
