...
void wxSTEUpdateSearchCtrl(wxSearchCtrl * searchCtrl,
                           const wxSTEditorFindReplaceData * findReplaceData)
{
    if ((searchCtrl == NULL) || (findReplaceData == NULL))
    {
        return;
    }
...
int wxSTEditorFindReplaceData::STEToScintillaFindFlags(int ste_flags)
{
    ...
    if (STE_HASBIT(ste_flags, STE_FR_MATCHCASE))
    {
        sci_flags |= wxSTC_FIND_MATCHCASE;
    }

    if (STE_HASBIT(ste_flags, STE_FR_WHOLEWORD))
    {
        sci_flags |= wxSTC_FIND_WHOLEWORD;
    }

    if (STE_HASBIT(ste_flags, STE_FR_WORDSTART))
    {
        sci_flags |= wxSTC_FIND_WORDSTART;
    }

    if (STE_HASBIT(ste_flags, STE_FR_REGEXP))
    {
        sci_flags |= wxSTC_FIND_REGEXP;
    }

    if (STE_HASBIT(ste_flags, STE_FR_POSIX))
    {
        sci_flags |= wxSTC_FIND_POSIX;
    }
...
int wxSTEditorFindReplaceData::ScintillaToSTEFindFlags(int sci_flags)
{
    ...
    if (STE_HASBIT(sci_flags, wxSTC_FIND_MATCHCASE))
    {
    ...
    {
        ste_flags |= STE_FR_WHOLEWORD;
    }

    if (STE_HASBIT(sci_flags, wxSTC_FIND_WORDSTART))
    {
        ste_flags |= STE_FR_WORDSTART;
    }

    if (STE_HASBIT(sci_flags, wxSTC_FIND_REGEXP))
    {
        ste_flags |= STE_FR_REGEXP;
    }

    if (STE_HASBIT(sci_flags, wxSTC_FIND_POSIX))
    {
        ste_flags |= STE_FR_POSIX;
    }
...
void wxSTEditorFindReplacePanel::SetData(wxSTEditorFindReplaceData * data)
{
    ...
    if (STE_HASBIT(flags, STE_FR_FROMCURSOR))
    {
        ...
        if (STE_HASBIT(flags, STE_FR_ALLDOCS))
        {
...
wxSTEditor * wxSTEditorFindReplacePanel::GetEditor() const
{
    ...
    if (targetWindow)
    {
        if (wxDynamicCast(targetWindow, wxSTEditorNotebook))
        {
            ...
            if (wxDynamicCast(targetWindow, wxSTEditorSplitter))
            {
                edit = wxDynamicCast(targetWindow, wxSTEditorSplitter)->GetEditor();
            }
            else
                if (wxDynamicCast(targetWindow, wxSTEditor))
                {
...
void wxSTEditorFindReplacePanel::UpdateFindFlags()
{
    ...
    if (m_findallCheckBox->GetValue())
    {
        m_flags |= STE_FR_FINDALL;
    }

    if (m_bookmarkallCheckBox->GetValue())
    {
        m_flags |= STE_FR_BOOKMARKALL;
    }
    ...
    if (m_scopewholeRadioButton->GetValue())
    {
        ...
        if (m_scopecursorRadioButton->GetValue())
        {
            m_flags |= STE_FR_FROMCURSOR;
        }
        else
            if (m_scopealldocsRadioButton->GetValue())
            {
...
void wxSTEditorFindReplacePanel::UpdateButtons()
{
    if (!m_created)
    {
        return;    // skip initial events sent from combobox in GTK
    }
    ...
    if (HasFlag(wxFR_REPLACEDIALOG))
    {
        ...
        if (enable && edit && !edit->SelectionIsFindString(findStr, flags))
        {
            ...
            if (!m_regexpFindCheckBox->IsChecked() && !m_findReplaceData->StringCmp(findStr, selText, flags))
            {
                enable = false;
            }
