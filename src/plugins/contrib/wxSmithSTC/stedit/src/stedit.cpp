                              ...
                              STE_TranslatePosType type)
{
    ...
    if (trans_start_pos)
    {
        *trans_start_pos = wxMin(start_pos, end_pos);
    }

    if (trans_end_pos)
    {
        *trans_end_pos   = wxMax(start_pos, end_pos);
    }
                                ...
                                STE_TranslatePosType type)
{
    ...
    if (trans_top_line)
    {
        *trans_top_line    = wxMin(top_line, bottom_line);
    }

    if (trans_bottom_line)
    {
        *trans_bottom_line = wxMax(top_line, bottom_line);
    }
...
wxString wxSTEditor::GetLineText(int line) const
{
    ...
    if (len > 0)
    {
        if (lineText[len - 1] == wxT('\n'))
        {
            ...
            if (lineText[len - 1] == wxT('\r'))             // remove \r for mac
            {
                                       ...
                                       STE_TranslatePosType type)
{
    ...
    if (GetTabWidth() > 0)
    {
        spaceString = wxString(wxT(' '), GetTabWidth());
    }
    ...
    for (line = top_line; line <= bottom_line; line++)
    {
        ...
        for (int n = 0; n < len;)
        {
            ...
            if (match_len == 0)
            {
                if (n == len)
                {
                    break;    // all done - perhaps whitespace?
                }
                                ...
                                bool    *   selected_file_bom)
{
    ...
    if (use_dialog)
    {
        ...
        if (fileDialog.ShowModal() == wxID_OK)
        {
            if (selectedFileName)
            {
                *selectedFileName     = fileDialog.GetPath();
            }

            if (selectedFileEncoding)
            {
                *selectedFileEncoding = fileDialog.m_encoding;
            }

            if (selected_file_bom)
            {
                *selected_file_bom    = fileDialog.m_file_bom;
            }
                                   ...
                                   STE_TextPos * found_start_pos, STE_TextPos * found_end_pos)
{
    ...
    if (found_start_pos)
    {
        *found_start_pos = search_target_start;
    }

    if (found_end_pos)
    {
        *found_end_pos   = search_target_end;
    }
    ...
    if (pos >= 0)
    {
        ...
        if (STE_HASBIT(flags, STE_FR_WRAPAROUND))
        {
                                  ...
                                  wxArrayInt * endPositions)
{
            ...
            pos = FindString(str, found_end_pos, -1, flags, STE_FINDSTRING_NOTHING, &found_start_pos, &found_end_pos))
    {
        ...
        if (startPositions)
        {
            startPositions->Add(found_start_pos);
        }

        if (endPositions)
        {
            endPositions->Add(found_end_pos);
        }
    }
...

{
    ...
    while ((curLine < maxLines) && (curLine > 0) && !isInside)
    {
        ...
        if (((direction == 1) && (status == ppcStart)) || ((direction == -1) && (status == ppcEnd)))
        {
            ...
            if ((level > 0) && (((direction == 1) && (status == ppcEnd)) || ((direction == -1) && (status == ppcStart))))
            {
                level--;
            }
            else
                if ((level == 0) && ((status == condEnd1) || (status == condEnd2)))
                {
                    isInside = true;
                }
    }
...
size_t wxSTEditor::DoGetAutoCompleteKeyWords(const wxString & root, wxArrayString & wordArray)
{
    ...
    if (!langs.IsOk() || !langs.HasLanguage(lang_n))
    {
        return 0;
    }
...
wxString wxSTEditor::EliminateDuplicateWords(const wxString & words0) const
{
    ...
    if (!words.IsEmpty())
    {
        words.RemoveLast(1);    // remove trailing space
    }
...
bool wxSTEditor::HandleMenuEvent(wxCommandEvent & event)
{
    ...
    switch (win_id)
    {
        ...
        case ID_STE_TOOLBAR_SEARCHCTRL_MENU9 :
        {
            ...
            if (index >= (int)findReplaceData->GetFindStrings().GetCount())
            {
                return true;
            }
        ...
        if ((tab_width > 1) && (len * tab_width > longest_len))
        {
            ...
            for (int i = 0; i < len; i++, c++)
            {
                if (*c == '\t')
                {
                    ...
                    if ((ctrl_char_symbol >= ste_ctrlCharLenghts_size) &&
