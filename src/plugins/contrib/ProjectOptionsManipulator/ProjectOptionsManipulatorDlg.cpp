 ...
 *
 * $Revision: 12304 $
 * $Id: ProjectOptionsManipulatorDlg.cpp 12304 2021-03-16 23:28:31Z fuscated $
...
{
    if (m_ChkOptionReplacePattern->GetValue() && (opt == eReplacePattern))
    {
        return true;
    }

    if (m_ChkOptionsCompiler->GetValue()      && (opt == eCompiler))
    {
        return true;
    }

    if (m_ChkOptionsLinker->GetValue()        && (opt == eLinker))
    {
        return true;
    }

    if (m_ChkOptionsResCompiler->GetValue()   && (opt == eResCompiler))
    {
        return true;
    }

    if (m_ChkOptionsCompilerPath->GetValue()  && (opt == eCompilerPaths))
    {
        return true;
    }

    if (m_ChkOptionsLinkerPath->GetValue()    && (opt == eLinkerPaths))
    {
        return true;
    }

    if (m_ChkOptionsResCompPath->GetValue()   && (opt == eResCompPaths))
    {
        return true;
    }

    if (m_ChkOptionsLinkerLibs->GetValue()    && (opt == eLinkerLibs))
    {
        return true;
    }

    if (m_ChkOptionsCustomVar->GetValue()     && (opt == eCustomVars))
    {
        return true;
    }
...
{
    if (m_ChoOptionLevel->GetSelection() == 0 && (opt == eProject))
    {
        return true;
    }

    if (m_ChoOptionLevel->GetSelection() == 1 && (opt == eTarget))
    {
        return true;
    }

    if (m_ChoOptionLevel->GetSelection() == 2)
    {
        return true;
    }
...
void ProjectOptionsManipulatorDlg::OnSearchCompilerClick(wxCommandEvent & event)
{
    ...
    if (choose_dlg.ShowModal() == wxID_OK)
    {
        if (event.GetId() == ID_BTN_SEARCH_COMPILER_SRC)
        {
            ...
            if (event.GetId() == ID_BTN_SEARCH_COMPILER_DEST)
            {
