 ...
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/contrib/help_plugin/man2html.cpp $
 */
        ...
        if (len == 2)
        {
            ...
            // Courier
                ...
                if (name == "CR")
                {
                    ...
                    if (name == "CW")   // CW is used by pod2man(1) (part of perldoc(1))
                    {
                        markup += /*"<span style=\"font-family:monospace\">"*/ closure;
                        closure = "";
                    }
                    ...
                    if (name == ".P")
                    {
                        ...
                        // Special KDE KIO man:
                            ...
                            if (name == ".KDE_VERSION_MAJOR")
                            {
                                ...
                                if (name == ".KDE_VERSION_MINOR")
                                {
                                    return KDE_VERSION_MINOR;
                                }
                                else
                                    if (name == ".KDE_VERSION_RELEASE")
                                    {
    ...
    while (currow)
    {
        ...
        while (currow->has(curfield))
        {
            if (currow->at(curfield).align != 'S' && currow->at(curfield).align != '^')
            {
                ...
                if (currow->at(curfield).size)
                {
                    out_html(change_to_size(currow->at(curfield).size));
                }
                ...
                if (currow->at(curfield).space)
                    for (i = 0; i < currow->at(curfield).space; i++)
                    {
                        out_html("&nbsp;");
                    }
            ...
            if (*c == 't')
            {
                ...
                if (*c == '\'' || *c == '"' || *c < ' ' || (*c == '\\' && c[1] == '('))
                {
                    ...
                    */
                    ...
                    while (*c != sep && (!tcmp || qstrncmp(c, tcmp, 4)))
                    {
                        c++;
                    }
...
static const char * lookup_abbrev(char * c)
{
    ...
    while (abbrev_list[i] && qstrcmp(c, abbrev_list[i]))
    {
        i = i + 2;
    }
...
static const char * section_name(char * c)
{
    ...
    while (section_list[i] && qstrcmp(c, section_list[i]))
    {
        i = i + 2;
    }
...
static int get_request(char * req, int len)
{
    ...
    while (requests[r] && qstrncmp(req, requests[r], len))
    {
        r++;
    }
...
static char * scan_request(char * c)
{
    ...
    while (*c == ' ' || *c == '\t')
    {
        c++;    // Spaces or tabs allowed between control character and request
    }
            ...
            switch (int request = get_request(c, nlen))
            {
                ...
                case REQ_El: // mdoc(7) "End List"
                {
                    ...
                    if (dl_set[itemdepth] & BL_DESC_LIST)
                    {
                        ...
                        if (dl_set[itemdepth] & BL_BULLET_LIST)
                        {
                            out_html("</UL>\n");
                        }
                        else
                            if (dl_set[itemdepth] & BL_ENUM_LIST)
                            {
                                out_html("</OL>\n");
                            }
                ...
                case REQ_It: // mdoc(7) "list ITem"
                {
                    ...
                    if (dl_set[itemdepth] & BL_DESC_LIST)
                    {
                        ...
                        if (dl_set[itemdepth] & (BL_BULLET_LIST | BL_ENUM_LIST))
                        {
                            out_html("<LI>");
                            c = scan_troff_mandoc(c, 1, NULL);
                            out_html(NEWLINE);
