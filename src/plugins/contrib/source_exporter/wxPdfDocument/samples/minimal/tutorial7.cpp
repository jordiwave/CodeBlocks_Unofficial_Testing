...
int createFontList(bool testMode)
{
    wxPdfDocument pdf;
...
int twoEncodings(bool testMode)
{
    int rc = 0;
...
int fontSample(bool testMode)
{
    ...
    if (!pdf.AddFont(wxS("liz"), wxS(""), wxS("liz.otf")))
    {
        ++rc;
    }

    if (!pdf.AddFont(wxS("GFSDidot"), wxS(""), wxS("GFSDidot.xml")))
    {
        ++rc;
    }
    ...
    if (!pdf.AddFont(wxS("Calligrapher"), wxS(""), wxS("calligra.xml")))
    {
        ++rc;
    }

    if (!pdf.AddFont(wxS("Feta"), wxS(""), wxS("feta16.xml")))
    {
        ++rc;
    }

    if (!pdf.AddFont(wxS("FetaAlphabet"), wxS(""), wxS("feta-alphabet16.xml")))
    {
        ++rc;
    }
