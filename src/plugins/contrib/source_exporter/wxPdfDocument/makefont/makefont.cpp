...
static int CompareGlyphListEntries(GlyphListEntry * item1, GlyphListEntry * item2)
{
...
/// Read encoding map
bool MakeFont::ReadMap(const wxString & enc, CTGMap & cc2gn)
{
    //Read a map file
...
short MakeFont::ReadShort(wxInputStream * stream)
{
    short i16;
    stream->Read(&i16, 2);
...
int MakeFont::ReadInt(wxInputStream * stream)
{
    int i32;
    stream->Read(&i32, 4);
...
void MakeFont::CheckTTF(const wxString & fileName, bool & embeddingAllowed, bool & subsettingAllowed, int & cffOffset, int & cffLength)
{
...
void MakeFont::CreateFontMetricsFile(const wxString & xmlFileName, wxPdfFontData & font, bool includeGlyphInfo)
{
...
bool MakeFont::MakeFontAFM(const wxString & fontFileName, const wxString & afmFileName,
        ...
        if (code == wxS("C"))
        {
            ...
            while (tkz.HasMoreTokens())
            {
                ...
                if (token == wxS("WX")) // Character width
                {
                    ...
                    if (token == wxS("N")) // Glyph name
                    {
                        glyphname = tkz.GetNextToken(); // Glyph name
                        dummy = tkz.GetNextToken(); // Semicolon
                    }
                    else
                        if (token == wxS("G")) // Glyph number
                        {
                            ...
                            if (token == wxS("B")) // Character bounding box
                            {
                ...
                if (code == wxS("Weight"))
                {
                    ...
                    if (code == wxS("ItalicAngle"))
                    {
                        ...
                        if (code == wxS("Ascender"))
                        {
                            ...
                            if (code == wxS("Descender"))
                            {
                                param.ToLong(&nParam);
                                fd.SetDescent(nParam);
                            }
                            else
                                if (code == wxS("UnderlineThickness"))
                                {
                                    param.ToLong(&nParam);
                                    fd.SetUnderlineThickness(nParam);
                                }
                                else
                                    if (code == wxS("UnderlinePosition"))
                                    {
                                        param.ToLong(&nParam);
                                        fd.SetUnderlinePosition(nParam);
                                    }
                                    else
                                        if (code == wxS("IsFixedPitch"))
                                        {
                                            if (param == wxS("true"))
                                            {
                                                flags += 1 << 0;
...
void MakeFont::WriteStreamBuffer(wxOutputStream & stream, const char * buffer)
{
    size_t buflen = strlen(buffer);
    stream.Write(buffer, buflen);
}

void MakeFont::WriteToUnicode(GlyphList & glyphs, wxMemoryOutputStream & toUnicode)
{
...
bool MakeFont::MakeFontUFM(const wxString & fontFileName,
                           const wxString & ufmFileName,
                           const wxString & type,
                           const wxString & volt)
{
...
bool MakeFont::MakeFontImmediate(const wxString & fontFileName)
{
    wxFileName fileName(fontFileName);
