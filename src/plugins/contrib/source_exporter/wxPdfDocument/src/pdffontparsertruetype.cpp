...
wxMemoryInputStream * wxPdfFontParserTrueType::LoadTrueTypeFontStream(const wxFont & font)
{
...
int wxPdfFontParserTrueType::CalculateChecksum(const char * b, size_t length)
{
    size_t len = length / 4;
    int d0 = 0;
    int d1 = 0;
...
wxPdfFontData * wxPdfFontParserTrueType::IdentifyFont(const wxString & fontFileName, int fontIndex)
{
    bool ok = true;
...
#ifdef __WXMSW__
wxPdfFontData * wxPdfFontParserTrueType::IdentifyFont(const wxFont & font)
{
    bool ok = true;
    wxPdfFontData * fontData = NULL;
...
bool wxPdfFontParserTrueType::PrepareFontData(wxPdfFontData * fontData)
{
    ...
    if (ok)
    {
        ...
        if (m_cmapExt != NULL)
        {
            ...
            if (!m_fontSpecific && m_cmap31 != NULL)
            {
                cMap = m_cmap31;
            }
            else
                if (m_fontSpecific && m_cmap10 != NULL)
                {
                    cMap = m_cmap10;
                }
                else
...
bool wxPdfFontParserTrueType::ReadGlyphWidths(int numberOfHMetrics, int unitsPerEm)
{
    wxPdfTableDirectoryEntry * tableLocation;
...
wxArrayString wxPdfFontParserTrueType::GetNames(int id, bool namesOnly)
{
    wxArrayString names;
    wxPdfTableDirectoryEntry * tableLocation;
    wxPdfTableDirectory::iterator entry = m_tableDirectory->find(wxS("name"));
