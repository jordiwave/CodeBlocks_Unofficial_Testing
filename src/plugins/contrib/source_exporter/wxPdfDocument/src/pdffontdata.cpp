...
wxString wxPdfFontData::GetNodeContent(const wxXmlNode * node)
{
...
bool wxPdfFontData::GetFontDescription(const wxXmlNode * node, wxPdfFontDescription & fontDescription)
{
    bool bAscent             = false,
         bDescent            = false,
         bCapheight          = false,
    ...
    while (child)
    {
        ...
        if (child->GetName() == wxS("ascent"))
        {
            ...
            if (child->GetName() == wxS("descent"))
            {
                ...
                if (child->GetName() == wxS("cap-height"))
                {
                    value = GetNodeContent(child);
                    ...
                    if (child->GetName() == wxS("flags"))
                    {
                        ...
                        if (child->GetName() == wxS("font-bbox"))
                        {
                            value = GetNodeContent(child);

                            if (value.Length() > 0 && value[0] == wxS('[') && value.Last() == wxS(']'))
                            {
                                bFontbbox = true;
                            ...
                            if (child->GetName() == wxS("italic-angle"))
                            {
                                value = GetNodeContent(child);
                                ...
                                if (child->GetName() == wxS("stem-v"))
                                {
                                    ...
                                    if (child->GetName() == wxS("missing-width"))
                                    {
                                        value = GetNodeContent(child);
                                        ...
                                        if (child->GetName() == wxS("x-height"))
                                        {
                                            ...
                                            if (child->GetName() == wxS("underline-position"))
                                            {
                                                value = GetNodeContent(child);
                                                ...
                                                if (child->GetName() == wxS("underline-thickness"))
                                                {
                                                    value = GetNodeContent(child);
...
void wxPdfFontData::SetEncoding(wxPdfEncoding * encoding)
{
    m_encoding = encoding;
}
...
void wxPdfFontData::SetEncodingChecker(wxPdfEncodingChecker * encodingChecker)
{
    m_encodingChecker = encodingChecker;
}

wxPdfEncodingChecker * wxPdfFontData::GetEncodingChecker() const
{
...
void wxPdfFontData::SetFamily(const wxString & family)
{
    m_family = family;
}
...
int wxPdfFontData::GetBBoxTopPosition() const
{
    ...
    wxStringTokenizer tkz(bBox, wxS(" []"), wxTOKEN_STRTOK);
...
void wxPdfFontData::SetEncoding(const wxString & enc)
{
    m_enc = enc;
}
...
wxString wxPdfFontData::GetWidthsAsString(bool subset, wxPdfSortedArrayInt * usedGlyphs, wxPdfChar2GlyphMap * subsetGlyphs) const
{
    wxUnusedVar(subset);
...
double wxPdfFontData::GetStringWidth(const wxString & s, const wxPdfEncoding * encoding, bool withKerning, double charSpacing) const
{
    wxUnusedVar(s);
...
bool wxPdfFontData::GetGlyphNames(wxArrayString & glyphNames) const
{
    wxUnusedVar(glyphNames);
    return false;
}

size_t wxPdfFontData::WriteFontData(wxOutputStream * fontData, wxPdfSortedArrayInt * usedGlyphs, wxPdfChar2GlyphMap * subsetGlyphs)
{
...
size_t wxPdfFontData::WriteUnicodeMap(wxOutputStream * mapData,
...
size_t wxPdfFontData::WriteCIDToGIDMap(wxOutputStream * mapData,
                                       const wxPdfEncoding * encoding, wxPdfSortedArrayInt * usedGlyphs, wxPdfChar2GlyphMap * subsetGlyphs)
{
    wxUnusedVar(mapData);
    wxUnusedVar(encoding);
...
size_t wxPdfFontData::WriteCIDSet(wxOutputStream * setData,
                                  const wxPdfEncoding * encoding, wxPdfSortedArrayInt * usedGlyphs,  wxPdfChar2GlyphMap * subsetGlyphs)
{
    wxUnusedVar(setData);
    wxUnusedVar(encoding);
...
const wxPdfFontDescription & wxPdfFontData::GetDescription() const
{
    return m_desc;
}

bool wxPdfFontData::LoadFontMetrics(wxXmlNode * root)
{
    wxUnusedVar(root);
    return false;
};

bool wxPdfFontData::CanShow(const wxString & s, const wxPdfEncoding * encoding) const
{
    wxUnusedVar(encoding);
    bool canShow = true;
#if wxUSE_UNICODE
...
wxString wxPdfFontData::ConvertToValid(const wxString & s, wxUniChar replace) const
{
    wxString t;
...
wxString wxPdfFontData::ConvertCID2GID(const wxString & s,
...
wxString wxPdfFontData::ConvertGlyph(wxUint32 glyph,
                                     const wxPdfEncoding * encoding,
                                     wxPdfSortedArrayInt * usedGlyphs,
                                     wxPdfChar2GlyphMap * subsetGlyphs) const
{
...
#if wxUSE_UNICODE
wxMBConv * wxPdfFontData::GetEncodingConv() const
{
    return &wxConvISO8859_1;
}

void wxPdfFontData::CreateDefaultEncodingConv()
{
}
#endif

void wxPdfFontData::SetGlyphWidths(const wxPdfArrayUint16 & glyphWidths)
{
    wxUnusedVar(glyphWidths);
}

int wxPdfFontData::FindStyleFromName(const wxString & name)
{
...
int wxPdfFontData::CompareGlyphListEntries(wxPdfGlyphListEntry * item1, wxPdfGlyphListEntry * item2)
{
    return item1->m_gid - item2->m_gid;
}

void wxPdfFontData::WriteStreamBuffer(wxOutputStream & stream, const char * buffer)
{
    size_t buflen = strlen(buffer);
    stream.Write(buffer, buflen);
}

void wxPdfFontData::WriteToUnicode(wxPdfGlyphList & glyphs, wxMemoryOutputStream & toUnicode, bool simple)
{
    wxString gidFormat = (simple) ? wxString(wxS("<%02x>")) : wxString(wxS("<%04x>"));
    ...
    for (k = 0; k < numGlyphs; ++k)
    {
        if (size == 0)
        {
            ...
            wxString sizeStr = wxString::Format(wxS("%u"), size);
