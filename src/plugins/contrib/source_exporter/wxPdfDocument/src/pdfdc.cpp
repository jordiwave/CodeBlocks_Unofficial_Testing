...
void wxPdfDC::SetImageType(wxBitmapType bitmapType, int quality)
{
...
static double angleByCoords(wxCoord xa, wxCoord ya, wxCoord xc, wxCoord yc)
{
...
void wxPdfDCImpl::SetImageType(wxBitmapType bitmapType, int quality)
{
    m_jpegFormat = (bitmapType == wxBITMAP_TYPE_JPEG);
    m_jpegQuality = (quality >= 0) ? ((quality <= 100) ? quality : 75) : 75;
}

void wxPdfDCImpl::Clear()
{
    // Not yet implemented
}

bool wxPdfDCImpl::StartDoc(const wxString & message)
{
...
void wxPdfDCImpl::SetFont(const wxFont & font)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    m_font = font;
...
void wxPdfDCImpl::SetPen(const wxPen & pen)
{
    if (pen.Ok())
    {
        m_pen = pen;
...
void wxPdfDCImpl::SetBrush(const wxBrush & brush)
{
    if (brush.Ok())
    {
        m_brush = brush;
...
void wxPdfDCImpl::SetBackground(const wxBrush & brush)
{
    if (brush.Ok())
    {
...
void wxPdfDCImpl::SetBackgroundMode(int mode)
{
    // TODO: check implementation
    m_backgroundMode = (mode == wxBRUSHSTYLE_TRANSPARENT) ? wxBRUSHSTYLE_TRANSPARENT : wxBRUSHSTYLE_SOLID;
}

void wxPdfDCImpl::SetPalette(const wxPalette & palette)
{
    // Not yet implemented
    wxUnusedVar(palette);
}

void wxPdfDCImpl::SetTextForeground(const wxColour & colour)
{
    if (colour.IsOk())
    {
...
void wxPdfDCImpl::DestroyClippingRegion()
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
wxCoord wxPdfDCImpl::GetCharHeight() const
{
...
wxCoord wxPdfDCImpl::GetCharWidth() const
{
    int height;
    int width = 8;
...
bool wxPdfDCImpl::CanDrawBitmap() const
{
    return true;
}

bool wxPdfDCImpl::CanGetTextExtent() const
{
    return true;
}

int wxPdfDCImpl::GetDepth() const
{
    // TODO: check value
    return 32;
}

wxSize wxPdfDCImpl::GetPPI() const
{
    int ppi = (int) m_ppi;
    return wxSize(ppi, ppi);
}

void wxPdfDCImpl::ComputeScaleAndOrigin()
{
    m_scaleX = m_logicalScaleX * m_userScaleX;
    m_scaleY = m_logicalScaleY * m_userScaleY;
}

void wxPdfDCImpl::SetMapMode(wxMappingMode mode)
{
    m_mappingMode = mode;
...
void wxPdfDCImpl::SetUserScale(double x, double y)
{
    m_userScaleX = x;
    m_userScaleY = y;
...
void wxPdfDCImpl::SetLogicalScale(double x, double y)
{
    m_logicalScaleX = x;
    m_logicalScaleY = y;
    ComputeScaleAndOrigin();
...
void wxPdfDCImpl::SetLogicalOrigin(wxCoord x, wxCoord y)
{
...
void wxPdfDCImpl::SetDeviceOrigin(wxCoord x, wxCoord y)
{
    m_deviceOriginX = x;
    m_deviceOriginY = y;
    ComputeScaleAndOrigin();
...
void wxPdfDCImpl::SetAxisOrientation(bool xLeftRight, bool yBottomUp)
{
...
bool wxPdfDCImpl::CanUseTransformMatrix() const
{
    return true;
}

bool wxPdfDCImpl::SetTransformMatrix(const wxAffineMatrix2D & matrix)
{
...
wxAffineMatrix2D wxPdfDCImpl::GetTransformMatrix() const
{
    wxCHECK_MSG(m_pdfDocument, wxAffineMatrix2D(), wxS("Invalid PDF DC"));
    return m_matrix;
}

void wxPdfDCImpl::ResetTransformMatrix()
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::SetLogicalFunction(wxRasterOperationMode function)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    // TODO: check implementation
...
bool wxPdfDCImpl::DoFloodFill(wxCoord x, wxCoord y, const wxColour & col, wxFloodFillStyle style)
{
    wxUnusedVar(x);
    wxUnusedVar(y);
    wxUnusedVar(col);
...
void wxPdfDCImpl::DoGradientFillLinear(const wxRect & rect,
...
void wxPdfDCImpl::DoGradientFillConcentric(const wxRect & rect,
                                           const wxColour & initialColour,
                                           const wxColour & destColour,
...
bool wxPdfDCImpl::DoGetPixel(wxCoord x, wxCoord y, wxColour * col) const
{
    wxUnusedVar(x);
    wxUnusedVar(y);
    wxUnusedVar(col);
...
void wxPdfDCImpl::DoDrawPoint(wxCoord x, wxCoord y)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    SetupPen();
    SetupAlpha();
...
#if wxUSE_SPLINES
void wxPdfDCImpl::DoDrawSpline(const wxPointList * points)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    SetupPen();
    SetupAlpha();
...
void wxPdfDCImpl::DoDrawLine(wxCoord x1, wxCoord y1, wxCoord x2, wxCoord y2)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::DoDrawArc(wxCoord x1, wxCoord y1,
...
void wxPdfDCImpl::DoDrawCheckMark(wxCoord x, wxCoord y,
...
void wxPdfDCImpl::DoDrawEllipticArc(wxCoord x, wxCoord y, wxCoord width, wxCoord height,
                                    double sa, double ea)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    bool doFill = GetBrush().IsNonTransparent();
...
void wxPdfDCImpl::DoDrawRectangle(wxCoord x, wxCoord y, wxCoord width, wxCoord height)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    bool doFill = GetBrush().IsNonTransparent();
    bool doDraw = GetPen().IsNonTransparent();
...
void wxPdfDCImpl::DoDrawRoundedRectangle(wxCoord x, wxCoord y,
                                         wxCoord width, wxCoord height,
                                         double radius)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::DoDrawEllipse(wxCoord x, wxCoord y, wxCoord width, wxCoord height)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    bool doFill = GetBrush().IsNonTransparent();
    bool doDraw = GetPen().IsNonTransparent();
...
void wxPdfDCImpl::DoCrossHair(wxCoord x, wxCoord y)
{
    wxUnusedVar(x);
    wxUnusedVar(y);
...
void wxPdfDCImpl::DoDrawIcon(const wxIcon & icon, wxCoord x, wxCoord y)
{
    DoDrawBitmap(icon, x, y, true);
}

void wxPdfDCImpl::DoDrawBitmap(const wxBitmap & bitmap, wxCoord x, wxCoord y, bool useMask)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::DoDrawText(const wxString & text, wxCoord x, wxCoord y)
{
    if (text.Find(wxS('\n')) == wxNOT_FOUND)
    {
...
void wxPdfDCImpl::DoDrawRotatedText(const wxString & text, wxCoord x, wxCoord y, double angle)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
bool wxPdfDCImpl::DoBlit(wxCoord xdest, wxCoord ydest, wxCoord width, wxCoord height,
...
void wxPdfDCImpl::DoGetSize(int * width, int * height) const
{
    int w;
    int h;
...
void wxPdfDCImpl::DoGetSizeMM(int * width, int * height) const
{
    int w;
    int h;
...
void wxPdfDCImpl::DoDrawLines(int n, const wxPoint points[], wxCoord xoffset, wxCoord yoffset)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::DoDrawPolygon(int n, const wxPoint points[],
...
void wxPdfDCImpl::DoDrawPolyPolygon(int n, const int count[], const wxPoint points[],
                                    wxCoord xoffset, wxCoord yoffset,
...
void wxPdfDCImpl::DoSetClippingRegionAsRegion(const wxRegion & region)
{
    wxCoord x, y, w, h;
...
void wxPdfDCImpl::DoSetClippingRegion(wxCoord x, wxCoord y, wxCoord width, wxCoord height)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::DoSetDeviceClippingRegion(const wxRegion & region)
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
    wxCoord x, y, w, h;
    region.GetBox(x, y, w, h);
...
void wxPdfDCImpl::DoGetTextExtent(const wxString & text,
                                  wxCoord * x, wxCoord * y,
                                  wxCoord * descent,
...
bool wxPdfDCImpl::DoGetPartialTextExtents(const wxString & text, wxArrayInt & widths) const
{
...
bool wxPdfDCImpl::MustSetCurrentPen(const wxPen & currentPen) const
{
...
bool wxPdfDCImpl::MustSetCurrentBrush(const wxBrush & currentBrush) const
{
    ...
    if (curBrush != wxNullBrush)
    {
        if (MustSetCurrentBrush(curBrush))
        {
            ...
            if (pdfPatternStyle >= wxPDF_PATTERNSTYLE_FIRST_HATCH && pdfPatternStyle <= wxPDF_PATTERNSTYLE_LAST_HATCH)
            {
                ...
                if (pdfPatternStyle == wxPDF_PATTERNSTYLE_IMAGE)
                {
...
void wxPdfDCImpl::SetupTextAlpha()
{
    wxCHECK_RET(m_pdfDocument, wxS("Invalid PDF DC"));
...
void wxPdfDCImpl::CalculateFontMetrics(wxPdfFontDescription * desc, int pointSize,
