...
class wxPdfCffDictElement
{
    public:
        ...
        wxPdfCffIndexElement * GetArgument() const
        {
            return m_argument;
        }
        int GetArgumentLength() const
        {
            return m_argument->GetLength();
        }
        void SetArgumentOffset(int argOffset)
        {
            m_argOffset = argOffset;
        }
        int GetArgumentOffset() const
        {
            return m_argOffset;
        }
...
wxMemoryOutputStream * wxPdfFontSubsetCff::CreateSubset(wxInputStream * inFont, wxPdfChar2GlyphMap * usedGlyphs, bool includeCmap)
{
    m_inFont = inFont;
...
bool wxPdfFontSubsetCff::ReadCffFont()
{
    ...
    if (ok)
    {
        ok = ReadFontName();    //  cairo_cff_font_read_name,
    }

    if (ok)
    {
        ok = ReadTopDict();    //  cairo_cff_font_read_top_dict,
    }

    if (ok)
    {
        ok = ReadStrings();    //  cairo_cff_font_read_strings,
    }

    if (ok)
    {
        ok = ReadGlobalSubroutines();    //  cairo_cff_font_read_global_subroutines,
    }
...
bool wxPdfFontSubsetCff::ReadFontDict(wxPdfCffDictionary * dict, int dictOffset, int dictSize)
{
    bool ok = true;
    SeekI(dictOffset);
...
wxPdfCffDictElement * wxPdfFontSubsetCff::FindDictElement(wxPdfCffDictionary * dict, int key)
{
...
void wxPdfFontSubsetCff::SetDictElementArgument(wxPdfCffDictionary * dict, int key, wxMemoryOutputStream & buffer)
{
...
void wxPdfFontSubsetCff::RemoveDictElement(wxPdfCffDictionary * dict, int key)
{
    wxPdfCffDictionary::iterator dictIter = dict->find(key);
...
bool wxPdfFontSubsetCff::ReadPrivateDict(wxPdfCffDictionary * privateDict, wxPdfCffIndexArray * localSubrIndex, int offset, int size)
{
...
void wxPdfFontSubsetCff::EncodeIntegerMax(int value, wxMemoryOutputStream & buffer)
{
    char locBuffer[5];
    locBuffer[0] = 29;
...
void wxPdfFontSubsetCff::EncodeInteger(int value, wxMemoryOutputStream & buffer)
{
    char locBuffer[5];
    size_t count;
...
void wxPdfFontSubsetCff::SubsetDictString(wxPdfCffDictionary * dict, int op)
{
...
void wxPdfFontSubsetCff::WriteInteger(int value, int size, wxMemoryOutputStream * buffer)
{
    char locBuffer[4];
    int i = 0;
...
void wxPdfFontSubsetCff::WriteDictOperator(wxPdfCffDictElement * dictElement)
{
    int offset = TellO();
...
int wxPdfFontSubsetCff::GetLocation(wxPdfCffDictionary * dict, int op)
{
    int offset = -1;
...
void wxPdfFontSubsetCff::WritePrivateDict(int dictNum, wxPdfCffDictionary * parentDict, wxPdfCffDictionary * privateDict)
{
...
void wxPdfFontSubsetCff::WriteLocalSub(int dictNum, wxPdfCffDictionary * privateDict, wxPdfCffIndexArray * localSubrIndex)
{
...
void wxPdfFontSubsetCff::SubsetSubrs(wxPdfCffIndexArray & subrIndex, wxPdfSortedArrayInt & subrsUsed)
{
...
void wxPdfFontSubsetCff::FindSubrsUsed(int fd, wxPdfCffIndexArray & localSubrIndex,
