...
void wxPdfFontDataType0::CreateDefaultEncodingConv()
{
    if (m_conv == NULL)
    {
        if (m_enc.Length() > 0)
...
void wxPdfFontDataType0::SetHalfWidthRanges(bool hwRange, wxUniChar hwFirst, wxUniChar hwLast)
{
    m_hwRange = hwRange;
    m_hwFirst = hwFirst;
    m_hwLast  = hwLast;
...
bool wxPdfFontDataType0::LoadFontMetrics(wxXmlNode * root)
{
    bool bName     = false,
         bDesc     = false,
         bRegistry = false,
...
wxString wxPdfFontDataType0::GetWidthsAsString(bool subset, wxPdfSortedArrayInt * usedGlyphs, wxPdfChar2GlyphMap * subsetGlyphs) const
{
    wxUnusedVar(subset);
...
double wxPdfFontDataType0::GetStringWidth(const wxString & s, const wxPdfEncoding * encoding, bool withKerning, double charSpacing) const
{
...
bool wxPdfFontDataType0::CanShow(const wxString & s, const wxPdfEncoding * encoding) const
{
    wxUnusedVar(encoding);
    bool canShow = true;
#if wxUSE_UNICODE
...
wxString wxPdfFontDataType0::ConvertCID2GID(const wxString & s,
