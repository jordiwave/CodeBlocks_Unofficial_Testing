...
void wxPdfDocument::SaveGraphicState()
{
...
void wxPdfDocument::RestoreGraphicState()
{
...
void wxPdfDocument::ClearGraphicState()
{
    wxPdfGraphicState * state = NULL;
...
bool wxPdfDocument::SelectFont(const wxString & family, const wxString & style, double size, bool setFont)
{
...
bool wxPdfDocument::SelectFont(const wxString & family, int style, double size, bool setFont)
{
...
bool wxPdfDocument::SelectFont(const wxFont & font, bool setFont)
{
#if wxUSE_UNICODE
    int styles = wxPDF_FONTSTYLE_REGULAR;
...
bool wxPdfDocument::SelectFont(const wxPdfFont & font, int style, double size, bool setFont)
{
    bool selected = false;
...
void wxPdfDocument::ForceCurrentFont()
{
    if (m_currentFont != NULL)
    {
...
wxString wxPdfDocument::ApplyVisualOrdering(const wxString & txt)
{
    wxString s;
...
void wxPdfDocument::BeginPage(int orientation, wxSize pageSize)
{
    m_page++;
...
void wxPdfDocument::PutHeader()
{
    OutAscii(wxString(wxS("%PDF-")) + m_PDFVersion);
    Out("%\xE2\xE3\xCF\xD3");
}

void wxPdfDocument::PutTrailer()
{
...
void wxPdfDocument::NewObj(int objId)
{
    // Begin a new object
...
void wxPdfDocument::PutFormFields()
{
...
void wxPdfDocument::PutCatalog()
{
    ...
    if (m_layoutMode == wxPDF_LAYOUT_SINGLE)
    {
        ...
        if (m_layoutMode == wxPDF_LAYOUT_CONTINUOUS)
        {
            Out("/PageLayout /OneColumn");
        }
        else
            if (m_layoutMode == wxPDF_LAYOUT_TWO)
            {
...
static size_t * makeFail(const char * target, size_t tlen)
{
    size_t t = 0;
    size_t s, m;
    m = tlen;
...
static size_t findString(const char * src, size_t slen, const char * target, size_t tlen, size_t * f)
{
    size_t s = 0;
    size_t i;
    size_t m = tlen;
...
void wxPdfDocument::ReplaceNbPagesAlias()
{
...
void wxPdfDocument::PutExtGStates()
{
...
void wxPdfDocument::PutFonts()
{
    ...
    for (fontIter = m_fonts->begin(); fontIter != m_fonts->end(); fontIter++)
    {
        ...
        if (type == wxS("core"))
        {
            ...
            if (type == wxS("Type1") || type == wxS("TrueType"))
            {
                ...
                if (type == wxS("TrueTypeUnicode") || type == wxS("OpenTypeUnicode"))
                {
                    // Type0 Font
...
void wxPdfDocument::PutTemplates()
{
    wxString filter = (m_compress) ? wxS("/Filter /FlateDecode ") : wxS("");
...
void wxPdfDocument::PutImportedObjects()
{
...
void wxPdfDocument::PutXObjectDict()
{
    wxPdfImageHashMap::iterator image = m_images->begin();
...
void wxPdfDocument::PutResourceDict()
{
    Out("/ProcSet [/PDF /Text /ImageB /ImageC /ImageI]");
    Out("/Font <<");
...
void wxPdfDocument::PutBookmarks()
{
...
void wxPdfDocument::PutEncryption()
{
    Out("/Filter /Standard");
...
void wxPdfDocument::PutSpotColours()
{
...
void wxPdfDocument::InitPatternIds()
{
...
void wxPdfDocument::PutPatterns()
{
    wxPdfPatternMap::iterator patternIter = m_patterns->begin();
...
void wxPdfDocument::PutJavaScript()
{
    if (m_javascript.Length() > 0)
    {
        NewObj();
...
void wxPdfDocument::PutColourProfile()
{
...
static wxXmlNode * AddXmpDescription(const wxString & alias, const wxString & ns)
{
...
static wxXmlNode * AddXmpSimple(const wxString & tag, const wxString & value)
{
...
static wxXmlNode * AddXmpSeq(const wxString & tag, const wxString & value)
{
    wxXmlNode * tagNode = new wxXmlNode(wxXML_ELEMENT_NODE, tag);
...
static wxXmlNode * AddXmpAlt(const wxString & tag, const wxString & value)
{
    wxXmlNode * tagNode = new wxXmlNode(wxXML_ELEMENT_NODE, tag);
...
void wxPdfDocument::PutMetaData()
{
    wxXmlDocument xmp;
...
void wxPdfDocument::PutResources()
{
    PutExtGStates();
    PutShaders();
    PutFonts();
...
wxString wxPdfDocument::DoDecoration(double x, double y, const wxString & txt)
{
    // Decorate text
...
void wxPdfDocument::TextEscape(const wxString & s, bool newline)
{
    if (m_currentFont != NULL)
    {
...
size_t wxPdfDocument::CalculateStreamLength(size_t length)
{
...
void wxPdfDocument::OutEscape(const char * s, size_t len)
{
    size_t j;
...
void wxPdfDocument::OutTextstring(const wxString & s, bool newline)
{
...
void wxPdfDocument::OutRawTextstring(const wxString & s, bool newline)
{
    // Format a text string
    size_t ofs = CalculateStreamOffset();
    size_t len = s.Length();;
...
void wxPdfDocument::OutHexTextstring(const wxString & s, bool newline)
{
...
void wxPdfDocument::OutAsciiTextstring(const wxString & s, bool newline)
{
...
void wxPdfDocument::OutAscii(const wxString & s, bool newline)
{
    // Add a line of ASCII text to the document
    Out((const char *) s.ToAscii(), newline);
}

void wxPdfDocument::Out(const char * s, bool newline)
{
    size_t len = strlen(s);
    Out(s, len, newline);
}

void wxPdfDocument::Out(const char * s, size_t len, bool newline)
{
    if (m_state == 2)
    {
        if (!m_inTemplate)
...
void wxPdfDocument::OutPoint(double x, double y)
{
...
void wxPdfDocument::OutPointRelative(double dx, double dy)
{
    m_x += dx;
    m_y += dy;
...
void wxPdfDocument::OutLine(double x, double y)
{
...
void wxPdfDocument::OutLineRelative(double dx, double dy)
{
    m_x += dx;
    m_y += dy;
    // Draws a line from last draw point
...
void wxPdfDocument::OutCurve(double x1, double y1, double x2, double y2, double x3, double y3)
{
...
void wxPdfDocument::SetFillGradient(double x, double y, double w, double h, int gradient)
{
...
wxString wxPdfDocument::MakeFontKey(const wxString & fontName, const wxString & fontEncoding)
{
    wxString fontKey;
