...
wxMemoryOutputStream * wxPdfParser::DecodePredictor(wxMemoryOutputStream * osIn, wxPdfObject * dicPar)
{
...
bool wxPdfLzwDecoder::Decode(wxMemoryInputStream * dataIn, wxMemoryOutputStream * dataOut)
{
    m_dataOut  = dataOut;
    m_dataIn   = dataIn;
...
void wxPdfLzwDecoder::AddStringToTable(int oldCode, char newString)
{
    size_t j;
    ...
    if (m_tableIndex == 511)
    {
        ...
        if (m_tableIndex == 1023)
        {
            m_bitsToGet = 11;
        }
        else
            if (m_tableIndex == 2047)
            {
                m_bitsToGet = 12;
            }
}
