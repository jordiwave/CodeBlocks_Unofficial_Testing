...
int wxPdfDocument::SetAlpha(double lineAlpha, double fillAlpha, wxPdfBlendMode blendMode)
{
    int n = 0;
...
void wxPdfDocument::SetAlphaState(int alphaState)
{
...
void wxPdfShape::MoveTo(double x, double y)
{
...
void wxPdfShape::LineTo(double x, double y)
{
    if (m_subpath >= 0)
    {
...
void wxPdfShape::CurveTo(double x1, double y1, double x2, double y2, double x3, double y3)
{
    if (m_subpath >= 0)
    {
...
wxPdfSegmentType wxPdfShape::GetSegment(int iterType, int iterPoints, double coords[]) const
{
...
static void SubdivideCubicCurve(double src[], int srcOff,
...
static double GetFlatnessSq(double x1, double y1, double cx1, double cy1,
...
void wxPdfDocument::ShapedText(const wxPdfShape & shape, const wxString & text, wxPdfShapedTextMode mode)
{
...
void wxPdfDocument::SetFillingRule(int rule)
{
...
int wxPdfDocument::GetFillingRule()
{
    return m_fillRule;
}

void wxPdfDocument::Line(double x1, double y1, double x2, double y2)
{
    // Draw a line
...
void wxPdfDocument::Rect(double x, double y, double w, double h, int style)
{
    wxString op;
...
void wxPdfDocument::RoundedRect(double x, double y, double w, double h,
...
void wxPdfDocument::Curve(double x0, double y0, double x1, double y1,
...
void wxPdfDocument::Ellipse(double x0, double y0, double rx, double ry,
...
void wxPdfDocument::Circle(double x0, double y0, double r, double astart, double afinish,
                           int style, int nSeg)
{
    Ellipse(x0, y0, r, 0, 0, astart, afinish, style, nSeg);
}

void wxPdfDocument::Sector(double xc, double yc, double r, double astart, double afinish,
...
void wxPdfDocument::Polygon(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y, int style)
{
...
void wxPdfDocument::RegularPolygon(double x0, double y0, double r, int ns, double angle, bool circle, int style,
...
void wxPdfDocument::StarPolygon(double x0, double y0, double r, int nv, int ng, double angle, bool circle, int style,
                                int circleStyle, const wxPdfLineStyle & circleLineStyle, const wxPdfColour & circleFillColour)
{
    if (nv < 2)
    {
...
static void SolveTridiagonalSpecial(const wxPdfArrayDouble & r, wxPdfArrayDouble & x)
{
    size_t i;
    size_t n = r.GetCount();
    x.SetCount(n);
...
static bool GetBezierControlPoints(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y,
...
void wxPdfDocument::BezierSpline(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y, int style)
{
    size_t n = x.GetCount();
...
static bool SolveTridiagonalGeneral(const wxPdfArrayDouble & a, const wxPdfArrayDouble & b,
...
static bool SolveCyclic(const wxPdfArrayDouble & a, const wxPdfArrayDouble & b,
...
static bool GetCyclicControlPoints(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y,
                                   wxPdfArrayDouble & x1, wxPdfArrayDouble & y1,
                                   wxPdfArrayDouble & x2, wxPdfArrayDouble & y2)
{
    size_t i, j;
...
void wxPdfDocument::ClosedBezierSpline(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y, int style)
{
    size_t n = x.GetCount();
...
void wxPdfDocument::Shape(const wxPdfShape & shape, int style)
{
    wxString op;
    ...
    else
    {
        if ((style & wxPDF_STYLE_MASK) == wxPDF_STYLE_FILLDRAW)
        {
            ...
            if ((style & wxPDF_STYLE_MASK) == (wxPDF_STYLE_DRAWCLOSE | wxPDF_STYLE_FILL))
            {
                // small 'b' means closing the path as well
                op = (m_fillRule == wxODDEVEN_RULE) ? wxS("b*") : wxS("b");
            }
            else
                if ((style & wxPDF_STYLE_MASK) == wxPDF_STYLE_DRAWCLOSE)
                {
                    op = wxS("s"); // small 's' means closing the path as well
                }
                else
...
void wxPdfDocument::ClippingText(double x, double y, const wxString & txt, bool outline)
{
...
void wxPdfDocument::ClippingRect(double x, double y, double w, double h, bool outline)
{
...
void wxPdfDocument::ClippingEllipse(double x, double y, double rx, double ry, bool outline)
{
    wxString op = outline ? wxS("S") : wxS("n");
...
void wxPdfDocument::ClippingPolygon(const wxPdfArrayDouble & x, const wxPdfArrayDouble & y, bool outline)
{
    unsigned int np = (x.GetCount() < y.GetCount()) ? (unsigned int) x.GetCount() : (unsigned int) y.GetCount();
    wxString op = outline ? wxS("S") : wxS("n");
    Out("q");
...
void wxPdfDocument::ClippingPath()
{
    Out("q");
    SaveGraphicState();
}

void wxPdfDocument::MoveTo(double x, double y)
{
    OutPoint(x, y);
}

void wxPdfDocument::LineTo(double x, double y)
{
    OutLine(x, y);
}

void wxPdfDocument::CurveTo(double x1, double y1, double x2, double y2, double x3, double y3)
{
    OutCurve(x1, y1, x2, y2, x3, y3);
}

void wxPdfDocument::EndPath(int style)
{
    wxString op;

    switch (style)
    {
        case wxPDF_STYLE_FILL:
            op = (m_fillRule == wxODDEVEN_RULE) ? wxS("f*") : wxS("f");
            break;

        case wxPDF_STYLE_FILLDRAW:
            op = (m_fillRule == wxODDEVEN_RULE) ? wxS("B*") : wxS("B");
            break;
...
void wxPdfDocument::ClosePath(int style)
{
    wxString op;
...
void wxPdfDocument::ClippingPath(const wxPdfShape & shape, int style)
{
    ClippingPath();
    double scratch[6];
    unsigned int iterType;
...
void wxPdfDocument::UnsetClipping()
{
    Out("Q");
    RestoreGraphicState();
}

void wxPdfDocument::ClippedCell(double w, double h, const wxString & txt,
...
void wxPdfDocument::SetLineStyle(const wxPdfLineStyle & linestyle)
{
...
const wxPdfLineStyle & wxPdfDocument::GetLineStyle()
{
    return m_lineStyle;
}

void wxPdfDocument::StartTransform()
{
...
bool wxPdfDocument::ScaleX(double sx, double x, double y)
{
    return Scale(sx, 100, x, y);
}

bool wxPdfDocument::ScaleY(double sy, double x, double y)
{
    return Scale(100, sy, x, y);
}

bool wxPdfDocument::ScaleXY(double s, double x, double y)
{
    return Scale(s, s, x, y);
}

bool wxPdfDocument::Scale(double sx, double sy, double x, double y)
{
    if (x < 0)
    {
        x = m_x;
...
void wxPdfDocument::MirrorH(double x)
{
    Scale(-100, 100, x);
}

void wxPdfDocument::MirrorV(double y)
{
    Scale(100, -100, -1, y);
}

void wxPdfDocument::TranslateX(double tx)
{
    Translate(tx, 0);
}

void wxPdfDocument::TranslateY(double ty)
{
    Translate(0, ty);
}

void wxPdfDocument::Translate(double tx, double ty)
{
    if (m_inTransform == 0)
    {
        StartTransform();
...
void wxPdfDocument::Rotate(double angle, double x, double y)
{
    if (m_inTransform == 0)
    {
        StartTransform();
...
void wxPdfDocument::Transform(double a, double b, double c, double d, double tx, double ty)
{
    if (m_inTransform == 0)
    {
        StartTransform();
...
bool wxPdfDocument::SkewX(double xAngle, double x, double y)
{
    return Skew(xAngle, 0, x, y);
}

bool wxPdfDocument::SkewY(double yAngle, double x, double y)
{
    return Skew(0, yAngle, x, y);
}

bool wxPdfDocument::Skew(double xAngle, double yAngle, double x, double y)
{
    if (x < 0)
    {
        x = m_x;
...
void wxPdfDocument::StopTransform()
{
...
static bool ColourSpaceOk(const wxPdfColour & col1, const wxPdfColour & col2)
{
    return (col1.GetColourType() != wxPDF_COLOURTYPE_SPOT &&
            col1.GetColourType() == col2.GetColourType());
}

int wxPdfDocument::LinearGradient(const wxPdfColour & col1, const wxPdfColour & col2,
...
int wxPdfDocument::AxialGradient(const wxPdfColour & col1, const wxPdfColour & col2,
...
int wxPdfDocument::MidAxialGradient(const wxPdfColour & col1, const wxPdfColour & col2,
                                    double x1, double y1, double x2, double y2,
...
int wxPdfDocument::RadialGradient(const wxPdfColour & col1, const wxPdfColour & col2,
...
int wxPdfDocument::CoonsPatchGradient(const wxPdfCoonsPatchMesh & mesh, double minCoord, double maxCoord)
{
    int n = 0;
...
void wxPdfDocument::Marker(double x, double y, wxPdfMarker markerType, double size)
{
...
void wxPdfDocument::Arrow(double x1, double y1, double x2, double y2, double linewidth, double height, double width)
{
    double saveLineWidth = m_lineWidth;
    double dx = x2 - x1;
    double dy = y2 - y1;
