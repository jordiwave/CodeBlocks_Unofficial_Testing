 ...
 *
 * $Revision: 8649 $
 * $Id: compilerLCC.cpp 8649 2012-12-12 19:18:18Z mortenmacfly $
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/plugins/compilergcc/compilerLCC.cpp $
 */
...
AutoDetectResult CompilerLCC::AutoDetectInstallationDir()
{
    ...
    if (wxFileExists(mpHKLM     + compiler))
    {
        ...
        if (wxFileExists(mpHKCU     + compiler))
        {
            m_MasterPath = mpHKCU;
        }
        else
            if (wxFileExists(mpLccRoot  + compiler))
            {
                m_MasterPath = mpLccRoot;
            }
            else
                if (wxFileExists(mpLccLnk   + compiler))
                {
                    m_MasterPath = mpLccLnk;
                }
                else
                    if (wxFileExists(mpCompiler + compiler))
                    {
