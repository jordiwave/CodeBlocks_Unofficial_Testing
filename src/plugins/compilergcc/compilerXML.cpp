...
AutoDetectResult CompilerXML::AutoDetectInstallationDir()
{
    ...
    while (node)
    {
        if (node->GetName() == wxT("if") && node->GetChildren())
        {
            ...
            if (node->GetName() == wxT("Path") && node->GetChildren())
            {
                ...
                if (node->GetName() == wxT("Search") && sm != none)
                {
                    wxString value;
                    ...
                    if (node->GetAttribute(wxT("envVar"), &value))
                    {
                        ...
                        if (!pathValues.IsEmpty())
                        {
                            ...
                            for (size_t i = 0; i < pathArray.GetCount(); ++i)
                            {
                                if ((targ.IsEmpty() && wxDirExists(pathArray[i])) || wxFileExists(pathArray[i] + wxFILE_SEP_PATH + targ))
                                {
                                    ...
                                    if (sm == master && ((targ.IsEmpty() && wxDirExists(value + wxFILE_SEP_PATH + wxT("bin")))
                        ...
                        if (node->GetAttribute(wxT("path"), &value))
                        {
                            wxString targ = GetExecName(node->GetAttribute(wxT("for"), wxEmptyString));
                            ...
                            if (wxIsWild(value))
                            {
                                ...
                                if ((targ.IsEmpty() && wxDirExists(value)) || wxFileExists(value + wxFILE_SEP_PATH + targ))
                                {
                                    ...
                                    if (sm == master && ((targ.IsEmpty() && wxDirExists(value + wxFILE_SEP_PATH + wxT("bin"))) || wxFileExists(value + wxFILE_SEP_PATH + wxT("bin") + wxFILE_SEP_PATH + targ)))
                                    {
                                        AddPath(value + wxFILE_SEP_PATH + wxT("bin"), sm);
                                    }
                        }
                        else
                            if (node->GetAttribute(wxT("file"), &value))
                            {
                                ...
                                if (node->GetAttribute(wxT("registry"), &value))
                                {
                                    wxRegKey key;
                                    wxString dir;
                                    key.SetName(value);
                    ...
                    if (node->GetName() == wxT("Add"))
                    {
                        ...
                        if (node->GetAttribute(wxT("cFlag"), &value))
                        {
                            ...
                            if (node->GetAttribute(wxT("lFlag"), &value))
                            {
                                AddLinkerOption(value);
                            }
                            else
                                if (node->GetAttribute(wxT("lib"), &value))
                                {
                                    AddLinkLib(value);
                                }
                                else
                                    if (sm != none)
                                    {
                                        ...
                                        while (child)
                                        {
                                            if (child->GetType() == wxXML_TEXT_NODE || child->GetType() == wxXML_CDATA_SECTION_NODE)
                                            {
                                                ...
                                                if (child->GetName() == wxT("master"))
                                                {
                                                    path << m_MasterPath;
                                                }
                                                else
                                                    if (child->GetName() == wxT("separator"))
                                                    {
                                                        path << wxFILE_SEP_PATH;
                                                    }
                                                    else
                                                        if (child->GetName() == wxT("envVar"))
                                                        {
                        ...
                        if (node->GetName() == wxT("Fallback") && sm != none)
                        {
