    ...
    if (*exp->_p == SQREX_SYMBOL_ESCAPE_CHAR)
    {
        ...
        switch (*exp->_p)
        {
            ...
            case 'm':
            {
                ...
                if ((!cb) || (!ce))
                {
                    sqstd_rex_error(exp, _SC("balanced chars expected"));
                }

                if (cb == ce)
                {
                    sqstd_rex_error(exp, _SC("open/close char can't be the same"));
                }
...
static SQInteger sqstd_rex_class(SQRex * exp)
{
    ...
    {
        ret = sqstd_rex_newnode(exp, OP_CLASS);
    }

    if (*exp->_p == ']')
    {
        sqstd_rex_error(exp, _SC("empty class"));
    }
    ...
    while (*exp->_p != ']' && exp->_p != exp->_eol)
    {
        if (*exp->_p == '-' && first != -1)
        {
            ...
            if (*exp->_p++ == ']')
            {
                sqstd_rex_error(exp, _SC("unfinished range"));
            }

            r = sqstd_rex_newnode(exp, OP_RANGE);

            if (exp->_nodes[first].type > *exp->_p)
            {
                sqstd_rex_error(exp, _SC("invalid range"));
            }

            if (exp->_nodes[first].type == OP_CCLASS)
            {
                sqstd_rex_error(exp, _SC("cannot use character classes in ranges"));
            }
...
static SQInteger sqstd_rex_parsenumber(SQRex * exp)
{
    ...
    while (isdigit(*exp->_p))
    {
        ...
        {
            sqstd_rex_error(exp, _SC("overflow in numeric constant"));
        }
...
static SQInteger sqstd_rex_element(SQRex * exp)
{
    ...
    switch (*exp->_p)
    {
        ...
        case '{':
            ...
            if (!isdigit(*exp->_p))
            {
                sqstd_rex_error(exp, _SC("number expected"));
            }
...
static SQBool sqstd_rex_matchclass(SQRex * exp, SQRexNode * node, SQChar c)
{
    do
    {
        switch (node->type)
        {
            ...
            case OP_CCLASS:
                if (sqstd_rex_matchcclass(node->left, c))
                {
                    return SQTrue;
                }
...
static const SQChar * sqstd_rex_matchnode(SQRex * exp, SQRexNode * node, const SQChar * str, SQRexNode * next)
{
    ...
    switch (type)
    {
                            ...
                            (greedystop->type == OP_GREEDY && ((greedystop->right >> 16) & 0x0000FFFF) != 0))
                    {
                        ...
                        if (stop)
                        {
                            ...
                            if (p0 == p1 && p0 == nmaches)
                            {
                                ...
                                if (nmaches >= p0 && p1 == 0xFFFF)
                                {
                                    break;
                                }
                                else
                                    if (nmaches >= p0 && nmaches <= p1)
                                    {
                                        break;
                                    }
                        }
        ...
        case OP_MB:
        {
            ...
            if (*str != cb)
            {
                return NULL;    // string doesnt start with open char
            }
...
void sqstd_rex_free(SQRex * exp)
{
    if (exp)
    {
        if (exp->_nodes)
        {
            sq_free(exp->_nodes, exp->_nallocated * sizeof(SQRexNode));
        }

        if (exp->_jmpbuf)
        {
            sq_free(exp->_jmpbuf, sizeof(jmp_buf));
        }

        if (exp->_matches)
        {
            sq_free(exp->_matches, exp->_nsubexpr * sizeof(SQRexMatch));
        }
