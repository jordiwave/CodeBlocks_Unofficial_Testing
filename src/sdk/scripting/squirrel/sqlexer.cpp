        ...
        switch (CUR_CHAR)
        {
            ...
            default:
            {
                if (scisdigit(CUR_CHAR))
                {
                    ...
                    if (scisalpha(CUR_CHAR) || CUR_CHAR == _SC('_'))
                    {
                        SQInteger t = ReadID();
                        RETURN_TOKEN(t);
                    }
...
SQInteger SQLexer::ProcessStringHexEscape(SQChar * dest, SQInteger maxdigits)
{
    ...
    {
        Error(_SC("hexadecimal number expected"));
    }
...
SQInteger SQLexer::ReadString(SQInteger ndelim, bool verbatim)
{
        ...
        while (CUR_CHAR != ndelim)
        {
            ...
            switch (x)
            {
                ...
                case _SC('\n'):
                    if (!verbatim)
                    {
                        Error(_SC("newline in a constant"));
                    }
    ...
    if (ndelim == _SC('\''))
    {
        if (len == 0)
        {
            Error(_SC("empty constant"));
        }

        if (len > 1)
        {
            Error(_SC("constant too long"));
        }
...
#define TOCTAL 5
    ...
    if (firstchar == _SC('0') && (toupper(CUR_CHAR) == _SC('X') || scisodigit(CUR_CHAR)))
    {
        if (scisodigit(CUR_CHAR))
        {
            ...
            if (scisdigit(CUR_CHAR))
            {
                Error(_SC("invalid octal number"));
            }
        }
        else
        {
            ...
            {
                Error(_SC("too many digits for an Hex number"));
            }
        }
    }
    else
    {
        ...
        {
            if (CUR_CHAR == _SC('.') || isexponent(CUR_CHAR))
            {
                type = TFLOAT;
            }
