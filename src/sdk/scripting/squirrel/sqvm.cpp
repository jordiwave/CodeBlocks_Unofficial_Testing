    ...
    switch (tmask)
    {
        ...
        default:
            if (op == '+' && (tmask & _RT_STRING))
            {
                ...
                if (!ArithMetaMethod(op, o1, o2, trg))
                {
                    return false;
                }
    }
        ...
        case OT_INSTANCE:
            if (_delegable(o)->_delegate)
            {
                ...
                if (_delegable(o)->GetMetaMethod(this, MT_UNM, closure))
                {
                    ...
                    if (!CallMetaMethod(closure, MT_UNM, 1, temp_reg))
                    {
                        return false;
                    }
        ...
        if (!res && sq_type(o1) == OT_INSTANCE)
        {
            if (_delegable(o1)->_delegate)
            {
                ...
                if (_delegable(o1)->GetMetaMethod(nullptr, MT_CMP, closure))
                {
                    ...
                    {
                        if (sq_type(cmpResult) != OT_INTEGER)
                        {
                            Raise_Error(_SC("_cmp must return an integer"));
                            return false;
                        }
            ...
            switch (_i_.op)
            {
                    ...
                    switch (sq_type(clo))
                    {
                        ...
                        case OT_INSTANCE:
                        {
                            ...
                            if (_delegable(clo)->_delegate && _delegable(clo)->GetMetaMethod(this, MT_CALL, closure))
                            {
                                ...
                                if (!CallMetaMethod(closure, MT_CALL, arg3 + 1, clo))
                                {
                                    SQ_THROW();
                                }
                ...
                case _OP_YIELD:
                {
                    if (ci->_generator)
                    {
                        ...
                        if (sarg1 != MAX_FUNC_STACKSIZE)
                        {
                            _Swap(STK(arg1), temp_reg);    //STK(arg1) = temp_reg;
                        }
                    }
                    else
                    {
...
exception_trap:
    ...
    assert(0);
    return false; // keep compiler happy, but will never get here due to assert(0)!!!!
}
...
SQInteger SQVM::FallBackGet(const SQObjectPtr & self, const SQObjectPtr & key, SQObjectPtr & dest)
{
    switch (sq_type(self))
    {
        ...
        case OT_USERDATA:
            ...
            {
                if (Get(SQObjectPtr(_delegable(self)->_delegate), key, dest, 0, DONT_FALL_BACK))
                {
                    return FALLBACK_OK;
                }
            }
...
void SQVM::dumpstack(SQInteger stackbase, bool dumpall)
{
    ...
    for (SQInteger i = 0; i < size; i++)
    {
        ...
        switch (sq_type(obj))
        {
            ...
            case OT_INTEGER:
                scprintf(_SC("INTEGER %d"), _integer(obj));
                break;
