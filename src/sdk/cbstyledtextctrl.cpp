 ...
 *
 * $Revision: 12401 $
 * $Id: cbstyledtextctrl.cpp 12401 2021-05-09 12:48:48Z fuscated $
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/sdk/cbstyledtextctrl.cpp $
 */
        ...
        case WXK_BACK:
        {
            if (m_tabSmartJump)
            {
                if (!(event.ControlDown() || event.ShiftDown() || event.AltDown()))
                {
                    ...
                    if (m_lastPosition != wxSCI_INVALID_POSITION && event.ControlDown())
                    {
...
void cbStyledTextCtrl::DoBraceCompletion(const wxChar & ch)
{
    ...
    if (ch == wxT('\'') || ch == wxT('"'))
    {
        if (GetCharAt(pos) == ch)
        {
            ...
            if (!IsString(GetStyleAt(pos - 2)) && !IsCharacter(GetStyleAt(pos - 2)))
            {
                InsertText(pos, ch);
            }
