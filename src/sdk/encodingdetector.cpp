    ...
    public:
        ...
        bool DetectEncodingEx(const wxByte * buffer, size_t size)
        {
            ...
            if (size >= 2)
            {
                if (size >= 4 && memcmp(buffer, "\xFF\xFE\x00\x00", 4) == 0)
                {
                    ...
                    if (size >= 4 && memcmp(buffer, "\xFE\xFF\x00\x00", 4) == 0)
                    {
                        ...
                        if (size >= 4 && memcmp(buffer, "\x00\x00\xFE\xFF", 4) == 0)
                        {
                            ...
                            if (size >= 4 && memcmp(buffer, "\x00\x00\xFF\xFE", 4) == 0)
                            {
                                ...
                                if (memcmp(buffer, "\xFF\xFE", 2) == 0)
                                {
                                    ...
                                    if (memcmp(buffer, "\xFE\xFF", 2) == 0)
                                    {
                                        ...
                                        if (size >= 3 && memcmp(buffer, "\xEF\xBB\xBF", 3) == 0)
                                        {
                                            ...
                                            if (size >= 5 && memcmp(buffer, "\x2B\x2F\x76\x38\x2D", 5) == 0)
                                            {
            ...
            {
                if (size >= 4 && memcmp(buffer, "\x3C\x00\x00\x00", 4) == 0)
                {
                    enc = wxFONTENCODING_UTF32LE;
                }
                else
                    if (size >= 4 && memcmp(buffer, "\x00\x00\x00\x3C", 4) == 0)
                    {
                        enc = wxFONTENCODING_UTF32BE;
                    }
                    else
                        if (memcmp(buffer, "\x3C\x00",         2) == 0)
                        {
                            enc = wxFONTENCODING_UTF16LE;
                        }
                        else
                            if (memcmp(buffer, "\x00\x3C",         2) == 0)
                            {
                                enc = wxFONTENCODING_UTF16BE;
                            }
            }
            ...
            if (enc == wxFONTENCODING_DEFAULT)
            {
                ...
                while (buff_ptr != buff_end)
                {
                    ...
                    {
                        if (*((wxUint32 *)buff_ptr) == 0)
                        {
                            ++bad_utf32_count;
                        }

                        if (*((wxUint32 *)buff_ptr) == wxUINT32_SWAP_ON_BE(0x0A))
                        {
                            ++nl_utf32le_count;
                        }

                        if (*((wxUint32 *)buff_ptr) == wxUINT32_SWAP_ON_LE(0x0A))
                        {
                            ++nl_utf32be_count;
                        }
                    }
                    ...
                    {
                        if (*((wxUint16 *)buff_ptr) == 0)
                        {
                            ++bad_utf16_count;
                        }

                        if (*((wxUint16 *)buff_ptr) == wxUINT16_SWAP_ON_BE(0x0A))
                        {
                            ++nl_utf16le_count;
                        }

                        if (*((wxUint16 *)buff_ptr) == wxUINT16_SWAP_ON_LE(0x0A))
                        {
                            ++nl_utf16be_count;
                        }
                    }
                ...
                if (bad_utf_count   == 0)
                {
                    ...
                    if (bad_utf32_count == 0 && nl_utf32le_count > size / 400)
                    {
                        enc = wxFONTENCODING_UTF32LE;
                    }
                    else
                        if (bad_utf32_count == 0 && nl_utf32be_count > size / 400)
                        {
                            enc = wxFONTENCODING_UTF32BE;
                        }
                        else
                            if (bad_utf16_count == 0 && nl_utf16le_count > size / 200)
                            {
                                enc = wxFONTENCODING_UTF16LE;
                            }
                            else
                                if (bad_utf16_count == 0 && nl_utf16be_count > size / 200)
                                {
                                    enc = wxFONTENCODING_UTF16BE;
                                }
                                else
        ...
        void Report(const char * aCharset) override
        {
            ...
            if (m_MozillaResult == _T("gb18030")) // hack, because wxWidgets only knows cp936
            {
                ...
                if (m_MozillaResult.Contains(wxT("*ASCII*"))) // remove our "specials"
                {
                    m_MozillaResult = wxEmptyString;
                }
        }
        ...
        bool ConvertToWxString(const wxByte * buffer, size_t size)
        {
            ...
            if (m_Encoding == wxFONTENCODING_UTF7)
            {
                ...
                if (m_Encoding == wxFONTENCODING_UTF8)
                {
                    ...
                    if (m_Encoding == wxFONTENCODING_UTF16BE)
                    {
                        wxMBConvUTF16BE conv;
                        wideBuff = conv.cMB2WC((const char *)buffer, size + 4 - m_BOMSizeInBytes, &outlen);
                    }
                    else
                        if (m_Encoding == wxFONTENCODING_UTF16LE)
                        {
                            wxMBConvUTF16LE conv;
                            wideBuff = conv.cMB2WC((const char *)buffer, size + 4 - m_BOMSizeInBytes, &outlen);
                        }
                        else
                            if (m_Encoding == wxFONTENCODING_UTF32BE)
                            {
                                wxMBConvUTF32BE conv;
                                wideBuff = conv.cMB2WC((const char *)buffer, size + 4 - m_BOMSizeInBytes, &outlen);
                            }
                            else
                                if (m_Encoding == wxFONTENCODING_UTF32LE)
                                {
