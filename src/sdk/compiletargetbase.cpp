 ...
 * $HeadURL: https://svn.code.sf.net/p/codeblocks/code/trunk/src/sdk/compiletargetbase.cpp $
 */
...
void CompileTargetBase::SetImportLibraryFilename(const wxString & filename)
{
    if (filename.IsEmpty())
    {
        ...
        if (m_ImportLibraryFilename == filename)
        {
            return;
        }
...
void CompileTargetBase::SetDefinitionFileFilename(const wxString & filename)
{
    if (filename.IsEmpty())
    {
        ...
        if (m_DefinitionFileFilename == filename)
        {
            return;
        }
