...
{
    public:
        wxSCITimer(ScintillaWX * swx, ScintillaWX::TickReason reason)
        {
            m_swx = swx;
            m_reason = reason;
        }
...
#error "Must define rectangular text selection clipboard format."
    ...
    timers[tickCaret] = new wxSCITimer(this, tickCaret);
    timers[tickScroll] = new wxSCITimer(this, tickScroll);
    timers[tickWiden] = new wxSCITimer(this, tickWiden);
    timers[tickDwell] = new wxSCITimer(this, tickDwell);
    m_surfaceData = NULL;
}
...
void ScintillaWX::SetMouseCapture(bool on)
{
    if (mouseDownCaptures)
    {
        if (on && !capturedMouse)
        {
            ...
            if (!on && capturedMouse && stc->HasCapture())
            {
                stc->ReleaseMouse();
            }
...
void ScintillaWX::DoHScroll(int type, int pos)
{
    ...
    if (type == wxEVT_SCROLLWIN_LINEUP || type == wxEVT_SCROLL_LINEUP)
    {
        ...
        if (type == wxEVT_SCROLLWIN_LINEDOWN || type == wxEVT_SCROLL_LINEDOWN)
        {
            xPos += H_SCROLL_STEP;
        }
        else
            if (type == wxEVT_SCROLLWIN_PAGEUP || type == wxEVT_SCROLL_PAGEUP)
            {
                xPos -= pageWidth;
            }
            else
                if (type == wxEVT_SCROLLWIN_PAGEDOWN || type == wxEVT_SCROLL_PAGEDOWN)
                {
                    xPos += pageWidth;
                    ...
                    if (type == wxEVT_SCROLLWIN_TOP || type == wxEVT_SCROLL_TOP)
                    {
                        xPos = 0;
                    }
                    else
                        if (type == wxEVT_SCROLLWIN_BOTTOM || type == wxEVT_SCROLL_BOTTOM)
                        {
                            xPos = scrollWidth;
                        }
                        else
                            if (type == wxEVT_SCROLLWIN_THUMBTRACK || type == wxEVT_SCROLL_THUMBTRACK)
                            {
                                xPos = pos;
                            }
...
void ScintillaWX::DoMarkerDefineBitmap(int markerNumber, const wxBitmap & bmp)
{
    if (0 <= markerNumber && markerNumber <= MARKER_MAX)
    {
        ...
        wxScopedArray<unsigned char> rgba(new unsigned char[4 * bmp.GetWidth()*bmp.GetHeight()]);
#else
        std::unique_ptr<unsigned char[]> rgba(new unsigned char[4 * bmp.GetWidth()*bmp.GetHeight()]);
