...
void MacrosManager::RecalcVars(const cbProject * project, EditorBase * editor, const ProjectBuildTarget * target)
{
    ...
    if (!project)
    {
        ...
        if ((project != m_LastProject) || (project->GetTitle() != m_ProjectName)
    ...
    if (!target)
    {
        ...
        if ((target != m_LastTarget) or (target->GetTitle() != m_TargetName))
        {
                                  ...
                                  bool subrequest)
{
    ...
    while (m_RE_RemoveQuotes.Matches(buffer))
    {
        ...
        {
            if (content.IsSameAs("\"\"") || content.IsSameAs("''") || content.empty())
            {
                buffer.Replace(search, "", false);
            }
            else
