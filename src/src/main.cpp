 ...
 *
 * $Revision: 12751 $
 * $Id: main.cpp 12751 2022-03-14 17:20:31Z wh11204 $
...
void MainFrame::OnStartHereLink(wxCommandEvent & event)
{
    ...
    if (link.IsSameAs(_T("CB_CMD_NEW_PROJECT")))
    {
        ...
        //        OnSettingsCompilerDebugger(evt);
            ...
            if (link.StartsWith(_T("CB_CMD_OPEN_HISTORY_")))
            {
                ...
                if (link.StartsWith(_T("CB_CMD_DELETE_HISTORY_")))
                {
                    RecentItemsList * recent;
                    ...
                    if (link.IsSameAs(_T("CB_CMD_TIP_OF_THE_DAY")))
                    {
                        ShowTips(true);
                    }
}
...
void MainFrame::OnEditEOLMode(wxCommandEvent & event)
{
    ...
    if (ed)
    {
        ...
        if (event.GetId() == idEditEOLCRLF)
        {
            ...
            if (event.GetId() == idEditEOLCR)
            {
                mode = wxSCI_EOL_CR;
            }
            else
                if (event.GetId() == idEditEOLLF)
                {
                    mode = wxSCI_EOL_LF;
                }
...
void MainFrame::OnEditEncoding(wxCommandEvent & event)
{
    ...
    if (event.GetId() == idEditEncodingDefault)
    {
        ...
        if (event.GetId() == idEditEncodingAscii)
        {
            encoding = wxFONTENCODING_ISO8859_1;
        }
        else
            if (event.GetId() == idEditEncodingUtf7)
            {
                encoding = wxFONTENCODING_UTF7;
            }
            else
                if (event.GetId() == idEditEncodingUtf8)
                {
                    encoding = wxFONTENCODING_UTF8;
                }
                else
                    if (event.GetId() == idEditEncodingUtf16)
                    {
                        encoding = wxFONTENCODING_UTF16;
                    }
                    else
                        if (event.GetId() == idEditEncodingUtf32)
                        {
                            encoding = wxFONTENCODING_UTF32;
                        }
                        else
                            if (event.GetId() == idEditEncodingUnicode)
                            {
                                encoding = wxFONTENCODING_UNICODE;
                            }
                            else
                                if (event.GetId() == idEditEncodingUnicode16BE)
                                {
                                    encoding = wxFONTENCODING_UTF16BE;
                                }
                                else
                                    if (event.GetId() == idEditEncodingUnicode16LE)
                                    {
                                        encoding = wxFONTENCODING_UTF16LE;
                                    }
                                    else
                                        if (event.GetId() == idEditEncodingUnicode32BE)
                                        {
                                            encoding = wxFONTENCODING_UTF32BE;
                                        }
                                        else
                                            if (event.GetId() == idEditEncodingUnicode32LE)
                                            {
            ...
            if (id == idFilePrint)
            {
                ...
                else
                {
                    ...
                    if (id == idFileReopenProject)
                    {
                        ...
                        if (id == idFileCloseProject || id == idFileSaveProjectAs || id == idFileSaveProjectTemplate)
                        {
                            event.Enable(project != nullptr);
                        }
                        else
                            if (id == idFileSaveProject)
                            {
                                event.Enable(project && project->GetModified());
                            }
                            else
                                if (id == idFileOpenDefWorkspace || id == idFileSaveWorkspaceAs || id == idFileSaveAll)
                                {
                                    event.Enable(true);
                                }
                                else
        ...
        if (id == idEditRedo)
        {
            ...
            if (id == idEditClearHistory)
            {
                ...
                if (id == idEditCut)
                {
                    ...
                    if (id == idEditCopy || id == idEditSelectNextSkip)
                    {
                        ...
                        if (id == idEditPaste)
                        {
                            ...
                            if (id == idEditSwapHeaderSource || id == idEditGotoMatchingBrace
                                    ...
                                    || id == idEditCompleteCode)
                            {
                                ...
                                if (id == idEditSelectAll)
                                {
                                    ...
                                    if (id == idEditFolding)
                                    {
                                        ...
                                        if (id == idEditSpecialCommandsCase)
                                        {
...
void MainFrame::OnToggleBar(wxCommandEvent & event)
{
    ...
    if (event.GetId() == idViewManager)
    {
        ...
        if (event.GetId() == idViewLogManager)
        {
            win = m_pInfoPane;
        }
        else
            if (event.GetId() == idViewToolMain)
            {
                win = m_pToolbar;
                toolbar = true;
            }
            else
                if (event.GetId() == idViewToolDebugger)
                {
