 ...
 *
 * $Revision: 12079 $
 * $Id: debuggermenu.cpp 12079 2020-05-18 22:43:36Z fuscated $
...
void DebuggerMenuHandler::OnUpdateUI(wxUpdateUIEvent & event)
{
    ...
    if (id == idMenuDebug)
    {
        ...
        if (id == idMenuNext || id == idMenuNextInstr || id == idMenuStepIntoInstr
                || id == idMenuStepOut)
        {
...
void DebuggerMenuHandler::OnStart(cb_unused wxCommandEvent & event)
{
    ...
    if (!m_activeDebugger->IsRunning())
    {
        ...
        if (m_activeDebugger->IsStopped() && !m_disableContinue)
        {
            HideValueTooltip();
