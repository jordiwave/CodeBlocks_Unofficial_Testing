    ...
    mDirPrepend()
{
    ...
    if (!txtCrashLogContentFont.Ok())
    {
        txtCrashLogContentFont = wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
    }
    ...
    if (!txtResultFont.Ok())
    {
        txtResultFont = wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT);
    }
...
void Addr2LineUIDialog::OnCrashLogFile(wxFileDirPickerEvent & event)
{
    ...
    if (file.Open())
    {
        ...
        if (mCrashLogFileContent.Count() > 0)
        {
            btnOperate->Enable();
        }
        else
...
void Addr2LineUIDialog::OnOperateClick(wxCommandEvent & WXUNUSED(event))
{
    ...
    */
    ...
    wxRegEx reAddr32(wxT("([A-Fa-f0-9]{8})"));
    wxString tag32(wxT("AddrPC   Params"));
    size_t len32 = 4 * 8 + 4; // 4 times 8bit  address + 4 times space
    wxRegEx reAddr64(wxT("([A-Fa-f0-9]{16})"));
    wxString tag64(wxT("AddrPC           Params"));
    size_t len64 = 4 * 16 + 4; // 4 times 16bit address + 4 times space
    bool is64Bit = false;
    txtResult->Clear();
    ...
    for (size_t i = 0; i < mCrashLogFileContent.Count(); i++)
    {
        ...
        if (is64Bit)
        {
            for (size_t j = 1; j <= reAddr64.GetMatchCount(); j++)
            {
                switch (j)
                {
                    case 1:
                    {
                        theAddr = reAddr64.GetMatch(line, 1);
                    }
                    break;
        ...
        else // 32 bit
        {
            for (size_t j = 1; j <= reAddr32.GetMatchCount(); j++)
            {
                switch (j)
                {
                    case 1:
                    {
                        theAddr = reAddr32.GetMatch(line, 1);
                    }
                    break;
        ...
        if (theFile.Contains(wxT(" ")))
        {
            theFile = wxT("\"") + theFile + wxT("\"");
        }
