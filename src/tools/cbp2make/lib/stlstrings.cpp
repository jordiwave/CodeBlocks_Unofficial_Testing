    ...
    if (NULL != stream)
    {
        ...
        while (!feof(stream))
        {
            ...
            switch (c)
            {
                ...
                default:
                {
                    if (buf_pos >= buffer.GetLength())
                    {
                        buffer.SetLength(buffer.GetLength() + 0x100);
                    }
...
void CCharset::UpdateCharset(void)
{
    ...
    for (int i = 0; i < m_Alphabet.GetLength(); i++)
    {
        m_Charset[(unsigned char)m_Alphabet[i]] = true;
    }
}
...
{
    if (m_Charset[*(unsigned char *)&AChar])
    {
        return AChar;
    }
    else
...
{
    if (m_Position < (int)m_String->GetLength())
    {
        m_Position++;
    }
...
char & CStringListIterator::Next(void)
{
    ...
    if (m_Position < (int)m_StringList->GetString(m_Line).GetLength())
    {
        m_Position++;
        return This();
    }
...
void CCharHistogram::Analyze(void)
{
        ...
        if (textnum_charset.Isset(char(i)))
        {
            ...
            if (realnum_charset.Isset(char(i)))
            {
                ...
                if (integer_charset.Isset(char(i)))
                {
                    ...
                    if (numeric_charset.Isset(char(i)))
                    {
                        numeric_chars += _CharCount;
                    }
                }
            }
        }
    ...
    if (total_chars > ascii_chars)
    {
        m_Flags |= FLAG_CUSTOM_BINARY;
    }
    else
    {
        // if there are letters other than exponent specifiers
        if (total_chars > textnum_chars)
        {
            m_Flags |= FLAG_ASCII_TEXT;
        }
        else
        {
            ...
            if (numeric_chars > 0)
            {
                if (realnum_chars > integer_chars)
                {
                    m_Flags |= FLAG_PURE_NUMERIC;
                }
                else
                {
                    m_Flags |= FLAG_PURE_INTEGER;
                }
            }
            else
            {
...
{
    if (m_Histogram[(unsigned char)AChar] < MAX_CHAR_FREQUENCY)
    {
        m_Histogram[(unsigned char)AChar]++;
    }
...
void CCharHistogram::Insert(const CString & AString)
{
    for (int i = 0, n = AString.GetLength(); i < n; i++)
    {
        ...
        if (m_Histogram[(unsigned char)c] < MAX_CHAR_FREQUENCY)
        {
            m_Histogram[(unsigned char)c]++;
        }
    }
...
{
    if (m_Histogram[(unsigned char)AChar] > 0)
    {
        m_Histogram[(unsigned char)AChar]--;
    }
...
void CCharHistogram::Remove(const CString & AString)
{
    for (int i = 0, n = AString.GetLength(); i < n; i++)
    {
        ...
        if (m_Histogram[(unsigned char)c] > 0)
        {
            m_Histogram[(unsigned char)c]--;
        }
    }
...
CString StringOfChars(const char AChar, const int Count)
{
    ...
    if (Count > 0)
    {
        result.SetLength(Count).Fill(AChar);
    }
...
CString EnumStr(const CString & Prefix, const int Counter, const CString & Suffix, const unsigned int Length)
{
    ...
    if (Length > length)
    {
        lead_zero.SetLength(Length - length);
        lead_zero.Fill('0');
    }
...
CString SubStr(const CString & AString, const int FirstPos, const int LastPos)
{
    ...
    for (int i = first_pos, j = 0; i <= last_pos; i++, j++)
    {
        result[j] = AString.GetChar(i);
    }
...
CString ShiftStr(const CString & AString, const int Shift)
{
    ...
    if (Shift > 0)
    {
        return SubStr(AString, Shift, AString.GetLength() - 1);
    }

    if (Shift < 0)
        if (AString.GetLength() > (-Shift))
        {
            return SubStr(AString, -Shift, AString.GetLength() - Shift);
        }
...
CString FilterStr(const CString & AString, const CCharset & ValidChars, const CCharset & InvalidChars, const bool ValidateUndefined)
{
    ...
    {
        if (InvalidChars.Isset(result[i]))
        {
            continue;
        }

        if (ValidChars.Isset(result[i]) || ValidateUndefined)
        {
...
{
    for (int i = Position; i < AString.GetLength(); i++)
        if (AChar == AString.GetChar(i))
        {
            return i;
        }
...
void ParseStr(const CString & AString, const CString & Delimiters, CStringList & SubStrings)
{
    ...
    while (str_pos < AString.GetLength())
    {
        ...
        {
            sub_str.Append(AString.GetChar(str_pos));
        }
...
CString CollectStr(const CStringList & SubStrings, const CString & Delimiters)
{
    ...
    do
    {
        ...
        if (sub_str_set)
        {
            result.Append(SubStrings.GetString(sub_strIdx++));
        }

        if (delim_set)
        {
            result.Append(Delimiters.GetChar(delim_pos++));
        }
...
CString StripDupChars(const CString & AString, const char AChar)
{
    ...
    for (int i = 0; i < AString.GetLength(); i++)
    {
        if ((AChar != AString.GetChar(i)) || (AChar != AString.GetChar(i + 1)))
        {
            result[length++] = result[i];
        }
    }
...
CString StripDupChars(const CString & AString)
{
    ...
    for (int i = 0; i < AString.GetLength(); i++)
    {
        if (AString.GetChar(i) != AString.GetChar(i + 1))
        {
            result[length++] = result[i];
        }
    }
...
CString CheckFirstChar(const CString & AString, const char AChar)
{
    ...
    if (AChar != AString.GetFirstChar())
    {
        result = CString(AChar) + AString;
    }
    else
    {
        result = AString;
...
int ParseMemSizeStr(const CString & AString, int & UnitSize, char & UnitChar)
{
    ...
    if (valid_suffix_chars.Isset('k') || valid_suffix_chars.Isset('K'))
    {
        UnitSize = 0x00000400;
        UnitChar = 'k';
    };

    if (valid_suffix_chars.Isset('m') || valid_suffix_chars.Isset('M'))
    {
        UnitSize = 0x00010000;
        UnitChar = 'm';
    };

    if (valid_suffix_chars.Isset('g') || valid_suffix_chars.Isset('G'))
    {
        UnitSize = 0x40000000;
        UnitChar = 'g';
    };
...
long long int ParseMemSizeStr64(const CString & AString, long long int & UnitSize, char & UnitChar)
{
    ...
    if (valid_suffix_chars.Isset('t') || valid_suffix_chars.Isset('T'))
    {
        UnitSize = tb;
        UnitChar = 'T';
    };
...
int GuessStr(const CString & AString, const CStringList & Variants, CString & Guessed, const bool TryFuzzy)
{
    ...
    {
        freq_v.Reset();
        freq_v.Insert(Variants[i]);
        unsigned int match = 0;

        for (int j = 0; j < CHARSET_SIZE; j++)
        {
            match += freq_a.GetAt(j) * freq_v.GetAt(j);
        }
...
CString UnquoteStr(const CString & AString)
{
    if (IsQuoted(AString))
    {
        return SubStr(AString, 1, AString.GetLength() - 2);
    }
    else
    {
        return AString;
