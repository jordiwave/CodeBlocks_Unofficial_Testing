...
{
    if ((Index>=0)&&(Index<GetLength())) return m_String[Index];
    else return 0;
}
...
{
    if (m_String.size()) return m_String[GetLength()-1];
    else return 0;
}
...
{
    if (m_Charset[*(unsigned char *)&AChar]) return AChar;
    else return char(0);
}
...
{
    m_Flags = 0;
    for (int i = 0; i < CHARSET_SIZE; i++) m_Histogram[i] = 0;
}
...
CString TrimStr(const CString& AString)
{
    ...
    {
        if (' '==AString.GetChar(i)) first_pos++;
        else break;
    }
    for (int i = last_pos; i > first_pos; i--)
    {
        if (' '==AString.GetChar(i)) last_pos--;
        else break;
    }
...
{
    CString result;
    if (AChar!=AString.GetFirstChar()) result = CString(AChar) + AString;
    else result = AString;
    return result;
}
...
{
    CString result;
    if (AChar!=AString.GetLastChar()) result = AString + AChar;
    else result = AString;
    return result;
}
