 ...
 *
 * $Revision: 12787 $
 * $Id: mainframe.cpp 12787 2022-04-12 16:10:00Z mortenmacfly $
                          ...
                          std::vector<TiXmlNode *> * nodes, const wxString & prefix)
{
    ...
    if (sectionLower.Matches(wxT("auto_complete")))      // auto complete (abbreviations)
    {
        ...
        if (sectionLower.Matches(wxT("code_completion"))) // code completion plugin token replacements
        {
            TiXmlNode * child = NULL;
            ...
            if (sectionLower.Matches(wxT("compiler")))    // compiler sets
            {
                listbox->Append(wxT("<") + section + wxT(">"));
                ...
                if (sectionLower.Matches(wxT("debugger_common"))) // debugger common options
                {
                    listbox->Append(wxT("<") + section + wxT(">"));
                    ...
                    if (sectionLower.Matches(wxT("editor")))      // editor colour sets
                    {
                        TiXmlNode * child = NULL;
                        ...
                        if (sectionLower.Matches(wxT("envvars")))     // envvar plugin variables
                        {
                            listbox->Append(wxT("<") + section + wxT(">"));
                            nodes->push_back(*node);
                        }
                        else
                            if (sectionLower.Matches(wxT("gcv")))         // global variables
                            {
                                listbox->Append(wxT("<") + section + wxT(">"));
                                nodes->push_back(*node);
                            }
                            else
                                if (sectionLower.Matches(wxT("help_plugin"))) // help plugin files
                                {
                                    listbox->Append(wxT("<") + section + wxT(">"));
                                    nodes->push_back(*node);
                                }
                                else
                                    if (sectionLower.Matches(wxT("mime_types")))  // mime types
                                    {
                                        listbox->Append(wxT("<") + section + wxT(">"));
                                        nodes->push_back(*node);
                                    }
                                    else
                                        if (sectionLower.Matches(wxT("plugins")))     // plugins
                                        {
                                            listbox->Append(wxT("<") + section + wxT(">"));
                                            nodes->push_back(*node);
                                        }
                                        else
                                            if (sectionLower.Matches(wxT("colours")))     // plugins
                                            {
                                                listbox->Append(wxT("<") + section + wxT(">"));
                                                nodes->push_back(*node);
                                            }
                                            else
                                                if (sectionLower.Matches(wxT("project_manager"))) // file groups
                                                {
                                                    TiXmlNode * child = NULL;
                                                    ...
                                                    if (sectionLower.Matches(wxT("tools")))       // tools setup by the user
                                                    {
                                                        listbox->Append(wxT("<") + section + wxT(">"));
                                                        nodes->push_back(*node);
                                                    }
                                                    ...
                                                    // ----------------------------------------------------------
                                                        ...
                                                        if (prefix.Matches(wxT("<code_completion>"))
                                                        ...
                                                        // -----------------------------------------------
                                                            ...
                                                            if (prefix.Matches(wxT("<compiler>"))
                                                                    && sectionLower.Matches(wxT("sets")))     // compiler sets
                                                            {
                                                                ...
                                                                // --------------------------------------------------------
                                                                    ...
                                                                    if (prefix.Matches(wxT("<compiler><sets>")))         // individual compiler sets
                                                                    {
                                                                        listbox->Append(prefix + wxT("<") + section + wxT(">"));
                                                                        nodes->push_back(*node);
                                                                    }
                                                                    ...
                                                                    // -------------------------------------------------------------
                                                                        ...
                                                                        if (prefix.Matches(wxT("<compiler><user_sets>")))    // individual compiler user sets
                                                                        {
                                                                            listbox->Append(prefix + wxT("<") + section + wxT(">"));
                                                                            nodes->push_back(*node);
                                                                        }
                                                                        ...
                                                                        // --------------------------------------------
                                                                            ...
                                                                            if (prefix.Matches(wxT("<debugger_common>"))
                                                                            ...
                                                                            // ---------------------------------------------------------------
                                                                                ...
                                                                                if (prefix.Matches(wxT("<debugger_common><sets>"))) // individual debugger sets
                                                                                {
                                                                                    listbox->Append(prefix + wxT("<") + section + wxT(">"));
                                                                                    nodes->push_back(*node);
                                                                                }
                                                                                ...
                                                                                // ------------------------------------------
                                                                                    ...
                                                                                    // --------------------------------------------------------
                                                                                        ...
                                                                                        if (prefix.Matches(wxT("<editor><colour_sets>"))
                                                                                        ...
                                                                                        // -------------------------------------------
                                                                                            ...
                                                                                            // ---------------------------------------------------
                                                                                                ...
                                                                                                if (prefix.Matches(wxT("<project_manager>"))
