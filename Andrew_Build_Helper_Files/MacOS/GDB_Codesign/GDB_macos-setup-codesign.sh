...
#sudo codesign --entitlements gdb-entitlement.xml -fs gdb_codesign $(which gdb)
#sudo codesign --entitlements gdb-entitlement.xml -fs gdb_codesign /usr/local/Cellar/gdb/11.2/bin/gdb
sudo codesign --entitlements gdb-entitlement.xml -fs ${CERT} $(which gdb)
[ $? -eq 0 ] || error Something went wrong when Codesigning GDB
