...
@rem ----------------------------------------------------------------------------
@rem Ask the user if they want to run the update31_%BUILD_BITS%.bat
@rem ----------------------------------------------------------------------------
:QuestionsStart
...
:QuestionUpdateStart
@echo.
set /p UserInput=Do you want to run the update31_%BUILD_BITS%.bat automatically after the compilation finishes [Y/N]?
if /I "%UserInput%" EQU "Y" @echo You have chosen to run the update31_%BUILD_BITS%.bat.
if /I "%UserInput%" NEQ "Y" @echo You have chosen NOT to run the update31_%BUILD_BITS%.bat.
@echo.
@echo.
...
@rem ----------------------------------------------------------------------------
@rem Run the update31_%BUILD_BITS%.bat if the user wanted it to run and we did not spawn the build 
@rem ----------------------------------------------------------------------------
:UpdateStart
if "%SpawnBuild%" == "True" goto UpdateFinish
if /I "%UserInput%" NEQ "Y" goto UpdateFinish
@echo.
@echo Running the %CD%\update31_%BUILD_BITS%.bat.
call update31_%BUILD_BITS%.bat %GCC_ROOT% > nul
@echo.
@echo.
:UpdateFinish
goto Finish
