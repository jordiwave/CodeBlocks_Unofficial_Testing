...
# --------------------------------------------------------------------------------------#
#     This bash script builds wxWidgets on either :                                     #
#           - Windows using MSYS2 Mingw64                                               #
#           - Future - Linux                                                            #
# ------------------------------------------------------------------------------------------------------------
# GIT NOTES:
#     git clone --recurse-submodules https://github.com/wxWidgets/wxWidgets github_wxWidget_trunk
#     git clone -b v3.1.6 --single-branch --recurse-submodules https://github.com/wxWidgets/wxWidgets github_wxWidget_3.1.6
#
...
if [ "${GITHUB_ACTIONS}" != "true" ] ; then
    ...
    # file specified:
    exec > >(tee -i wxWidget_Build.log) 2>&1
...
if [ "${GITHUB_ACTIONS}" == "true" ] ; then
    # D:\a\CodeBlocks_Unofficial_Testing\CodeBlocks_Unofficial_Testing\Libraries\wxWidgets-3.1.6_win64'    
...
if [ ! -d "$WX_CB_BUILD_DIR" ]; then
    ...
    echo "|                                                                                                 |"
    echo "+-------------------------------------------------------------------------------------------------+"
    echo "Debug Info -Up one directory: ${PWD}/../"
    ls -la ${PWD}/..
    echo
    cd ${CurrentDir}
    exit 2
